...
class NaiveFockMatrix {
 ...
 private:
  ...
  double SingleIntegral_(int i, int j, int k, int l) {
    ...
    double constant = 0.25 * pow((math::PI/bandwidth_), 2.5);
    
    integral_value = integral_value * constant;
    
    //printf("constant %g\n", constant);

    double f_val = F_(bandwidth_ * four_way_dist);
    
    integral_value = integral_value * f_val;
        
    //printf("f_val %g\n", f_val);
        
    double exp_part = exp(-0.5 * bandwidth_ * (ij_dist + kl_dist));
    //printf("exp_part %g\n", exp_part);

    integral_value = integral_value * exp_part;
