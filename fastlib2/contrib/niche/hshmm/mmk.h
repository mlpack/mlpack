...
double Compute(LDS lds_1, LDS lds_2) {
  ...
  double Compute(const HMM<TDistribution> &hmm_a,
    ...
    for(int i = 0; i < n1; i++) {
      printf("pre1 %d\n", i);
      hmm_a.state_distributions()[i].mu().PrintDebug("mu");
      hmm_a.state_distributions()[i].sigma().PrintDebug("sigma");
    }

    for(int i = 0; i < n2; i++) {
      printf("pre2 %d\n", i);
      hmm_b.state_distributions()[i].mu().PrintDebug("mu");
      hmm_b.state_distributions()[i].sigma().PrintDebug("sigma");
    }
