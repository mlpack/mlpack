...
void SaveKFoldFMRIHMMs(int n_folds) {
  ...
  for(int fold_num = 0; fold_num < n_folds; fold_num++) {
    ...
    HMM<DiagGaussian> hmm;
    hmm.Init(n_states, n_dims, GAUSSIAN);
    hmm.InitParameters(selected_training_sequences, false);
    //hmm.PrintDebug("hmm after calling InitParameters(sequences)");
    hmm.ViterbiUpdate(selected_training_sequences, false);
    //hmm.PrintDebug("hmm after calling ViterbiUpdate(sequences)");
