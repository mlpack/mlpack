/**************************************************************************
Contains modul DBLAMPP.C of ZIB optimizer MCF
...
**************************************************************************/
/*  LAST EDIT: Tue Jun  3 10:41:24 2003 by Andreas Loebel (opt0.zib.de)  */
#ident "$Id: dblampp.c,v 1.1.1.1 2003/06/03 12:06:54 bzfloebe Exp $"
...
MCF_node_p BLAMPP(K,B)( DECLARE_DUAL_BLA_PARAMETERS )
{
    ...
    else
    {
        for( i = 2, next = 0; i <= B && i <= basket_size; i++ )
        {
            ...
            infeas = -flow;
#endif
            if( infeas > (MCF_flow_t)ZERO_EPS )
            {
                next++;
                perm[next]->a = node;
                perm[next]->infeas = infeas;
                perm[next]->abs_infeas = ABS(infeas);
            }
            else if( (infeas = (arc->upper - flow)) < (MCF_flow_t)(-ZERO_EPS) )
            {
                next++;
                perm[next]->a = node;
                perm[next]->infeas = infeas;
...
NEXT:
    ...
    for( node++; node < stop_nodes; node += nr_group )
    {
        ...
        if( infeas > (MCF_flow_t)ZERO_EPS )
        {
            ...
            perm[basket_size]->abs_infeas = ABS(infeas);
        }
        else if( (infeas = (arc->upper - flow)) < (MCF_flow_t)(-ZERO_EPS) )
        {
            basket_size++;
