...
void CalObsFreq(const ArrayList<Vector>& list, TriValMap* obsmap) {
  ...
  for (int i = 0; i < list.size(); i++) {
    for (int j = 0; j < list[i].length()-2; j++) {
      total += 1;
      TriValues obs(list[i][j], list[i][j+1], 0.0f);
  ...
  for (int rand_iter = 0; rand_iter < 500; rand_iter++) {
	...
	for (int s1 = 0; s1 < M; s1++)
	  for (int s2 = 0; s2 < M; s2++)
	      it->second += so[s1][obs.v1]*so[s2][obs.v2]
		*p[s1]*tr[s1][s2];//tri[s1][s2][s3];
      }
	...
	for (int s1 = 0; s1 < M; s1++)
	  for (int s2 = 0; s2 < M; s2++) {
	      ...
	      double con_so = so[s1][obs.v1]*so[s2][obs.v2];
	      double con_tr = tr[s1][s2];
	      double con_p  = p[s1];

	      af_dso[s1][obs.v1] += af*so[s2][obs.v2]
		*con_p*con_tr;
	      dso[s1][obs.v1] += so[s2][obs.v2]
		*con_p*con_tr;

	      af_dso[s2][obs.v2] += af*so[s1][obs.v1]
		*con_p*con_tr;
	      dso[s2][obs.v2] += so[s1][obs.v1]
		*con_p*con_tr;

	      af_dp[s1] += af*con_so*con_tr;
	      dp[s1] += con_so*con_tr;

	      af_dtr[s1][s2] += af*con_so*con_p;
	      dtr[s1][s2] += con_so*con_p;
