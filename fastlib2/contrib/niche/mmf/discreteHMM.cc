...
void DiscreteHMM::Train(const ArrayList<Vector>& seqs, Matrix* guessTR, Matrix* guessEM, int max_iter, double tol) {
  ...
  for (int iter = 0; iter < max_iter; iter++) {
    ...
    double s;
    for (int i = 1; i < M; i++) { // changed to i = 1 (used to be i = 0) - nishant
      s = 0;
	...
	for (int j = 0; j < M; j++) gTR.ref(i, j) = TR.get(i, j) / s;
      ...
      //printf("s = %f\n", s);
      if(s == 0) {
	double uniform_p = ((double)1) / ((double)N);
	for(int j = 0; j < N; j++) {
	  gEM.ref(i, j) = uniform_p;
	}
      }
      else {
