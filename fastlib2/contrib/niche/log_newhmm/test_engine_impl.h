...
void TestHMMGenMMKClassificationLog(const ArrayList<HMM<DiagGaussian> > &hmms,
				    const GenVector<int> &labels) {
  ...
  //GenerativeMMKBatch(lambda, rho, witness_length, hmms, &kernel_matrix);
  if(rho > 0) {
    PPKBatchLog(rho, witness_length, hmms, &kernel_matrix_log);
  }
  else {
    GenerativeMMKBatchLog(lambda, witness_length, hmms, &kernel_matrix_log);
  }
...
void TestMarkovMMKClassification(const ArrayList<GenMatrix<double> > &sequences,
  ...
  Matrix kernel_matrix;
  MarkovEmpiricalMMKBatch(lambda, order, sequences, &kernel_matrix);
  //NormalizeKernelMatrix(&kernel_matrix); // results are better with no normalization
