...
class SMD_SingleStep {
 ...
 public:
  ...
  void Eval(double *pt){
    ...
    long double f_old, f_new;
    double scale, scale_prev, eta = 0.1, gamma, mu = 0.05;
	...
	// but since we are using stochastic method, it is 
	// better to only consider \theta instead of f
	Vector diff;
	la::SubInit(pnew, pold, &diff);
	// f_tol = fabs(f_new - f_old);
	p_tol = sqrt(la::Dot(diff, diff));
...
class SMD_SingleStep {
 ...
 public:
  ...
  void Eval(double *pt){
    ...
    index_t dim = fx_param_int_req(opt_module_, "param_space_dim");
    index_t num_batch = fx_param_int(opt_module_, "BATCHES",50);
    Vector pold, pnew, eta, grad, prev_grad, one_vector;
    long double f_old, f_new;
    double scale, scale_prev, gamma;
    double mu = fx_param_double(opt_module_, "MU", 0.1);
    double p_tol = 0.0;//, f_tol = 0.0;
    Matrix data_batched, matrix_for_had_prod;
    index_t batch_size = fx_param_int(opt_module_,"BATCH_SIZE",
				      data().n_cols() / num_batch);
    ...
    eta.SetAll(0.1);
    one_vector.Init(dim);
    one_vector.SetAll(1.0);
    matrix_for_had_prod.Init(dim,dim);
    data_batched.Copy(data());
	...
	//	gamma = - eta / scale;
	gamma = -1.0;
	matrix_for_had_prod.SetDiagonal(eta);
	pnew.SetZero();
	la::AddTo(pold, &pnew);
	la::MulExpert(gamma, grad, matrix_for_had_prod, 1.0, &pnew);
	//printf("\n pold:");
	//for(index_t i = 0; i < dim; i++) {
	//  printf("%lf, ", pold.get(i));
	//}
	...
	//  printf("%lf, ", eta.get(i));
	//}
	Vector prev_grad_prod_eta;
	la::MulInit(prev_grad, matrix_for_had_prod, &prev_grad_prod_eta);
	matrix_for_had_prod.SetZero();
	matrix_for_had_prod.SetDiagonal(prev_grad_prod_eta);
	Vector grad_prod_prev_grad_prod_eta;
	la::MulInit(grad, matrix_for_had_prod, 
		    &grad_prod_prev_grad_prod_eta);
	Vector temp_eta;
	temp_eta.Copy(one_vector);
	la::AddExpert(mu, grad_prod_prev_grad_prod_eta, &temp_eta);
	//printf("\n 1+mu*.. : ");
	//for(index_t i = 0; i < dim; i++) {
	//  printf("%lf ",temp_eta.get(i));
	//}
	//printf("\n max(0.5, 1+mu*..) : ");
	for (index_t i = 0; i < dim; i++) {
	  if ((temp_eta.ptr())[i] < 0.5) {
	    (temp_eta.ptr())[i] = 0.5;
	  }
	}
	//for(index_t i = 0; i < dim; i++) {
	...
	//}
	matrix_for_had_prod.SetZero();
	matrix_for_had_prod.SetDiagonal(temp_eta);
	la::MulOverwrite(matrix_for_had_prod, eta, &temp_eta);
	eta.CopyValues(temp_eta);
	//printf("\n eta_new:");
	//for(index_t i = 0; i < dim; i++) {
	//  printf("%lf, ", eta.get(i));
	//}
