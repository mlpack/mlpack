...
class NWRCdeCommon {
 ...
 public:
  ...
  static void AddPostponed(TTree *node, NWRCdeQueryResult &query_results,
    ...
    query_results.numerator_sum_e[destination] += 
      node->stat().postponed.nwr_numerator_sum_e;
    query_results.denominator_sum_l[destination] += 
      node->stat().postponed.nwr_denominator_sum_l;
    query_results.denominator_sum_e[destination] += 
      node->stat().postponed.nwr_denominator_sum_e;
    query_results.n_pruned[destination] += 
      node->stat().postponed.n_pruned;
  }
  ...
  static void RefineBoundStatistics(NWRCdeQueryResult &query_results,
				    index_t q, TTree *qnode) {
    
    qnode->stat().query_summary.nwr_numerator_sum_l =
      std::min(qnode->stat().query_summary.nwr_numerator_sum_l,
	       query_results.nwr_numerator_sum_l[q]);
    qnode->stat().query_summary.nwr_denominator_sum_l =
      std::min(qnode->stat().query_summary.nwr_denominator_sum_l,
	       query_results.nwr_denominator_sum_l[q]);
    qnode->stat().query_summary.n_pruned_l =
      std::min(qnode->stat().query_summary.nwr_n_pruned_l,
	       query_results.n_pruned[q]);
    qnode->stat().query_summary.used_error_u =
      std::max(qnode->stat().query_summary.used_error_u,
	       query_results.used_error[q]);
  }
  ...
  template<typename TTree>
  static bool ConsiderPairExact(TTree *qnode, TTree *rnode, 
				double probability, NWRCdeDelta &delta) {
