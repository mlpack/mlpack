...
class CURDecomposition {
 ...
 public:
  ...
  static void Compute(const Matrix &a_mat,
		      ...
		      ArrayList<size_t> *column_indices,
    ...
    for(size_t i = 0; i < a_mat.n_cols(); i++) {
    ...
    int num_column_samples = std::max((int) (sqrt(a_mat.n_cols())), 1);

    column_indices->Init(num_column_samples);
    for(size_t s = 0; s < num_column_samples; s++) {
      double random_number = 
    ...
    for(size_t s = 0; s < num_row_samples; s++) {
      size_t sample_row_number = (*row_indices)[s];
      double probability = 
	((sample_row_number == 0) ?
	...
	r_mat->set(s, c, a_mat.get(sample_row_number, c) /
      ...
      for(size_t c = 0; c < num_column_samples; c++) {
    ...
    phi_mat.SetZero();
    for(size_t s = 0; s < singular_values.length(); s++) {
      if(singular_values[s] > 0.01) {
	for(size_t c = 0; c < phi_mat.n_cols(); c++) {
	  for(size_t r = 0; r < phi_mat.n_rows(); r++) {
