...
namespace proximity {
  ...
  public:
    ...
    (const arma::Mat<T>& lower_limit_matrix,
     const arma::Mat<T>& upper_limit_matrix, TKdTree *node, int split_dim) {
      return node->bound().get(split_dim).mid();
    }
  };
			 ...
			 std::vector<size_t> *new_from_old = NULL)
  {
    ...
    if (old_from_new)
    {
      ...
      {
        (*old_from_new).push_back (i);
      }

      old_from_new_ptr = &(*old_from_new)[0];
    }
    else {
