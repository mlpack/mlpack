...
class GenHrectBound {
 ...
 public:
  ...
  void StaticInit(size_t dimension)
  {
    ...
    Init (dimension); /* TODO: figure out what the hell this means */
  }
   ...
   */
  bool Contains(const std::vector<T>& point) const {
  ...
  {
    Log::Assert (i < dim_, "The index i is larger than dim_");
    return bounds_[i];
  }

  /** Calculates the midpoint of the range */
  void CalculateMidpoint(std::vector<T> *centroid) const {
    centroid->Init(dim_);
    for (size_t i = 0; i < dim_; i++) {
   ...
   * Calculates minimum bound-to-point squared distance.
   */
  double MinDistanceSq(const std::vector<T>& point) const {
   ...
   */
  double MaxDistanceSq(const std::vector<T>& point) const {
    double sum = 0;
   ...
   */
  GenRange<double> RangeDistanceSq(const std::vector<T>& point) const {
    double sum_lo = 0;
    double sum_hi = 0;
   ...
   */
  GenHrectBound& operator |= (const std::vector<T>& vector) {
