function[density_estimate_vec,contrib_mat,train_data,test_data]=reverse_local_rodeo(train_data,test_data,...
						  train_labels,test_labels,positive_prior,negative_prior)


%We shall do reverse local rodeo. here for each test point we shall calculate
...
if (nargin < 2)
  ...
  if(nargin==2)
    ...
    for i=1:num_test_points
      ...
      if mod(i, 200) == 0 
	display(sprintf('%d points done...', i));
      end
    end
  else
...
h_0 = 1e-2 / log(log(num_train_points));
...
while(length(active_dimensions)~=0)
  ...
  while(iter<=length(active_dimensions))
    ...
    if(abs(Z_j) < lambda_j)
      display('here');
      vector_bw(dimension_in_consideration)=...
	  vector_bw(dimension_in_consideration) / beta;
    else
