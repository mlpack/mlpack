...
void Objective::Init(fx_module *module) {
  ...
  size_t start_col=0;
  for(size_t i=0; i<info1.n_cols(); i++) {
    first_stage_x_[i].Init(x.n_rows(), (size_t)info1.get(0, i));
  ...
  start_col=0;
  for(size_t i=0; i<info1.n_cols(); i++) {
    second_stage_x_[i].Init(x.n_rows(), (size_t)info1.get(0,i));
	  //second_stage_x_[i].Init(first_stage_x_[0].n_rows(), (size_t)info2.get(0,i));
	
		second_stage_x_[i].CopyColumnFromMat(0, 
        start_col, 
        (size_t)info1.get(0,i),
        x);
    start_col+=(size_t)info1.get(0,i);
  }
  ...
  start_col=0;
  for(size_t i=0; i<info1.n_cols(); i++) {
    unknown_x_past_[i].Init(x.n_rows(), (size_t)info1.get(0,i));
	...
	size_t num_selected_people=first_stage_x_.size();
      ...
      alpha_temp=(l+1)*(alpha_weight_);
          ...
          pow((1-alpha_temp), q-1)/denumerator_beta_function_;
			...
			for(size_t i=0; i<first_stage_x_[n].n_cols(); i++){
				int count=0;
				for(size_t j=ind_unknown_x_[0]; j<=ind_unknown_x_[ind_unknown_x_.size()-1]; j++){
  ...
  //double sum=0;
	for(size_t n=0; n<first_stage_x_.size(); n++){
		...
		*/
		...
		for(size_t i=0; i<exp_betas_times_x1_.size(); i++){
