...
int main(int argc, char* argv[]) {
  ...
  const u32 n_train_per_class = (u32) fx_param_double_req(NULL, "n_train_per_class");
  ...
  u32 neg_cut_ind = n_train_per_class - 1; //(X_neg.n_cols / 2) - 1;
  u32 pos_cut_ind = n_train_per_class - 1; //(X_pos.n_cols / 2) - 1;
