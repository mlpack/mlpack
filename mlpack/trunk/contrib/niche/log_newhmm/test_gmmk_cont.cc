...
--exp=(name of experiment) (usually linked to size of dataset, like gunpoint100 versus gunpoint200, or dna500 versus dna1000. this determines the name for the saved HMMs and saved labels
...
--n_states=k (this determines the filename of the loaded HMMs)

--exp=(name of experiment) (this determines the filename of the loaded HMMs/labels)

--lambda=(smoothing parameter for mmk)

--witness_length=(evaluation sequence length for mmk)
...
void GetHMMs(int n_states,
  ...
  for(int i = 0; i < n_class1; i++) {
    ...
    hmms[i].Init(n_states, n_dims, GAUSSIAN, 1e-8, true);
  ...
  for(int i = 0; i < n_class0; i++) {
    ...
    class0_hmms[i].Init(n_states, n_dims, GAUSSIAN, 1e-8, true);
...
void LoadHMMs(ArrayList<HMM<DiagGaussian> >* p_hmms,
		   GenVector<int>* p_labels) {
  int n_states = fx_param_int_req(NULL, "n_states");
...
int main(int argc, char* argv[]) {
  ...
  if(strcmp(mode, "save") == 0) {
    ...
    TestHMMGenMMKClassificationLog(hmms, labels);
  }
  else {
