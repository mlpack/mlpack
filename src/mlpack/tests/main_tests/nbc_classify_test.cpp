/**
 * @file tests/main_tests/nbc_classify_test.cpp
 * @author Dirk Eddelbuettel
 *
 * Test RUN_BINDING() of nbc_classify_main.cpp
 *
...
#include <mlpack/methods/naive_bayes/nbc_classify_main.cpp>
...
BINDING_TEST_FIXTURE(NBCClassifyTestFixture);

/**
  * Ensuring that absence of input model is checked.
 **/
TEST_CASE_METHOD(NBCClassifyTestFixture,
                 "NBCClassifyNoModel",
                 "[NBCClassifyMainTest][BindingTests]")
{
  constexpr int D = 3;
  constexpr int M = 15;
 ...
 **/
TEST_CASE_METHOD(NBCClassifyTestFixture, "NBCClassifyEmptyModel",
                 "[NBCClassifyMainTest][BindingTests]")
{
  constexpr int D = 3;
  constexpr int M = 15;
 ...
 **/
TEST_CASE_METHOD(NBCClassifyTestFixture, "NBCClassifyNoData",
                 "[NBCClassifyMainTest][BindingTests]")
{
  constexpr int N = 10;
  constexpr int D = 3;
                 ...
                 "[NBCClassifyMainTest][BindingTests]")
{
  ...
  SetInputParam("input_model", std::move(model));
  SetInputParam("test", std::move(testX));
  // Required input data has wrong dimension.
  // Should throw invalid argument error.
  REQUIRE_THROWS_AS(RUN_BINDING(), std::runtime_error);
}
