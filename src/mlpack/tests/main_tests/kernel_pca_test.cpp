...
BOOST_AUTO_TEST_CASE(KernelPCA0DimensionalityTest)
{
  ...
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output").n_rows, 5);
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output").n_cols, 5);
}
...
BOOST_AUTO_TEST_CASE(KernelPCACenterTest)
{
  ...
  mlpackMain();
  arma::mat output1 = CMD::GetParam<arma::mat>("output");

  // Get output after centering the dataset.
  SetInputParam("input", std::move(x));
  SetInputParam("center", true);
  mlpackMain();
  arma::mat output2 = CMD::GetParam<arma::mat>("output");
...
BOOST_AUTO_TEST_CASE(KernelPCASamplingSchemeTest)
{
  ...
  mlpackMain();
  arma::mat output3 = CMD::GetParam<arma::mat>("output");
