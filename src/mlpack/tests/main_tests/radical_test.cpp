...
BOOST_AUTO_TEST_CASE(RadicalOutputDimensionTest)
{
  ...
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output_ic").n_rows, 5);
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output_ic").n_cols, 3);

  // Check dimension of W matrix.
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output_unmixing").n_rows, 5);
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output_unmixing").n_cols, 5);
}
...
BOOST_AUTO_TEST_CASE(RadicalDiffNoiseStdDevTest)
{
  ...
  arma::mat Y = CMD::GetParam<arma::mat>("output_ic");
  ...
  BOOST_REQUIRE_LT(arma::accu(Y ==
      CMD::GetParam<arma::mat>("output_ic")), Y.n_elem);
}
