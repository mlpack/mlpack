...
BOOST_AUTO_TEST_CASE(LSHOutputDimensionTest)
{
  ...
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::Mat<size_t>>("neighbors").n_rows, 6);
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::Mat<size_t>>("neighbors").n_cols,
                      100);

  // Check the distances matrix has 6 points for each of the 100 input points.
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("distances").n_rows, 6);
...
BOOST_AUTO_TEST_CASE(LSHModelValidityTest)
{
  ...
  SetInputParam("input_model", CMD::GetParam<LSHSearch<>*>("output_model"));
...
BOOST_AUTO_TEST_CASE(LSHDiffTablesTest)
{
  ...
  arma::Mat<size_t> neighbors = CMD::GetParam<arma::Mat<size_t>>("neighbors");
  arma::mat distances = CMD::GetParam<arma::mat>("distances");
  ...
  BOOST_REQUIRE_LT(arma::accu(neighbors ==
      CMD::GetParam<arma::Mat<size_t>>("neighbors")), neighbors.n_elem);
  BOOST_REQUIRE_LT(arma::accu(distances ==
      CMD::GetParam<arma::mat>("distances")), distances.n_elem);
}
