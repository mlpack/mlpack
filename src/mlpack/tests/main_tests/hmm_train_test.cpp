                 ...
                 "[HMMTrainMainTest][BindingTests]")
{
  ...
  if (!data::Load(inputObsFileName, trainObs))
    FAIL("Cannot load dataset hmm_train_obs.csv");
  if (!data::Load(inputLabFileName, trainLab))
    FAIL("Cannot load dataset hmm_train_lab.csv");
