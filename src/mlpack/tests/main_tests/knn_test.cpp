...
BOOST_AUTO_TEST_CASE(KNNRefModelTest)
{
  ...
  SetInputParam("input_model",
      std::move(CMD::GetParam<KNNModel*>("output_model")));
...
BOOST_AUTO_TEST_CASE(KNNInvalidRhoTest)
{
  ...
  CMD::GetSingleton().Parameters()["reference"].wasPassed = false;
  CMD::GetSingleton().Parameters()["rho"].wasPassed = false;
...
BOOST_AUTO_TEST_CASE(KNNModelReuseTest)
{
  ...
  output_model = std::move(CMD::GetParam<KNNModel*>("output_model"));
...
BOOST_AUTO_TEST_CASE(KNNDifferentTauTest)
{
  ...
  CMD::GetSingleton().Parameters()["reference"].wasPassed = false;
  CMD::GetSingleton().Parameters()["tau"].wasPassed = false;
...
BOOST_AUTO_TEST_CASE(KNNRandomBasisTest)
{
  ...
  neighbors = std::move(CMD::GetParam<arma::Mat<size_t>>("neighbors"));
  distances = std::move(CMD::GetParam<arma::mat>("distances"));
  BOOST_REQUIRE_EQUAL(CMD::GetParam<KNNModel*>("output_model")->RandomBasis(),
      true);
...
BOOST_AUTO_TEST_CASE(KNNAllAlgorithmsTest)
{
  ...
  for (int i = 0; i < nofalgorithms; ++i)
  {
    ...
    delete CMD::GetParam<KNNModel*>("output_model");
    CMD::GetParam<KNNModel*>("output_model") = NULL;
...
BOOST_AUTO_TEST_CASE(KNNDifferentLeafSizes)
{
  ...
  BOOST_CHECK_EQUAL(CMD::GetParam<KNNModel*>("output_model")->LeafSize(),
    (int) 10);
  delete output_model;
}
