                 ...
                 "[HMMGenerateMainTest][BindingTests]")
{
  ...
  *(h->DiagGMMHMM()) = HMM<DiagonalGMM>(2, DiagonalGMM(2, 2));
  ...
  h->DiagGMMHMM()->Emission()[0].Component(0) = DiagonalGaussianDistribution<arma::mat, arma::vec>(
      "2.75 1.60", "0.50 0.50");
  h->DiagGMMHMM()->Emission()[0].Component(1) = DiagonalGaussianDistribution<arma::mat, arma::vec>(
      "6.15 2.51", "1.00 1.50");
  h->DiagGMMHMM()->Emission()[1] = DiagonalGMM(2, 2);
  h->DiagGMMHMM()->Emission()[1].Weights() = arma::vec("0.4 0.6");
  h->DiagGMMHMM()->Emission()[1].Component(0) = DiagonalGaussianDistribution<arma::mat, arma::vec>(
      "-1.00 -3.42", "0.20 1.00");
  h->DiagGMMHMM()->Emission()[1].Component(1) = DiagonalGaussianDistribution<arma::mat, arma::vec>(
      "-3.10 -5.05", "1.20 0.80");
