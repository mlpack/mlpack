...
BOOST_AUTO_TEST_CASE(PreprocessSplitDimensionTest)
{
  ...
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("training").n_cols,
                      std::ceil(0.9 * inputSize));
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("test").n_cols,
                      std::floor(0.1 * inputSize));

  BOOST_REQUIRE_EQUAL(
      CMD::GetParam<arma::Mat<size_t>>("training_labels").n_cols,
      std::ceil(0.9 * labelSize));
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::Mat<size_t>>("test_labels").n_cols,
...
BOOST_AUTO_TEST_CASE(PreprocessSplitZeroTestRatioTest)
{
  ...
  // Now check that the output has desired dimensions.
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("training").n_cols, inputSize);
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("test").n_cols, 0);

  BOOST_REQUIRE_EQUAL(
      CMD::GetParam<arma::Mat<size_t>>("training_labels").n_cols, labelSize);
  BOOST_REQUIRE_EQUAL(
      CMD::GetParam<arma::Mat<size_t>>("test_labels").n_cols, 0);
}
...
BOOST_AUTO_TEST_CASE(PreprocessSplitUnityTestRatioTest)
{
  ...
  // Now check that the output has desired dimensions.
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("training").n_cols, 0);
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("test").n_cols, inputSize);

  BOOST_REQUIRE_EQUAL(
      CMD::GetParam<arma::Mat<size_t>>("training_labels").n_cols, 0);
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::Mat<size_t>>("test_labels").n_cols,
      labelSize);
}
