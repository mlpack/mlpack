...
BOOST_AUTO_TEST_CASE(LRDifferentLambdas)
{
  ...
  // The first solution.
  mlpackMain();
  const double testY1 = CMD::GetParam<arma::rowvec>("output_predictions")(0);
  ...
  // The second solution.
  mlpackMain();
  const double testY2 = CMD::GetParam<arma::rowvec>("output_predictions")(0);
...
BOOST_AUTO_TEST_CASE(LRModelReload)
{
  ...
  LinearRegression* model = CMD::GetParam<LinearRegression*>("output_model");
  const arma::rowvec testY1 = CMD::GetParam<arma::rowvec>("output_predictions");
  ...
  const arma::rowvec testY2 = CMD::GetParam<arma::rowvec>("output_predictions");
...
BOOST_AUTO_TEST_CASE(LRPredictionSizeCheck)
{
  ...
  const arma::rowvec testY = CMD::GetParam<arma::rowvec>("output_predictions");
