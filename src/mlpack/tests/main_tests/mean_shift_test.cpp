...
BOOST_AUTO_TEST_CASE(MeanShiftOutputDimensionTest)
{
  ...
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output").n_rows, 3 + 1);
  // Check number of output points are the same.
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output").n_cols, 100);
}
...
BOOST_AUTO_TEST_CASE(MeanShiftInPlaceTest)
{
  ...
  // Now check that the output has 1 extra row for labels.
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output").n_rows, numRows + 1);
  // Check number of output points are the same.
  BOOST_REQUIRE_EQUAL(CMD::GetParam<arma::mat>("output").n_cols, numCols);
}
...
BOOST_AUTO_TEST_CASE(MeanShiftForceConvergenceTest)
{
  ...
  const int numCentroids1 = CMD::GetParam<arma::mat>("centroid").n_cols;
  ...
  const int numCentroids2 = CMD::GetParam<arma::mat>("centroid").n_cols;
