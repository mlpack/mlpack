...
BOOST_AUTO_TEST_CASE(CFModelReuseTest)
{
  ...
  for (std::string& algorithm : algorithms)
  {
    ...
    CMD::GetSingleton().Parameters()["max_iterations"].wasPassed = false;
    ...
    SetInputParam("input_model",
        std::move(CMD::GetParam<CFModel*>("output_model")));

    mlpackMain();

    const Mat<size_t>& output = CMD::GetParam<Mat<size_t>>("output");
...
BOOST_AUTO_TEST_CASE(CFRankTest)
{
  ...
  const CFModel* outputModel = CMD::GetParam<CFModel*>("output_model");
...
BOOST_AUTO_TEST_CASE(CFMinResidueTest)
{
  ...
  outputModel =  CMD::GetParam<CFModel*>("output_model");
...
BOOST_AUTO_TEST_CASE(CFNeighborhoodTest)
{
  ...
  const arma::Mat<size_t> output1 = CMD::GetParam<arma::Mat<size_t>>("output");
  ...
  const arma::Mat<size_t> output2 = CMD::GetParam<arma::Mat<size_t>>("output");
...
BOOST_AUTO_TEST_CASE(CFInterpolationTest)
{
  ...
  const arma::Mat<size_t> output3 = CMD::GetParam<arma::Mat<size_t>>("output");
