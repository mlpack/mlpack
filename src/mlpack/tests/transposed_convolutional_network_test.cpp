/**
 * @file transposed_convolutional_network_test.cpp
 * @author Shikhar Jaiswal
 *
 * Tests the transposed convolutional neural network.
 *
...
BOOST_AUTO_TEST_SUITE(TransposedConvolutionalNetworkTest);
...
BOOST_AUTO_TEST_CASE(VanillaNetworkTest)
{
  ...
  /*
   * Construct a transposed convolutional neural network with a 28x28x1 input
   * layer, 24x24x8 transposed convolution layer, 12x12x8 pooling layer, 8x8x12
   * transposed convolution layer and a 4x4x12 pooling layer which is fully
   * connected with the output layer. The network structure looks like:
   *
  ...
  model.Add<TransposedConvolution<> >(1, 8, 5, 5, 1, 1, 0, 0, 28, 28);
  model.Add<ReLULayer<> >();
  model.Add<MaxPooling<> >(8, 8, 2, 2);
  model.Add<TransposedConvolution<> >(8, 12, 2, 2);
  model.Add<ReLULayer<> >();
