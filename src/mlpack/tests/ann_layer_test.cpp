...
BOOST_AUTO_TEST_CASE(ScaledDotProductAttentionTest)
{
  ...
  arma::mat key = arma::mat("0.4998 0.0864;\
                             0.4194 0.2730;\
                             0.7443 0.6238;\
                             0.2492 0.2221;\
                             0.2393 0.2254;\
  ...
  arma::mat value = arma::mat("0.7770 0.3621;\
                               0.5839 0.7364;\
                               0.9503 0.5194;\
                               0.4381 0.4230;\
                               0.3223 0.0290;\
  ...
  // Test the Forward function.
  module.Forward(input, output);
  arma::mat expOutput = arma::mat("0.419034 0.408245;\
                                   0.504154 0.640306;\
                                   0.547339 0.555034;\
                                   0.534132 0.724583;\
                                   0.417282 0.408423;\
  ...
  // Test the Backward function.
  arma::mat gy = arma::ones(embedDim * tLen, batchSize);
  arma::mat g;
  module.Backward(input, gy, g);
  arma::mat expGrad = arma::mat("0.010093 0.051616;\
                                 0.011682 0.021405;\
                                -0.009071 -0.014949;\
                                 0.028626 -0.005551;\
                                 0.014471 0.051683;\
  ...
  for (size_t i = 0; i < batchSize; ++i)
  {
    for (size_t j = 0; j < expGrad.n_rows; ++j)
    {
      BOOST_REQUIRE_CLOSE_FRACTION(g(j, i), expGrad(j, i), 1e-04);
    }
  }
  ...
  {
    for (size_t j = 0; j < expGrad.n_rows; ++j)
      BOOST_REQUIRE_CLOSE_FRACTION(g(j, i), expGrad(j, i), 1e-03);
  }
