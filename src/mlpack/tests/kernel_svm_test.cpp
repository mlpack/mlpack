...
/**
 * Test training of kernel svm for two classes on a complex gaussian dataset
 * using smo optimizer.
 */
BOOST_AUTO_TEST_CASE(GaussianKernelSVMFitIntercept)
{
  const size_t points = 1000;
  const size_t inputSize = 3;
  ...
  for (size_t trial = 0; trial < 5; ++trial)
  {
    ...
    // Now train a svm object on it.
    KernelSVM<arma::mat,
              kernel::GaussianKernel> svm(data,
                                labels, 1.0, false, 10);
