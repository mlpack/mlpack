...
BOOST_AUTO_TEST_CASE(AdaGradSpeedUpTestFunction)
{
  SGDTestFunction f;
  AdaGradUpdate adagradUpdate(1e-8);
  AdaGrad<SGDTestFunction> s(f, 0.0003, 2500000, 1e-9, true, adagradUpdate);
...
BOOST_AUTO_TEST_CASE(AdaGradGeneralizedRosenbrockTest)
{
  ...
  for (size_t i = 10; i < 50; i += 5)
  {
    ...
    AdaGradUpdate adagradUpdate(1e-8);
    AdaGrad<GeneralizedRosenbrockFunction> s(f, 0.001, 0, 1e-15, true, adagradUpdate);
