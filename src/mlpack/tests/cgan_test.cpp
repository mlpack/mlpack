...
BOOST_AUTO_TEST_SUITE(CGANNetworkTest);

/*
 * Tests the standard CGAN implementation on the MNIST dataset.
 ...
 */
BOOST_AUTO_TEST_CASE(CGANMNISTTest)
{
  ...
  discriminator.Add<ReLULayer<> >();
  discriminator.Add<MeanPooling<> >(2, 2, 2, 2);
  discriminator.Add<Concatenate<> >();
  discriminator.Add<Linear<> >(7 * 7 * 2 * dNumKernels + yDim, 1024);
  discriminator.Add<ReLULayer<> >();
