...
class UndirectedGraph
{
 ...
 private:
  void ComputeVertices() { numVertices = max(max(edges)) + 1; }
                     ...
                     const arma::mat& Z)
{
  ...
  {
    BOOST_REQUIRE_SMALL(fabs(arma::dot(sdp.SparseA()[i], X) - sdp.SparseB()[i]),
                        1e-5);
  }

  for (size_t i = 0; i < sdp.NumDenseConstraints(); i++)
  {
    BOOST_REQUIRE_SMALL(fabs(arma::dot(sdp.DenseA()[i], X) - sdp.DenseB()[i]),
                        1e-5);
  }
...
static inline arma::sp_mat RepeatBlockDiag(const arma::sp_mat& block,
                                           size_t repeat)
{
...
static inline arma::sp_mat BlockDiag(const std::vector<arma::sp_mat>& blocks)
{
...
static inline arma::mat RandomOrthogonalMatrix(size_t rows, size_t cols)
{
  arma::mat Q, R;
...
static inline arma::mat RandomFullRowRankMatrix(size_t rows, size_t cols)
{
