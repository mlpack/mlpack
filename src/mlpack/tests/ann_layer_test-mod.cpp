/**
 * @file mlpack/tests/ann_layer_test-mod.cpp
 * @author Marcus Edel
 * @author Praveen Ch
 *
...
{
  arma::mat output, input, input2, input3, error, outputA, outputB;
  ...
  moduleA.Forward(std::move(input), std::move(outputA));
  moduleB.Forward(std::move(input2), std::move(outputB));
  ...
  for (size_t axis = 0; axis < 3; ++axis)
  {
    ...
    module.Add(moduleA);
    module.Add(moduleB);
    arma::mat tmpInput(input3);
    module.Forward(std::move(tmpInput), std::move(output));
