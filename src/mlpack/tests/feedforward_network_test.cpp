...
BOOST_AUTO_TEST_CASE(VanillaNetworkTest)
{
  ...
  arma::mat trainData =
      dataset.submat(0, 0, dataset.n_rows - 4, dataset.n_cols - 1);

  arma::mat trainLabelsTemp = dataset.submat(
      dataset.n_rows - 3, 0, dataset.n_rows - 1, dataset.n_cols - 1);
  arma::mat trainLabels = arma::zeros<arma::mat>(1, trainLabelsTemp.n_cols);
  for (size_t i = 0; i < trainLabelsTemp.n_cols; ++i)
  {
    trainLabels(i) =
        arma::as_scalar(arma::find(
            arma::max(trainLabelsTemp.col(i)) == trainLabelsTemp.col(i), 1))
        + 1;
  }
  ...
  arma::mat testLabelsTemp = dataset.submat(
      dataset.n_rows - 3, 0, dataset.n_rows - 1, dataset.n_cols - 1);
  ...
  for (size_t i = 0; i < testLabels.n_cols; ++i)
  {
    testLabels(i) =
        arma::as_scalar(arma::find(
            arma::max(testLabelsTemp.col(i)) == testLabelsTemp.col(i), 1))
        + 1;
  }
  ...
  // Vanilla neural net with logistic activation function.
  BuildVanillaNetwork<>(dataset, labels, dataset, labels, 2, 10, 10, 0.2);
}
...
BOOST_AUTO_TEST_CASE(ForwardBackwardTest)
{
  ...
  while (iteration < 100)
  {
    ...
    {
      size_t batchEnd =
          std::min(batchStart + batchSize, (size_t)dataset.n_cols);
...
BOOST_AUTO_TEST_CASE(DropoutNetworkTest)
{
  ...
  // Vanilla neural net with logistic activation function.
  BuildDropoutNetwork<>(dataset, labels, dataset, labels, 2, 10, 10, 0.2);
}
...
BOOST_AUTO_TEST_CASE(DropConnectNetworkTest)
{
  ...
  // Vanilla neural net with logistic activation function.
  BuildDropConnectNetwork<>(dataset, labels, dataset, labels, 2, 10, 10, 0.2);
}
...
BOOST_AUTO_TEST_CASE(SerializationTest)
{
  ...
  CheckMatrices(
      predictions, xmlPredictions, textPredictions, binaryPredictions);
}
