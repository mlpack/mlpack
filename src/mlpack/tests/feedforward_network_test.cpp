...
/**
 * Train and evaluate a Maxout network(with a baselayer) with the
 * specified structure.
 */
template<typename MatType = arma::mat>
void BuildMaxoutNetwork(MatType& trainData,
                        MatType& trainLabels,
                        MatType& testData,
                        MatType& testLabels,
                        const size_t outputSize,
                        const size_t maxoutHiddenLayerSize,
                        const size_t maxoutUnitSize,
                        const size_t maxEpochs,
                        const double classificationErrorThreshold)
{
 /*
  *  Construct a feed forward network with trainData.n_rows input nodes,
  *  maxoutUnitSize hidden nodes and trainLabels.n_rows output nodes. The
  *  network struct that looks like:
  *
  *  Input         Maxout         Output
  *  Layer         Layer          Layer
  ...
  */
  ...
  FFN<NegativeLogLikelihood<> > model;
  model.Add<Maxout<> >(trainData.n_rows, maxoutHiddenLayerSize, maxoutUnitSize);
  model.Add<Linear<> >(maxoutUnitSize, outputSize);
  model.Add<LogSoftMax<> >();
...
/**
 * Train the maxout network on a larger dataset.
 */
BOOST_AUTO_TEST_CASE(MaxoutNetworkTest)
{
  // Load the dataset.
  arma::mat dataset;
  data::Load("thyroid_train.csv", dataset, true);
