      ...
      distanceComps(0)
{
  ...
  if (localMetric) this->metric = new MetricType();
      ...
      distanceComps(0)
{
  ...
  if (parent == NULL && localDataset)
  {
    ...
    for (size_t i = 0; i < NumChildren(); ++i) queue.push(children[i]);
...
CoverTree<MetricType, StatisticType, MatType, RootPointPolicy>::~CoverTree()
{
  ...
  if (localMetric) delete metric;

  // Delete the local dataset, if necessary.
  if (localDataset) delete dataset;
}
...
{
  return std::max(
      metric->Evaluate(dataset->col(point), other) - furthestDescendantDistance,
      0.0);
}
    ...
    size_t& usedSetSize)
{
  ...
  if (maxDistance == 0)
  {
    ...
    for (size_t i = 0; i < nearSetSize; ++i)
    {
      ...
      children.push_back(new CoverTree(*dataset, base, indices[i], INT_MIN,
                                       this, distances[i], indices, distances,
                                       0, tempSize, usedSetSize, *metric));
      distanceComps += children.back()->DistanceComps();
      usedSetSize++;
    }
  ...
  children.push_back(new CoverTree(*dataset, base, point, nextScale, this, 0,
                                   indices, distances, childNearSetSize,
  ...
  while (nearSetSize > 0)
  {
    ...
    if ((nearSetSize == 1) && (farSetSize == 0))
    {
      ...
      children.push_back(new CoverTree(
          *dataset, base, indices[0], nextScale, this, distances[0], indices,
          distances, childNearSetSize, farSetSize, usedSetSize, *metric));
      distanceComps += children.back()->DistanceComps();
    ...
    childNearSetSize = SplitNearFar(childIndices, childDistances, bound,
                                    nearSetSize + farSetSize - 1);
    childFarSetSize =
        PruneFarSet(childIndices, childDistances, base * bound,
                    childNearSetSize, (nearSetSize + farSetSize - 1));
    ...
    children.push_back(
        new CoverTree(*dataset, base, indices[0], nextScale, this, distances[0],
                      childIndices, childDistances, childNearSetSize,
                      childFarSetSize, childUsedSetSize, *metric));
    numDescendants += children.back()->NumDescendants();
  ...
  for (size_t i = (nearSetSize + farSetSize);
       i < (nearSetSize + farSetSize + usedSetSize); ++i)
    ...
    const size_t pointSetSize)
{
  ...
  while ((distances[left] <= bound) && (left != right)) ++left;
  while ((distances[right] > bound) && (left != right)) --right;
    ...
    const size_t farSetSize)
{
  ...
  // zero.
  if (bufferSize == 0) return (childFarSetSize + farSetSize);
    ...
    Archive& ar, const unsigned int /* version */)
{
  ...
  if (Archive::is_loading::value)
  {
    ...
    if (localMetric && metric) delete metric;
    if (localDataset && dataset) delete dataset;
  }
