        ...
        std::priority_queue<QueueFrameType>& referenceQueue)
{
  ...
  while (!referenceQueue.empty())
  {
    ...
    else if ((!queryNode.IsLeaf()) && referenceNode.IsLeaf())
    {
      ...
      QueueFrameType fl = {queryNode.Left(),
                           &referenceNode,
                           queryDepth + 1,
                           score,
                           rule.TraversalInfo()};
      ...
      QueueFrameType fr = {
          queryNode.Right(), &referenceNode, queryDepth + 1, score, ti};
    ...
    else if (queryNode.IsLeaf() && (!referenceNode.IsLeaf()))
    {
      ...
      QueueFrameType fr = {
          &queryNode, referenceNode.Right(), queryDepth, score, ti};
    ...
    else
    {
      ...
      // correctly.
      QueueFrameType fll = {queryNode.Left(),
                            referenceNode.Left(),
                            queryDepth + 1,
                            score,
                            rule.TraversalInfo()};
      ...
      QueueFrameType flr = {queryNode.Left(),
                            referenceNode.Right(),
                            queryDepth + 1,
                            score,
                            rule.TraversalInfo()};
      ...
      QueueFrameType frl = {queryNode.Right(),
                            referenceNode.Left(),
                            queryDepth + 1,
                            score,
                            rule.TraversalInfo()};
      ...
      QueueFrameType frr = {queryNode.Right(),
                            referenceNode.Right(),
                            queryDepth + 1,
                            score,
                            rule.TraversalInfo()};
