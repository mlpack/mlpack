...
RectangleTree<
    ...
    SplitType,
    DescentType,
    AuxiliaryInformationType>::RectangleTree(const MatType& data,
    ...
    : maxNumChildren(maxNumChildren),
      ...
      numChildren(0),
      children(maxNumChildren +
               1), // Add one to make splitting the node simpler.
      parent(NULL),
      begin(0),
      count(0),
      numDescendants(0),
      ...
      auxiliaryInfo(this)
{
  ...
  for (size_t i = firstDataIndex; i < data.n_cols; i++) root->InsertPoint(i);
}
...
RectangleTree<
    ...
    SplitType,
    DescentType,
    AuxiliaryInformationType>::RectangleTree(MatType&& data,
                                             const size_t maxLeafSize,
                                             const size_t minLeafSize,
                                             const size_t maxNumChildren,
                                             const size_t minNumChildren,
      ...
      auxiliaryInfo(std::move(other.auxiliaryInfo))
{
  ...
  {
    size_t iChild = 0;
    while (parent->children[iChild] != (&other)) iChild++;
    assert(iChild < numChildren);
    parent->children[iChild] = this;
  }
  if (!IsLeaf())
  {
    for (size_t i = 0; i < numChildren; i++) children[i]->parent = this;
  }
...
{
  for (size_t i = 0; i < numChildren; i++) delete children[i];

  if (ownsDataset) delete dataset;
}
                   ...
                   AuxiliaryInformationType>::SoftDelete()
{
  ...
  for (size_t i = 0; i < children.size(); i++) children[i] = NULL;
                   ...
                   AuxiliaryInformationType>::InsertPoint(const size_t point)
{
  ...
  for (size_t i = 0; i < lvls.size(); i++) lvls[i] = true;
                   ...
                   AuxiliaryInformationType>::DeletePoint(const size_t point)
{
  ...
  for (size_t i = 0; i < numChildren; i++)
    if (children[i]->Bound().Contains(dataset->col(point)))
      if (children[i]->DeletePoint(point, lvls)) return true;
                                                              ...
                                                              relevels)
{
  ...
  for (size_t i = 0; i < numChildren; i++)
    if (children[i]->Bound().Contains(dataset->col(point)))
      if (children[i]->DeletePoint(point, relevels)) return true;
...
bool RectangleTree<
    ...
    SplitType,
    DescentType,
    AuxiliaryInformationType>::RemoveNode(const RectangleTree* node,
                                          std::vector<bool>& relevels)
{
  for (size_t i = 0; i < numChildren; i++)
  {
    ...
    if (contains)
      if (children[i]->RemoveNode(node, relevels)) return true;
  }
...
{
  int n = 0;
  for (int i = 0; i < numChildren; i++) n += children[i]->TreeSize();
              ...
              AuxiliaryInformationType>::Descendant(const size_t index) const
{
  ...
  else
  {
    ...
    for (size_t i = 0; i < numChildren; ++i)
    {
      ...
      if (index - n < nd) return children[i]->Descendant(index - n);
      n += nd;
    }
                                                            ...
                                                            relevels)
{
  ...
  else
  {
    ...
    if (numChildren <= maxNumChildren) return; // We don't need to split.
...
RectangleTree<MetricType,
              ...
              AuxiliaryInformationType>::RectangleTree()
    : maxNumChildren(
          0), // Try to give sensible defaults, but it shouldn't matter
      minNumChildren(
          0),         // because this tree isn't valid anyway and is only used
...
void RectangleTree<
    ...
    SplitType,
    DescentType,
    AuxiliaryInformationType>::CondenseTree(const arma::vec& point,
                                            std::vector<bool>& relevels,
                                            const bool usePoint)
{
  ...
  if (IsLeaf() && count < minLeafSize && parent != NULL)
  {
    ...
    for (size_t i = 0; i < parent->NumChildren(); i++)
    {
      if (parent->children[i] == this)
      {
        ...
        {
          if (stillShrinking) stillShrinking = root->ShrinkBoundForBound(bound);
          root = root->Parent();
        }
        if (stillShrinking) stillShrinking = root->ShrinkBoundForBound(bound);
...
bool RectangleTree<
    ...
    SplitType,
    DescentType,
    AuxiliaryInformationType>::ShrinkBoundForPoint(const arma::vec& point)
{
  bool shrunk = false;
  if (IsLeaf())
  {
    ...
    ShrinkBoundForBound(const bound::HRectBound<MetricType>& /* b */)
{
  ...
  ElemType sum2 = 0;
  for (size_t i = 0; i < bound.Dim(); i++) sum2 += bound[i].Width();
                                         ...
                                         const unsigned int /* version */)
{
  ...
  if (Archive::is_loading::value)
  {
    ...
    if (ownsDataset && dataset) delete dataset;
  }
  ...
  if (Archive::is_loading::value && parent == NULL) ownsDataset = true;
