      ...
      minWidth(other.MinWidth())
{
  ...
  for (size_t i = 0; i < dim; i++) bounds[i] = other[i];
}
...
{
  for (size_t i = 0; i < dim; i++) bounds[i] = math::RangeType<ElemType>();
  minWidth = 0;
}
...
inline ElemType HRectBound<MetricType, ElemType>::Volume() const
{
  ...
  {
    if (bounds[i].Lo() >= bounds[i].Hi()) return 0;
 ...
 */
template<typename MetricType, typename ElemType>
ElemType HRectBound<MetricType, ElemType>::MinDistance(
    ...
    const HRectBound& bound) const
{
  for (size_t i = 0; i < dim; i++)
  {
    ...
    if (r_a.Hi() <= r_b.Lo() ||
        r_a.Lo() >= r_b.Hi()) // If a does not overlap b at all.
      return false;
  }
