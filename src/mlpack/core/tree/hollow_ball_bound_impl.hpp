    ...
    const HollowBallBound& other) const
{
  ...
  else
  {
    ...
    // bound.
    bool containOnOneSide =
        (hollowCenterDist - other.radii.Hi() >= radii.Lo()) &&
    ...
    const HollowBallBound& other) const
{
  ...
  else
  {
    ...
    const ElemType innerDistance1 =
        ...
        radii.Hi();
    if (innerDistance1 >= 0) return innerDistance1;
...
HollowBallBound<TMetricType, ElemType>::operator|=(const MatType& data)
{
  ...
  for (size_t i = 0; i < data.n_cols; ++i)
  {
    ...
    if (hollowDist < radii.Lo()) radii.Lo() = hollowDist;
  }
...
HollowBallBound<TMetricType, ElemType>::operator|=(const HollowBallBound& other)
{
  ...
  // second bound.
  if (radii.Lo() > innerDist) radii.Lo() = innerDist;
