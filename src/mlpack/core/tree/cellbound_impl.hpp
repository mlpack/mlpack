...
inline CellBound<MetricType, ElemType>::CellBound()
  : dim(0), bounds(NULL), loBound(arma::Mat<ElemType>()),
    hiBound(arma::Mat<ElemType>()), numBounds(0),
    loAddress(arma::Col<AddressElemType>()),
    hiAddress(arma::Col<AddressElemType>()), minWidth(0)
{ /* Nothing to do. */
}
...
inline CellBound<MetricType, ElemType>::CellBound(const size_t dimension)
  : dim(dimension), bounds(new math::RangeType<ElemType>[dim]),
    loBound(arma::Mat<ElemType>(dim, maxNumBounds)),
    hiBound(arma::Mat<ElemType>(dim, maxNumBounds)), numBounds(0),
    loAddress(dim), hiAddress(dim), minWidth(0)
{
...
inline CellBound<MetricType, ElemType>::CellBound(
  ...
  : dim(other.Dim()), bounds(new math::RangeType<ElemType>[dim]),
    loBound(other.loBound), hiBound(other.hiBound), numBounds(other.numBounds),
    loAddress(other.loAddress), hiAddress(other.hiAddress),
 ...
 */
template<typename MetricType, typename ElemType>
inline CellBound<MetricType, ElemType>& CellBound<MetricType, ElemType>::
operator=(const CellBound<MetricType, ElemType>& other)
{
  if (dim != other.Dim())
  {
...
inline CellBound<MetricType, ElemType>::CellBound(
  ...
  : dim(other.dim), bounds(other.bounds), loBound(std::move(other.loBound)),
    hiBound(std::move(other.hiBound)), numBounds(std::move(other.numBounds)),
    loAddress(std::move(other.loAddress)),
    hiAddress(std::move(other.hiAddress)), minWidth(other.minWidth)
{
  // Fix the other bound.
  other.dim = 0;
  other.bounds = NULL;
...
template<typename MetricType, typename ElemType>
inline void
CellBound<MetricType, ElemType>::Center(arma::Col<ElemType>& center) const
{
    ...
    typename std::enable_if_t<IsVector<VecType>::value>* /* junk */) const
{
  ...
  {
    if (MetricType::TakeRoot)
      return (ElemType)pow((double)minSum, 1.0 / (double)MetricType::Power)
             / 2.0;
    else
...
template<typename MetricType, typename ElemType>
ElemType
CellBound<MetricType, ElemType>::MinDistance(const CellBound& other) const
{
...
template<typename MetricType, typename ElemType>
inline ElemType
CellBound<MetricType, ElemType>::MaxDistance(const CellBound& other) const
{
  ElemType maxSum = std::numeric_limits<ElemType>::lowest();
...
CellBound<MetricType, ElemType>::RangeDistance(const CellBound& other) const
{
...
template<typename MetricType, typename ElemType>
template<typename VecType>
inline math::RangeType<ElemType> CellBound<MetricType, ElemType>::RangeDistance(
    const VecType& point,
    typename std::enable_if_t<IsVector<VecType>::value>* /* junk */) const
{
  ElemType minLoSum = std::numeric_limits<ElemType>::max();
...
template<typename MetricType, typename ElemType>
inline CellBound<MetricType, ElemType>& CellBound<MetricType, ElemType>::
operator|=(const CellBound& other)
{
...
template<typename VecType>
inline bool
CellBound<MetricType, ElemType>::Contains(const VecType& point) const
{
...
inline ElemType CellBound<MetricType, ElemType>::Diameter() const
{
  ElemType d = 0;
  for (size_t i = 0; i < dim; ++i)
    d += std::pow(bounds[i].Hi() - bounds[i].Lo(), (ElemType)MetricType::Power);
