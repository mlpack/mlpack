 ...
 */
#ifndef MLPACK_CORE_OPTIMIZERS_DOWNPOUR_SGD_DOWNPOUR_SGD_HPP
#define MLPACK_CORE_OPTIMIZERS_DOWNPOUR_SGD_DOWNPOUR_SGD_HPP
...
/**
 * Downpour Stochastic Gradient Descent is a technique for minimizing a
 ...
 * \f]
 *
 * and our task is to minimize \f$ A \f$.  Downpour SGD iterates over batches
 ...
 * point.  Then, Downpour SGD considers the gradient of the objective function
 * operation on an individual mini-batch of points in its update of \f$ A \f$.
 * Downpour runs multiple replicas of a model in parallel on each mini-batch.
 * Each replica sends their updates to a parameter server, which is responsible
 * for storing and updating the model's parameters.
 *
 * For more information, please refer to:
 *
 * @code
 * @inproceedings{Dean2012,
 *   author    = {Dean, Jeffrey and Corrado, Greg S. and Monga, Rajat and Chen,
 *                Kai and Devin, Matthieu and Le, Quoc V. and Mao, Mark Z. and
 *                Ranzato, Marc'Aurelio and Senior, Andrew and Tucker, Paul and
 ...
 * @endcode
 *
 * For Downpour SGD to work, the class must implement the following function:
 *
 *   size_t NumFunctions();
...
class DownpourSGD
{
 public:
  /**
   * Construct the DownpourSGD optimizer with the given function and
   ...
   * @param fetchSize Request updated parameters every numfetch (Default: 1).
   ...
   */
  DownpourSGD(const size_t batchSize = 1000,
              const double stepSize = 0.01,
              const size_t maxIterations = 100000,
              const double tolerance = 1e-5,
              const size_t fetchSize = 1,
  ...
  /**
   * Optimize the given function using Downpour SGD. The given starting point
  ...
  //! Get the fetch rate.
  size_t Fetch() const { return fetchSize; }
  //! Modify the fetch rate.
  size_t& Fetch() { return fetchSize; }
