 ...
 * known as Schatten Matrix Norms, which is the lp norm of the vector of singular 
 * values of the matrix. Used as LinearConstrSolverType in FrankWolfe Algorithm.
 *
...
/**
 * LinearConstrSolver for FrankWolfe algorithm. The constraint is in matrix atom 
 * domain, that is defined by unit ball under Schatten Matrix Norms.
 *
 * Since the p-Schatten Matrix Norm is just the lp norm of the singular value 
 * vector, we add a ConstrLpBallSolver (which is lp ball constraint for vector case)
 * inside the class.
 *
 */
...
public:
     ...
     * The constrained domain is the matrix unit lp ball under Schatten Matrix p norm.
     *
     ...
     * @param S Output optimal solution matrix in the constrained domain(matrix lp ball).
     */
