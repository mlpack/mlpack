...
double IQN::Optimize(DecomposableFunctionType& function, arma::mat& iterate)
{
  ...
  for (size_t i = 0, f = 0; i < numFunctions; f++)
  {
    ...
    t.slice(f) =
        arma::mat(initialIterate.memptr(), iterate.n_elem, 1, false, false);
  ...
  arma::mat iterateVec =
      arma::mat(iterate.memptr(), iterate.n_elem, 1, false, false);
  ...
  for (size_t i = 1; i != maxIterations; ++i)
  {
    for (size_t j = 0, f = 0; f < numFunctions; j++)
    {
      ...
      const size_t effectiveBatchSize =
          std::min(batchSize, numFunctions - it * batchSize);

      if (arma::norm(iterateVec - t.slice(it)) > 0)
      {
        function.Gradient(
            iterate, it * batchSize, gradient, effectiveBatchSize);
        gradient /= effectiveBatchSize;

        const arma::mat s = iterateVec - t.slice(it);
        const arma::mat yy = arma::vectorise(gradient - y.slice(it));

        const arma::mat stochasticHessian =
            Q.slice(it) + yy * yy.t() / arma::as_scalar(yy.t() * s)
            - Q.slice(it) * s * s.t() * Q.slice(it)
                  / arma::as_scalar(s.t() * Q.slice(it) * s);
        ...
        u += (1.0 / numBatches)
             * (stochasticHessian * iterateVec - Q.slice(it) * t.slice(it));
        ...
        iterateVec =
            stepSize * B.i() * (u - gVec) + (1 - stepSize) * iterateVec;
      }
