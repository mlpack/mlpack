    ...
    arma::Mat<eT> resizedImages, const size_t newWidth, const size_t newHeight)
{
  if (images.n_cols != resizedImages.n_cols)
  {
    ...
    oss << "ResizeImages(): the resizedImage matrix need to have identical"
      " dimensions to the images matrix" << std::endl;
  }
  arma::Mat<unsigned char> tempSrc(size(images));
  arma::Mat<unsigned char> tempDest(size(resizedImages));
...
//   // resize the image correctly according to the new height and width
...
//         tempMat.n_rows - CropUpDownEqually - 1);
//     }
