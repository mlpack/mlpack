                                   ...
                                   float highFreq)
{
  ...
  {
    eT left   = binIndices[m];
    eT center = binIndices[m + 1];
    eT right  = binIndices[m + 2];

    // Triangle vertices: rises from 0 at left to 1 at center, falls to 0
    // at right.  Bins outside [left, right] extrapolate to 0 according to arma
    // docs.
    arma::Col<eT> X = { left, center, right };
    arma::Col<eT> Y = { 0, 1, 0};

    arma::Col<eT> YI;
    arma::interp1(X, Y, XI, YI, "*linear", static_cast<eT>(0));

    melFilterbank.row(m) = YI.t();
  }
                    ...
                    std::is_floating_point<eT>::value>*)
{
  ...
  if (nFFT == 0)
    nFFT = NextPowerOf2(windowLength);
  ...
  arma::Mat<eT> H = MelFilterbank<eT>(numMelFilters, nFFT, sampleRate,
      lowFreq, highFreq);
  ...
  for (size_t i = 0; i < inputSignal.n_cols; ++i)
  {
    ...
    FinitImpulseResponseFilter(audioSignal, preEmphCoeff);
    ...
    slidingWindows.each_col() %= HammingWindow<eT>(windowLength);
    ...
    results[i] = arma::log((H * power) + 1e-10);
  }
