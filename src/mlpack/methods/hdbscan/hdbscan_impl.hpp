        ...
        arma::Row<size_t>& assignments)
{
  ...
  emst::DualTreeBoruvka<metric::HdbscanMetric, arma::mat, TreeType> 
  ...
  assignments = trans(result);
}

/**
 * Sorts the Matrix.
 * Index is the column number according to which the matrix has to be sorted.
 * If matrix passed is
 * [ 1 2 7
 *   4 5 3
 *   7 8 9 ]   and index is 2, then output will be
 * [ 4 5 3
 *   1 2 7
 *   7 8 9]
 */
...
void HDBSCAN<NeighborSearch, 
             MetricType, 
             TreeType>::
SortMatrix(MatType& results, 
           size_t index)
{
  typedef typename MatType::elem_type elem;
  typedef std::pair<elem, elem> ee;

  //create a priority queue in push
  //all the elments into it.
  std::priority_queue< ee > Q;
  for (size_t i = 0; i < results.n_rows; ++i)
  {
    Q.push(ee(results(i,index), i));
  }
  MatType resultsCopy(results);
 
  //extract all the elments from priority queue
  //and put them on proper places
  size_t i = results.n_rows-1;
  while (! Q.empty())
  {
    ee top = Q.top();
    Q.pop();
    results.row(i) = resultsCopy.row(top.second);
    i--;
  }
}   
...
{
  singleLinkageTree.set_size(inputMST.n_rows, 4);
  size_t no = inputMST.n_rows + 1;
  UnionForSingleLinkageClustering u(no);

  for (size_t i = 0; i < inputMST.n_rows; i++)
  {
    size_t v1 = inputMST(i,0);
    size_t v2 = inputMST(i,1);
    typename MatType::elem_type delta = inputMST(i,2);
    size_t parentOfv1 = u.Find(v1);
    size_t parentOfv2 = u.Find(v2);
    singleLinkageTree(i,0) = parentOfv1;
    singleLinkageTree(i,1) = parentOfv2;
    singleLinkageTree(i,2) = delta;
    singleLinkageTree(i,3) = u.GetSize(parentOfv1) + u.GetSize(parentOfv2);
							   ...
							   size_t rootOfBFS)
{
  ...
  while(! q.empty())
  {
    size_t topElement = q.front();
    q.pop();
    bfs.push_back(topElement);
    if (topElement >= numPoints)
    {
      size_t index = topElement -numPoints;
      q.push(singleLinkageTree(index, 0));
      q.push(singleLinkageTree(index, 1));
    }
  }
}
	         ...
	         size_t minClusterSize )
{
  ...
  {  
      ...
      leftChild = singleLinkageTree((currNode - noOfPoints), 0);
      rightChild = singleLinkageTree((currNode - noOfPoints), 1);
      if(leftChild>=noOfPoints)
      	leftCount = singleLinkageTree((leftChild - noOfPoints), 3);
      else
      	leftCount = 1;
      if(rightChild>=noOfPoints)
      	rightCount = singleLinkageTree((rightChild - noOfPoints), 3);
      else
      	rightCount = 1;

      // leftCount = ((leftChild >= noOfPoints) ? 
      //               singleLinkageTree((leftChild - noOfPoints), 3) :
      //               1);
      // rightCount = ((rightChild >= noOfPoints) ? 
      //                singleLinkageTree((rightChild - noOfPoints), 3) : 
      //                1);
      
      delta = singleLinkageTree((currNode - noOfPoints), 2);
      if (delta > 0.0)
        lambda = 1.0 / delta;
      else
      ...
      if (leftCount >= minClusterSize && rightCount >= minClusterSize)
      {
        ...
        nextLabel++;
        result(resultCounter, 0) = relabel[currNode];
        result(resultCounter, 1) = relabel[leftChild];
        result(resultCounter, 2) = lambda;
        result(resultCounter, 3) = leftCount;
        resultCounter++;

        relabel[rightChild] = nextLabel;
        nextLabel++;
        result(resultCounter, 0) = relabel[currNode];
        result(resultCounter, 1) = relabel[rightChild];
        result(resultCounter, 2) = lambda;
        result(resultCounter, 3) = rightCount;
        resultCounter++;
      }
      else if (leftCount < minClusterSize && rightCount < minClusterSize)
      {
        ...
        for (size_t j = 0; j < bfsOfChild.size(); j++)
        {
          ...
          {
            result(resultCounter, 0) = relabel[currNode];
            result(resultCounter, 1) = subNode;
            result(resultCounter, 2) = lambda;
            result(resultCounter, 3) = 1;
            resultCounter++;
          }
          ignore[subNode] = true;
        }
	           ...
	           std::map<size_t, double>& result)
{
  ...
  size_t largestChild = arma::max(condensedTree.col(1));
  size_t smallestCluster = arma::min(condensedTree.col(0));
  size_t noOfClusters = arma::max(condensedTree.col(0)) - 
  						arma::min(condensedTree.col(0)) + 1;
  ...
  MatType sortedChildData(condensedTree.n_rows, 2);
  sortedChildData.col(0) = condensedTree.col(1);
  sortedChildData.col(1) = condensedTree.col(2);
  ...
  for (size_t i = 0; i < sortedChildData.n_rows; i++)
  {
    child = sortedChildData(i, 0);
    lambda = sortedChildData(i, 1);
  ...
  for (size_t i = 0; i < condensedTree.n_rows; i++)
  {
    parent = condensedTree(i, 0);
    lambda = condensedTree(i, 2);
    childSize = condensedTree(i,3);
	                    ...
	                    std::vector<size_t>& resultBFS)
{
  ...
  if (clusteredTree.n_rows == 0 || clusteredTree.n_cols == 0)
    return;
  std::vector<bool> visited(std::max(arma::max(clusteredTree.col(0)), 
                                     arma::max(clusteredTree.col(1))), 
                                     false);
  ...
  while(! q.empty())
  {
    ...
    for (size_t i = 0; i < clusteredTree.n_rows; i++)
      if (clusteredTree(i, 0) == currentNode && !visited[clusteredTree(i, 1)])
        q.push(clusteredTree(i, 1));
  }
	      ...
	      arma::Mat<size_t>& result)
{
  ...
  size_t rootCluster = arma::min(condensedTree.col(0));
  result.set_size(rootCluster, 1);

  emst::UnionFind unionTree(arma::max(condensedTree.col(0))+1);
  ...
  for (size_t i = 0; i < condensedTree.n_rows; i++)
    if (! std::binary_search(clusters.begin(), 
    	                     clusters.end(), 
    	                     condensedTree(i,1)))
      unionTree.Union(condensedTree(i,0), condensedTree(i,1));
  ...
  if (clusters.size() == 1 && allowSingleCluster)
  {
    ...
    for (size_t i = 0; i < condensedTree.n_rows; i++)
      {
        if (condensedTree(i, 0) == rootCluster)
          lambdaRootCluster = std::max(lambdaRootCluster, condensedTree(i, 2));
      }
	        ...
	        arma::Mat<size_t>& result)
{
  ...
  for (it = stabilities.begin(); it != stabilities.end(); ++it)
  ...
  for (size_t i = 0; i < condensedTree.n_rows; i++)
    if (condensedTree(i, 3)  > 1)
      {
      	clusteredTree.row(clusterTreeIndex) = condensedTree.row(i);
      	clusterTreeIndex++;
      }
  clusteredTree.resize(clusterTreeIndex, condensedTree.n_cols);
  
  //Keeps track, if a point is in cluster or not
  std::vector<bool> isCluster(std::max(arma::max(condensedTree.col(0)), 
  	                          arma::max(condensedTree.col(1))), 
                              false);
  ...
  for (size_t i = 0; i < nodes.size(); i++)
  {
    ...
    for (size_t j = 0; j < clusteredTree.n_rows; j++)
        if (clusteredTree(j, 0) == currentNode)
        {
          subtreeStability += stabilities[(size_t)clusteredTree(j, 1)];
        }
