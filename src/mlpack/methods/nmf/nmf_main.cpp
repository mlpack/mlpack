...
PROGRAM_INFO(
    ...
    "the rank of the factorization (specified by the "
        + PRINT_PARAM_STRING("rank")
        + " parameter)."
          "\n\n"
          "Optionally, the desired update rules for each NMF iteration can be "
          "chosen "
          "from the following list:"
          "\n\n"
          " - multdist: multiplicative distance-based update rules (Lee and "
          "Seung "
          "1999)\n"
          " - multdiv: multiplicative divergence-based update rules (Lee and "
          "Seung "
          "1999)\n"
          " - als: alternating least squares update rules (Paatero and Tapper "
          "1994)"
          "\n\n"
          "The maximum number of iterations is specified with "
        + PRINT_PARAM_STRING("max_iterations")
        + ", and the minimum residue "
          "required for algorithm termination is specified with the "
        + PRINT_PARAM_STRING("min_residue")
        + " parameter."
          "\n\n"
          "For example, to run NMF on the input matrix "
        + PRINT_DATASET("V")
        + " "
          "using the 'multdist' update rules with a rank-10 decomposition and "
          "storing the decomposed matrices into "
        + PRINT_DATASET("W")
        + " and "
        + PRINT_DATASET("H")
...
PARAM_DOUBLE_IN("min_residue",
                "The minimum root mean square residue allowed "
                "for each iteration, below which the program terminates.",
                "e",
                1e-5);

PARAM_STRING_IN("update_rules",
...
static void mlpackMain()
{
  ...
  RequireParamInSet<string>("update_rules",
  ...
  else if (updateRules == "als")
  {
    Log::Info << "Performing NMF with alternating least squared update rules."
              << std::endl;
    SimpleResidueTermination srt(minResidue, maxIterations);
    AMF<SimpleResidueTermination, RandomInitialization, NMFALSUpdate> amf(srt);
    amf.Apply(V, r, W, H);
  }
