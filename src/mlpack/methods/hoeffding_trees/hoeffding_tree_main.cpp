...
PROGRAM_INFO(
    ...
    "The training file and associated labels are specified with the "
        ...
        + PRINT_PARAM_STRING("labels")
        + " is not specified, the labels are assumed "
        ...
        + " "
          "parameter, and if performance statistics are desired for that test "
          "set, "
          "labels may be specified with the "
        ...
        + " output parameter, and class "
          "probabilities for each prediction may be saved with the "
        + PRINT_PARAM_STRING("probabilities")
        + " output parameter."
          "\n\n"
          "For example, to train a Hoeffding tree with confidence 0.99 with "
          "data "
        + PRINT_DATASET("dataset")
        + ", saving the trained tree to "
        + PRINT_MODEL("tree")
        + ", the following command may be used:"
          "\n\n"
        + PRINT_CALL("hoeffding_tree",
                     "training",
                     "dataset",
                     "confidence",
                     0.99,
        ...
        + ", saving the predictions into "
        + PRINT_DATASET("predictions")
        + " and the class probabilities into "
        + PRINT_DATASET("class_probs")
...
PARAM_UROW_OUT("predictions",
               "Matrix to output label predictions for test "
               "data into.",
               "p");
...
PARAM_STRING_IN("numeric_split_strategy",
                "The splitting strategy to use for "
                "numeric features: 'domingos' or 'binary'.",
                "N",
                "binary");
PARAM_FLAG(
    "batch_mode",
...
PARAM_INT_IN("bins",
             "If the 'domingos' split strategy is used, this specifies "
             "the number of bins for each numeric split.",
             "B",
             10);
PARAM_INT_IN(
    "observations_before_binning",
    "If the 'domingos' split strategy "
...
static void mlpackMain()
{
  ...
  RequireAtLeastOnePassed(
      {"output_model", "predictions", "probabilities", "test_labels"},
      false,
  ...
  RequireParamInSet<string>("numeric_split_strategy",
                            {"domingos", "binary"},
                            true,
                            "unrecognized numeric split strategy");
  ...
  if (CLI::HasParam("training"))
  {
    ...
    const size_t observationsBeforeBinning =
        (size_t)CLI::GetParam<int>("observations_before_binning");
  ...
  if (CLI::HasParam("training"))
  {
    ...
    Log::Info << correct << " out of " << labels.n_elem << " correct "
              << "on training set ("
              << double(correct) / double(labels.n_elem) * 100.0 << ")."
              << endl;
  }
  ...
  if (CLI::HasParam("test"))
  {
    ...
    if (CLI::HasParam("test_labels"))
    {
      ...
      Log::Info << correct << " out of " << testLabels.n_elem << " correct "
                << "on test set ("
                << double(correct) / double(testLabels.n_elem) * 100.0 << ")."
                << endl;
    }
