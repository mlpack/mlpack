 ...
 */
#ifndef MLPACK_METHODS_ANN_FFNEQL_HPP
#define MLPACK_METHODS_ANN_FFNEQL_HPP
...
class FFNEQL
{
 public:
  ...
  using NetworkType = FFNEQL<InitializationRuleType>;

  /**
   * Create the FFNEQL object.
   *
   ...
   */
  FFNEQL(InitializationRuleType initializeRule = InitializationRuleType());
   ...
   */
  template<typename PredictorsType, typename ResponsesType>
  void Forward(const PredictorsType& input,
   ...
   * two functions can't satisfy some special requirements.
   *
   * @param inputStatePref Inputs state-preference pair of current pass.
  ...
  double Backward(const PredictorsType& inputStatePref,
                  const PredictorsType& targets,
 ...
 private:
  ...
  /**
   * Increase the lambda value per episode.
   */
  void LambdaAnneal();
  ...
  //! Locally-stored lambda parameter. Used for annealing Homotopy loss.
  double lambda;
