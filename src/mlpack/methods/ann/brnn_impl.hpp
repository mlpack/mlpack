        ...
        network.initializeRule)
{
  ...
  {
    boost::apply_visitor(resetVisitor, forwardRNN.network[i]);
  }

  for (size_t i = 0; i < backwardRNN.network.size(); ++i)
  {
    boost::apply_visitor(resetVisitor, backwardRNN.network[i]);
  }
}
