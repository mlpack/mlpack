  ...
  const TargetType&& target)
{
  ...
  // due to poor gradient properties
  // we add a value lamda = 10 ^ -6
  arma::mat distances_prime = sqrt(square(distances) + lambda);

  arma::mat margin_vector = (margin * arma::ones(1, distances.n_cols)) 
                            - distances_prime;

  // checking the condition whether 
  // margin - distance is greater than or less than zero
  // if less than zero, replace with zero
  margin_vector.for_each([] (arma::mat::elem_type& val) {if(val<0) val = 0;});

  // returning the total error
  return arma::accu( (0.5 * ((1 - target) % square(distances))) 
                    + (0.5 * (target % square(margin_vector))));
}
    ...
    OutputType&& output)
    {
      ...
      for (size_t i = 0; i<margin_vector.n_cols; i++){
        if (margin_vector(0, i) < distances_prime(0, i))
            ...
            result(0, i) = (target(0, i) * distances_prime(0, i)) 
                            - ((1 - target(0, i)) * (margin_vector(0, i) 
                            - distances_prime(0, i))); 
      }
      output = result;
    }
