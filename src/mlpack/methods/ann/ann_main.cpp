...
BINDING_EXAMPLE(
    "Train a model:\n"
    "$ mlpack_ann -t data.csv -l labels.csv -L layers.txt -i 4 -O 3 -M model.bin");

PARAM_MATRIX_IN("training", "Matrix of training data.", "t");
PARAM_UROW_IN("labels", "Labels of training data.", "l");
PARAM_MATRIX_IN("test", "Matrix of test data.", "T");
PARAM_MODEL_IN(FFN<>, "input_model", "Pre-trained model to use.", "m");
PARAM_MODEL_OUT(FFN<>, "output_model", "output trained model", "M");
...
PARAM_INT_IN("input_dim", "Input dimension (required if creating new model).", "i", 0);
PARAM_INT_IN("output_dim", "Output dimension (required if creating new model).", "O", 0);
...
void AddLayerFromString(FFN<>& model, const string& layerSpec){
...
void BINDING_FUNCTION(util::Params& params, util::Timers& /* timers */){
    ...
    if (training) {
        ...
        arma::Row<size_t> labels = std::move(params.Get<arma::Row<size_t>>("labels"));

        // Process labels
        if (!params.Has("regression")){
            // For NegativeLogLikelihood, we typically pass the class labels directly (from 0 to numClasses-1)
            // as a row vector, rather than one-hot encoding them.
             trainLabels = arma::conv_to<arma::mat>::from(labels);
        }
        else{
            trainLabels = arma::conv_to<arma::mat>::from(labels);
        }
    ...
    if (loadModel){
        model = std::move(params.Get<FFN<>>("input_model"));
        Log::Info << "Loaded model from file." << endl;
    }
    else if (training){ // Creating new model
        ...
        if (params.Has("output_dim")) {
            ...
            Log::Info << "Inferred output dimension from labels: " << outputDim << endl;
        }
        ...
        Log::Info<<"Created new model with "<<model.Network().size()<<" layers."<<endl;
    }
    ...
    if (training){
        ...
        {
        ens::Adam optimizer(stepSize, batchSize, 0.9, 0.999, 1e-8, maxIterations);
        model.Train(trainData, trainLabels, optimizer);
        }
    ...
    if (testing)
    {
        ...
        if (!params.Has("regression")){
            ...
            predictions.each_col([](arma::vec& v) { v = arma::exp(v); });
    ...
    {
        params.Get<FFN<>>("output_model") = std::move(model);
