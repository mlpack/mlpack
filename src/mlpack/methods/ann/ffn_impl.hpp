...
FFN<OutputLayerType, InitializationRuleType>::FFN(
  ...
  : outputLayer(std::move(outputLayer)),
    initializeRule(std::move(initializeRule)), width(0), height(0),
    reset(false), numFunctions(0), deterministic(true)
{
  /* Nothing to do here */
}
...
FFN<OutputLayerType, InitializationRuleType>::FFN(
  ...
  : outputLayer(std::move(outputLayer)),
    initializeRule(std::move(initializeRule)), width(0), height(0),
    reset(false), predictors(std::move(predictors)),
    responses(std::move(responses)), deterministic(true)
{
...
template<typename OutputLayerType, typename InitializationRuleType>
template<typename OptimizerType>
void FFN<OutputLayerType, InitializationRuleType>::Train(arma::mat predictors,
                                                         arma::mat responses)
{
  numFunctions = responses.n_cols;
...
template<typename OutputLayerType, typename InitializationRuleType>
void FFN<OutputLayerType, InitializationRuleType>::Forward(arma::mat inputs,
                                                           arma::mat& results)
{
...
template<typename OutputLayerType, typename InitializationRuleType>
double
FFN<OutputLayerType, InitializationRuleType>::Backward(arma::mat targets,
                                                       arma::mat& gradients)
{
...
template<typename OutputLayerType, typename InitializationRuleType>
void FFN<OutputLayerType, InitializationRuleType>::Predict(arma::mat predictors,
                                                           arma::mat& results)
{
  if (parameter.is_empty())
    ResetParameters();
  ...
  Forward(std::move(
      arma::mat(predictors.colptr(0), predictors.n_rows, 1, false, true)));
  resultsTemp =
      boost::apply_visitor(outputParameterVisitor, network.back()).col(0);
  ...
  for (size_t i = 1; i < predictors.n_cols; i++)
  {
    ...
    resultsTemp = boost::apply_visitor(outputParameterVisitor, network.back());
    ...
    arma::mat& gradient)
{
  ...
  for (size_t i = 0; i < network.size(); ++i)
  {
    offset += boost::apply_visitor(
        GradientSetVisitor(std::move(gradient), offset), network[i]);
  }
}
...
FFN<OutputLayerType, InitializationRuleType>::FFN(const FFN& network)
  : outputLayer(network.outputLayer), initializeRule(network.initializeRule),
    width(network.width), height(network.height), reset(network.reset),
    predictors(network.predictors), responses(network.responses),
    parameter(network.parameter), numFunctions(network.numFunctions),
    error(network.error), currentInput(network.currentInput),
    deterministic(network.deterministic), delta(network.delta),
    inputParameter(network.inputParameter),
    outputParameter(network.outputParameter), gradient(network.gradient)
{
  // Build new layers according to source network
  for (size_t i = 0; i < network.network.size(); ++i)
  {
    this->network.push_back(
        boost::apply_visitor(copyVisitor, network.network[i]));
  }
};

template<typename OutputLayerType, typename InitializationRuleType>
FFN<OutputLayerType, InitializationRuleType>::FFN(FFN&& network)
  : outputLayer(std::move(network.outputLayer)),
    initializeRule(std::move(network.initializeRule)), width(network.width),
    height(network.height), reset(network.reset),
    predictors(std::move(network.predictors)),
    responses(std::move(network.responses)),
    parameter(std::move(network.parameter)), numFunctions(network.numFunctions),
    error(std::move(network.error)),
    currentInput(std::move(network.currentInput)),
    deterministic(network.deterministic), delta(std::move(network.delta)),
