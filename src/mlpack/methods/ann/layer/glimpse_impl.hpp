...
Glimpse<InputDataType, OutputDataType>::Glimpse(const size_t inSize,
                                                const size_t size,
                                                const size_t depth,
                                                const size_t scale,
                                                const size_t inputWidth,
                                                const size_t inputHeight)
  : inSize(inSize), size(size), depth(depth), scale(scale),
    inputWidth(inputWidth), inputHeight(inputHeight), outputWidth(size),
    outputHeight(size), inputDepth(0), deterministic(true)
{
  // Nothing to do here.
}
    ...
    const arma::Mat<eT>&& input, arma::Mat<eT>&& output)
{
  ...
  {
    for (size_t depthIdx = 0, glimpseSize = size; depthIdx < depth;
         depthIdx++, glimpseSize *= scale)
    {
      size_t padSize = std::floor((glimpseSize - 1) / 2);

      arma::Cube<eT> inputPadded =
          arma::zeros<arma::Cube<eT>>(inputTemp.n_rows + padSize * 2,
                                      inputTemp.n_cols + padSize * 2,
                                      inputTemp.n_slices / inSize);

      inputPadded.tube(padSize,
                       padSize,
                       padSize + inputTemp.n_rows - 1,
                       padSize + inputTemp.n_cols - 1) =
          inputTemp.subcube(0,
                            0,
                            inputIdx * inputDepth,
                            inputTemp.n_rows - 1,
      ...
      size_t x = std::min(
          h, (size_t)std::max(0.0, (location(0, inputIdx) + 1) / 2.0 * h));
      size_t y = std::min(
          w, (size_t)std::max(0.0, (location(1, inputIdx) + 1) / 2.0 * w));

      if (depthIdx == 0)
      {
        for (size_t j = (inputIdx + depthIdx), paddedSlice = 0;
             j < outputTemp.n_slices;
             j += (inSize * depth), paddedSlice++)
        {
          outputTemp.slice(j) = inputPadded.subcube(x,
                                                    y,
                                                    paddedSlice,
                                                    x + glimpseSize - 1,
                                                    y + glimpseSize - 1,
      ...
      else
      {
        for (size_t j = (inputIdx + depthIdx * (depth - 1)), paddedSlice = 0;
             ...
             j += (inSize * depth), paddedSlice++)
        {
          arma::Mat<eT> poolingInput = inputPadded.subcube(x,
                                                           y,
                                                           paddedSlice,
                                                           x + glimpseSize - 1,
                                                           y + glimpseSize - 1,
    ...
    const arma::Mat<eT>&& /* input */, arma::Mat<eT>&& gy, arma::Mat<eT>&& g)
{
  ...
  arma::Cube<eT> mappedError =
      arma::zeros<arma::cube>(outputWidth, outputHeight, 1);
  ...
  for (size_t s = 0, j = 0; s < mappedError.n_slices; s += gy.n_cols, j++)
  {
    ...
    {
      mappedError.slice(s + i) =
          arma::Mat<eT>(gy.memptr(), outputWidth, outputHeight);
    }
  }
  ...
  for (size_t inputIdx = 0; inputIdx < inSize; inputIdx++)
  {
         ...
         depthIdx++, glimpseSize *= scale)
    {
      ...
      {
        for (size_t j = (inputIdx + depthIdx), paddedSlice = 0;
             j < mappedError.n_slices;
             j += (inSize * depth), paddedSlice++)
        {
          inputPadded.subcube(x,
                              y,
                              paddedSlice,
                              x + glimpseSize - 1,
                              y + glimpseSize - 1,
      ...
      else
      {
        for (size_t j = (inputIdx + depthIdx * (depth - 1)), paddedSlice = 0;
             ...
             j += (inSize * depth), paddedSlice++)
        {
          arma::Mat<eT> poolingOutput = inputPadded.subcube(x,
                                                            y,
                                                            paddedSlice,
                                                            x + glimpseSize - 1,
                                                            y + glimpseSize - 1,
          ...
          if (scale == 2)
          {
            Unpooling(inputTemp.slice(paddedSlice),
                      mappedError.slice(j),
                      poolingOutput);
          }
          else
      ...
      gTemp += inputPadded.tube(padSize,
                                padSize,
                                padSize + inputTemp.n_rows - 1,
                                padSize + inputTemp.n_cols - 1);
    }
