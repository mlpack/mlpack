...
namespace mlpack
{
  namespace ann /** Artificial Neural Network. */
  {
    ...
    GRU<InputDataType, OutputDataType>::GRU(
        ...
        const size_t rho) : inSize(inSize),
                            outSize(outSize),
                            rho(rho),
                            batchSize(1),
                            forwardStep(0),
        ...
        const arma::Mat<eT> &input, arma::Mat<eT> &output)
    {
      ...
      boost::apply_visitor(ForwardVisitor(output.submat(
                                          ...
                                          boost::apply_visitor(
                                              outputParameterVisitor, inputGateModule)),
                           inputGateModule);
      ...
      output = (boost::apply_visitor(outputParameterVisitor, inputGateModule) % (*prevOutput - boost::apply_visitor(outputParameterVisitor,
                                                                                                                    hiddenStateModule))) +
        ...
        const arma::Mat<eT> &input, const arma::Mat<eT> &gy, arma::Mat<eT> &g)
    {
      ...
      boost::apply_visitor(BackwardVisitor(boost::apply_visitor(
                                               outputParameterVisitor, hiddenStateModule),
                                           dOt,
      ...
      boost::apply_visitor(BackwardVisitor(boost::apply_visitor(
                                               outputParameterVisitor, forgetGateModule),
                                           dRt,
