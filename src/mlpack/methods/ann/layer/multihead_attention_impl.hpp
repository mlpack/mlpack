...
Reset()
{
  ...
  outBias = MatType(weights.memptr()
      + (4 * embedDim + 3) * embedDim, embedDim, 1, false, false);
}
...
Forward(const arma::Mat<eT>& input, arma::Mat<eT>& output)
{
  ...
  for (size_t i = 0; i < batchSize; ++i)
  {
    qProj.slice(i) = q.slice(i) * queryWt
        + arma::repmat(qBias, 1, tgtSeqLen).t();
    kProj.slice(i) = k.slice(i) * keyWt
        + arma::repmat(kBias, 1, srcSeqLen).t();
    vProj.slice(i) = v.slice(i) * valueWt
        + arma::repmat(vBias, 1, srcSeqLen).t();
  }
  ...
  for (size_t i = 0; i < batchSize; ++i)
  {
    output.col(i) = arma::vectorise(attnOut.slice(i) * outWt
        + arma::repmat(outBias, 1, tgtSeqLen).t());
  }
}
