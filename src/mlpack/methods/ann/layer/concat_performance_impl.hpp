...
ConcatPerformance<OutputLayerType, InputDataType, OutputDataType>::
  ...
  : inSize(inSize), outputLayer(std::move(outputLayer))
{
  // Nothing to do here.
}
...
double
ConcatPerformance<OutputLayerType, InputDataType, OutputDataType>::Forward(
...
template<typename OutputLayerType,
         ...
         typename OutputDataType>
template<typename eT>
void ConcatPerformance<OutputLayerType, InputDataType, OutputDataType>::
             ...
             arma::Mat<eT>&& output)
{
  ...
  outputLayer.Backward(
      std::move(subInput), std::move(target), std::move(subOutput));
