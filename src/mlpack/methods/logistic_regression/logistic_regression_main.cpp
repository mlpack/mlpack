...
static void mlpackMain()
{
  ...
  const double lambda = CMD::GetParam<double>("lambda");
  ...
  const double decisionBoundary = CMD::GetParam<double>("decision_boundary");
  ...
  // Load data matrix.
  if (CMD::HasParam("training"))
    regressors = std::move(CMD::GetParam<arma::mat>("training"));

  // Load the model, if necessary.
  LogisticRegression<>* model;
  if (CMD::HasParam("input_model"))
    model = CMD::GetParam<LogisticRegression<>*>("input_model");
  else
  {
  ...
  {
    responses = std::move(CMD::GetParam<arma::Row<size_t>>("labels"));
  ...
  // Verify the labels.
  if (CMD::HasParam("training") && max(responses) > 1)
  {
    // Clean memory if needed.
    if (!CMD::HasParam("input_model"))
      delete model;
  ...
  {
    const arma::mat& testSet = CMD::GetParam<arma::mat>("test");
    ...
    if (CMD::HasParam("predictions") || CMD::HasParam("output"))
    {
      Log::Info << "Predicting classes of points in '"
          << CMD::GetPrintableParam<arma::mat>("test") << "'." << endl;
      model->Classify(testSet, predictions, decisionBoundary);

      // The CMD param "output" is deprecated and replaced by "predictions"
      ...
      if (CMD::HasParam("output"))
        CMD::GetParam<arma::Row<size_t>>("output") = std::move(predictions);
    }

    // The CMD param "output_probabilities" is deprecated
    // and replaced by "probabilities"
    // "output_probabilities" parameter will be removed in mlpack 4.
    if (CMD::HasParam("output_probabilities") || CMD::HasParam("probabilities"))
    {
      ...
      if (CMD::HasParam("output_probabilities"))
        CMD::GetParam<arma::mat>("output_probabilities") = probabilities;
      if (CMD::HasParam("probabilities"))
        CMD::GetParam<arma::mat>("probabilities") = std::move(probabilities);
    }
  }

  CMD::GetParam<LogisticRegression<>*>("output_model") = model;
}
