...
PROGRAM_INFO(
    ...
    "This program allows loading a logistic regression model (via the "
        ...
        + " parameter) "
          "or training a logistic regression model given training data "
          "(specified "
          "with the "
        + PRINT_PARAM_STRING("training")
        + " parameter), or both "
          "those things at once.  In addition, this program allows "
          "classification on "
          "a test dataset (specified with the "
        + PRINT_PARAM_STRING("test")
        + " "
          "parameter) and the classification results may be saved with the "
        + PRINT_PARAM_STRING("output")
        + " output parameter.  The trained logistic "
        ...
        + " parameter.  Available options are "
          "'sgd' (stochastic gradient descent) and 'lbfgs' (the L-BFGS "
          "optimizer).  "
        ...
        + " parameter specifies the tolerance for "
          " convergence.  For the SGD optimizer, the "
        + PRINT_PARAM_STRING("step_size")
        + " parameter controls the step size taken"
          " at each iteration by the optimizer.  The batch size for SGD is "
          "controlled"
          " with the "
        + PRINT_PARAM_STRING("batch_size")
        + " parameter. If the "
          "objective function for your data is oscillating between Inf and 0, "
          "the "
          ...
          "\n\n"
          "For SGD, an iteration refers to a single point. So to take a single "
          "pass "
          "over the dataset with SGD, "
        + PRINT_PARAM_STRING("max_iterations")
        + " should be set to the number of points in the dataset."
          "\n\n"
          "Optionally, the model can be used to predict the responses for "
          "another "
          "matrix of data points, if "
        + PRINT_PARAM_STRING("test")
        + " is "
          "specified.  The "
        + PRINT_PARAM_STRING("test")
        ...
        + PRINT_PARAM_STRING("input_model")
        + " parameter.  The output predictions "
          "from the logistic regression model may be saved with the "
        + PRINT_PARAM_STRING("output")
        + " parameter."
          "\n\n"
          "This implementation of logistic regression does not support the "
          "general "
          "multi-class case but instead only the two-class case.  Any labels "
          "must "
        ...
        + PRINT_MODEL("lr_model")
        + "', the following command may be used:"
          "\n\n"
        + PRINT_CALL("logistic_regression",
                     "training",
                     "data",
                     "labels",
                     "labels",
        ...
        + PRINT_DATASET("test")
        + "', storing the output predictions in '"
...
PARAM_DOUBLE_IN("lambda",
                "L2-regularization parameter for training.",
                "L",
                0.0);
PARAM_STRING_IN("optimizer",
...
PARAM_INT_IN("max_iterations",
             "Maximum iterations for optimizer (0 indicates "
             "no limit).",
             "n",
             10000);
...
PARAM_MODEL_IN(LogisticRegression<>,
               "input_model",
               "Existing model "
               "(parameters).",
               "m");
PARAM_MODEL_OUT(LogisticRegression<>,
                "output_model",
                "Output for trained "
...
PARAM_DOUBLE_IN(
    ...
    "taken to be 0; otherwise, the class is 1.",
    "d",
    0.5);
...
static void mlpackMain()
{
  ...
  RequireParamInSet<string>(
      "optimizer", {"lbfgs", "sgd"}, true, "unknown optimizer");

  // Lambda must be positive.
  RequireParamValue<double>("lambda",
                            [](double x) { return x >= 0.0; },
                            true,
                            "lambda must be positive or zero");
  ...
  RequireParamValue<double>("step_size",
                            [](double x) { return x >= 0.0; },
                            true,
