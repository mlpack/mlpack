...
PROGRAM_INFO(
    ...
    "(specified using the "
        ...
        + " parameter), if the data supplied is linearly separable.  The "
          "perceptron "
          "is parameterized by a matrix of weight vectors that denote the "
          "numerical "
          "weights of the neural network."
          "\n\n"
          "This program allows loading a perceptron from a model (via the "
        + PRINT_PARAM_STRING("input_model")
        + " parameter) or training a perceptron "
          "given training data (via the "
        + PRINT_PARAM_STRING("training")
        + " parameter), or both those things at once.  In addition, this "
          "program "
          "allows classification on a test dataset (via the "
        + PRINT_PARAM_STRING("test")
        + " parameter) and the classification results "
        ...
        + " output parameter."
          "\n\n"
          "The training data given with the "
        + PRINT_PARAM_STRING("training")
        + " option may have class labels as its last dimension (so, if the "
          "training "
          "data is in CSV format, labels should be the last column).  "
          "Alternately, "
          "the "
        ...
        + " parameter may be used to specify "
          ...
          "\n\n"
          "All these options make it easy to train a perceptron, and then "
          "re-use that"
          " perceptron for later classification.  The invocation below trains "
          "a "
          "perceptron on "
        + PRINT_DATASET("training_data")
        + " with labels "
        + PRINT_DATASET("training_labels")
        + ", and saves the model to "
        + PRINT_MODEL("perceptron")
        + "."
          "\n\n"
        + PRINT_CALL("perceptron",
                     "training",
                     "training_data",
                     "labels",
                     "training_labels",
        ...
        + ".  The example below does precisely that, "
          "saving the predicted classes to "
        ...
        + PRINT_CALL("perceptron",
                     ...
                     "predictions")
        + "\n\n"
          "Note that all of the options may be specified at once: predictions "
          "may be "
          "calculated right after training a model, and model training can "
          "occur even"
          " if an existing perceptron model is passed with the "
        + PRINT_PARAM_STRING("input_model")
        + " parameter.  However, note that the "
          "number of classes and the dimensionality of all data must match.  "
          "So you "
          "cannot pass a perceptron model trained on 2 classes and then "
          "re-train with"
...
PARAM_INT_IN("max_iterations",
             ...
             "perceptron is to be run",
             "n",
             1000);

// Model loading/saving.
PARAM_MODEL_IN(PerceptronModel, "input_model", "Input perceptron model.", "m");
PARAM_MODEL_OUT(PerceptronModel,
                "output_model",
                "Output for trained perceptron"
                " model.",
                "M");
...
static void mlpackMain()
{
  ...
  RequireAtLeastOnePassed(
      {"output_model", "output"}, false, "no output will be saved");
    ...
    else
    {
      ...
      if (numClasses > p.P().Weights().n_cols)
      {
        Log::Fatal << "Perceptron from '"
                   << CLI::GetPrintableParam<PerceptronModel>("input_model")
                   << "' "
                   << "has " << p.P().Weights().n_cols
                   << " classes, but the training "
