...
static void mlpackMain()
{
  ...
  if (CMD::HasParam("training"))
  {
    trainingData = std::move(CMD::GetParam<arma::mat>("training"));

    const bool regularization = false;
//    const bool regularization = CMD::HasParam("volume_regularization");
    const int maxLeafSize = CMD::GetParam<int>("max_leaf_size");
    const int minLeafSize = CMD::GetParam<int>("min_leaf_size");
    const bool skipPruning = CMD::HasParam("skip_pruning");
    size_t folds = CMD::GetParam<int>("folds");
    ...
    if (CMD::HasParam("training_set_estimates"))
    {
  ...
  {
    tree = CMD::GetParam<DTree<arma::mat>*>("input_model");
  }
  ...
  {
    testData = std::move(CMD::GetParam<arma::mat>("test"));
    if (CMD::HasParam("test_set_estimates"))
    {
      ...
      CMD::GetParam<arma::mat>("test_set_estimates") = std::move(testDensities);
    }
    ...
    if (CMD::HasParam("vi"))
    {
      ...
      CMD::GetParam<arma::mat>("vi") = importances.t();
    }
  }
  ...
  if (CMD::HasParam("tag_file"))
  {
    const arma::mat& estimationData =
        CMD::HasParam("test") ? testData : trainingData;
    const string tagFile = CMD::GetParam<string>("tag_file");
    std::ofstream ofs;
    ...
    else if (CMD::HasParam("path_format"))
    {
      const bool reqCounters = CMD::HasParam("tag_counters_file");
      const string pathFormat = CMD::GetParam<string>("path_format");
      ...
      if (reqCounters)
      {
        ofs.open(CMD::GetParam<string>("tag_counters_file"),
                 std::ofstream::out);
    ...
    else
    {
      ...
      if (CMD::HasParam("tag_counters_file"))
        data::Save(CMD::GetParam<string>("tag_counters_file"), counters);
    }
  ...
  CMD::GetParam<DTree<arma::mat>*>("output_model") = tree;
}
