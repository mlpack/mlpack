...
PROGRAM_INFO(
    ...
    "specifying the "
        ...
        + " parameter.  "
          "This approach works by taking random samplings of the dataset; to "
          "specify "
          "the number of samplings, the "
        + PRINT_PARAM_STRING("samplings")
        + " parameter is used, and to specify the percentage of the dataset to "
          "be "
          "used in each sample, the "
        + PRINT_PARAM_STRING("percentage")
        + " parameter is used (it should be a value between 0.0 and 1.0)."
          "\n\n"
          "There are several options available for the algorithm used for each "
          "Lloyd "
          "iteration, specified with the "
        + PRINT_PARAM_STRING("algorithm")
        + " "
          " option.  The standard O(kN) approach can be used ('naive').  Other "
          "options include the Pelleg-Moore tree-based algorithm "
          "('pelleg-moore'), "
          "Elkan's triangle-inequality based algorithm ('elkan'), Hamerly's "
          "modification to Elkan's algorithm ('hamerly'), the dual-tree "
          "k-means "
          "algorithm ('dualtree'), and the dual-tree k-means algorithm using "
          "the "
          "cover tree ('dualtree-covertree')."
          "\n\n"
          "The behavior for when an empty cluster is encountered can be "
          "modified with"
          " the "
        + PRINT_PARAM_STRING("allow_empty_clusters")
        + " option.  When "
          "this option is specified and there is a cluster owning no points at "
          "the "
          "end of an iteration, that cluster's centroid will simply remain in "
          "its "
          "position from the previous iteration. If the "
        + PRINT_PARAM_STRING("kill_empty_clusters")
        + " option is specified, then "
          "when a cluster owns no points at the end of an iteration, the "
          "cluster "
          "centroid is simply filled with DBL_MAX, killing it and effectively "
          "reducing k for the rest of the computation.  Note that the default "
          "option "
          "when neither empty cluster option is specified can be "
          "time-consuming to "
          "calculate; therefore, specifying either of these parameters will "
          "often "
          "accelerate runtime."
          "\n\n"
          "Initial clustering assignments may be specified using the "
        + PRINT_PARAM_STRING("initial_centroids")
        + " parameter, and the maximum "
          "number of iterations may be specified with the "
        + PRINT_PARAM_STRING("max_iterations")
        + " parameter."
          "\n\n"
          "As an example, to use Hamerly's algorithm to perform k-means "
          "clustering "
          "with k=10 on the dataset "
        + PRINT_DATASET("data")
        + ", saving the "
          "centroids to "
        + PRINT_DATASET("centroids")
        + " and the assignments for "
          "each point to "
        + PRINT_DATASET("assignments")
        + ", the following "
        ...
        + PRINT_CALL("kmeans",
                     ...
                     "centroids")
        + "\n\n"
          "To run k-means on that same dataset with initial centroids "
          "specified in "
        + PRINT_DATASET("initial")
        + " with a maximum of 500 iterations, "
          "storing the output centroids in "
        + PRINT_DATASET("final")
        + " the "
...
PARAM_FLAG("kill_empty_clusters",
...
PARAM_MATRIX_IN("initial_centroids",
                "Start with the specified initial "
                "centroids.",
                "I");

// Parameters for "refined start" k-means.
PARAM_FLAG("refined_start",
           "Use the refined initial point strategy by Bradley "
           "and Fayyad to choose initial points.",
           "r");
PARAM_INT_IN("samplings",
             "Number of samplings to perform for refined start "
             "(use when --refined_start is specified).",
             "S",
             100);
PARAM_DOUBLE_IN("percentage",
                "Percentage of dataset to use for each refined "
                "start sampling (use when --refined_start is specified).",
                "p",
                0.02);
...
static void mlpackMain()
{
  ...
  {
    RequireParamValue<int>("samplings",
                           [](int x) { return x > 0; },
                           true,
...
void FindLloydStepType(const InitialPartitionPolicy& ipp)
{
  ...
  if (algorithm == "elkan")
    ...
    RunKMeans<InitialPartitionPolicy, EmptyClusterPolicy, PellegMooreKMeans>(
        ipp);
  else if (algorithm == "dualtree")
    RunKMeans<InitialPartitionPolicy,
              EmptyClusterPolicy,
...
void RunKMeans(const InitialPartitionPolicy& ipp)
{
  ...
  {
    RequireParamValue<int>("clusters",
                           [](int x) { return x > 0; },
                           true,
  ...
  RequireAtLeastOnePassed(
      {"in_place", "output", "centroid"}, false, "no results will be saved");
  ...
  KMeans<metric::EuclideanDistance,
         ...
         EmptyClusterPolicy,
         LloydStepType>
      kmeans(maxIterations, metric::EuclideanDistance(), ipp);
  ...
  if (CLI::HasParam("output") || CLI::HasParam("in_place"))
  {
    ...
    kmeans.Cluster(
        dataset, clusters, assignments, centroids, false, initialCentroidGuess);
