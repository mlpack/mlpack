...
void FindEmptyClusterPolicy(const InitialPartitionPolicy& ipp)
{
  if (CMD::HasParam("allow_empty_clusters") ||
      CMD::HasParam("kill_empty_clusters"))
  ...
  else if (CMD::HasParam("kill_empty_clusters"))
...
void RunKMeans(const InitialPartitionPolicy& ipp)
{
  ...
  if (!CMD::HasParam("initial_centroids"))
  {
  ...
  int clusters = CMD::GetParam<int>("clusters");
  if (clusters == 0 && CMD::HasParam("initial_centroids"))
  {
  ...
  arma::mat dataset = CMD::GetParam<arma::mat>("input");  // Load our dataset.
  arma::mat centroids;

  const bool initialCentroidGuess = CMD::HasParam("initial_centroids");
  // Load initial centroids if the user asked for it.
  if (initialCentroidGuess)
  {
    centroids = std::move(CMD::GetParam<arma::mat>("initial_centroids"));
    if (clusters == 0)
  ...
  if (CMD::HasParam("output") || CMD::HasParam("in_place"))
  {
