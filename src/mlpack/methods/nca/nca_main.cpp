...
PROGRAM_INFO(
    ...
    "row of the input dataset (specified with "
        ...
        + ")."
          "\n\n"
          "This implementation of NCA uses stochastic gradient descent, "
          "mini-batch "
          "stochastic gradient descent, or the L_BFGS optimizer.  These "
          "optimizers do"
          " not guarantee global convergence for a nonconvex objective "
          "function "
          "(NCA's objective function is nonconvex), so the final results could "
          "depend"
          " on the random seed or other optimizer parameters."
          "\n\n"
          "Stochastic gradient descent, specified by the value 'sgd' for the "
          "parameter "
        + PRINT_PARAM_STRING("optimizer")
        + ", depends "
          "primarily on three parameters: the step size (specified with "
        + PRINT_PARAM_STRING("step_size")
        + "), the batch size (specified with "
        + PRINT_PARAM_STRING("batch_size")
        + "), and the maximum number of iterations"
          " (specified with "
        + PRINT_PARAM_STRING("max_iterations")
        + ").  In "
          "addition, a normalized starting point can be used by specifying the "
        + PRINT_PARAM_STRING("normalize")
        + " parameter, which is necessary if many "
          "warnings of the form 'Denominator of p_i is 0!' are given.  Tuning "
          "the "
          "step size can be a tedious affair.  In general, the step size is "
          "too large"
          " if the objective is not mostly uniformly decreasing, or if "
          "zero-valued "
          "denominator warnings are being issued.  The step size is too small "
          "if the "
          "objective is changing very slowly.  Setting the termination "
          "condition can "
          "be done easily once a good step size parameter is found; either "
          "increase "
          "the maximum iterations to a large number and allow SGD to find a "
          "minimum, "
          "or set the maximum iterations to 0 (allowing infinite iterations) "
          "and set "
        ...
        + ") to "
          ...
          "terminate.  Be careful---setting the tolerance instead of the "
          "maximum "
          "iterations can take a very long time and may actually never "
          "converge due "
          "to the properties of the SGD optimizer. Note that a single "
          "iteration of "
          "SGD refers to a single point, so to take a single pass over the "
          "dataset, "
          "set the value of the "
        + PRINT_PARAM_STRING("max_iterations")
        + " parameter equal to the number of points in the dataset."
          "\n\n"
          "The L-BFGS optimizer, specified by the value 'lbfgs' for the "
          "parameter "
        + PRINT_PARAM_STRING("optimizer")
        + ", uses a back-tracking line search "
          "algorithm to minimize a function.  The following parameters are "
          "used by "
          "L-BFGS: "
        + PRINT_PARAM_STRING("num_basis")
        ...
        + ", "
        + PRINT_PARAM_STRING("wolfe")
        + ", "
        + PRINT_PARAM_STRING("tolerance")
        + " (the optimization is terminated when the gradient norm is below "
          "this "
          "value), "
        + PRINT_PARAM_STRING("max_line_search_trials")
        + ", "
        + PRINT_PARAM_STRING("min_step")
        + ", and "
        + PRINT_PARAM_STRING("max_step")
        + " (which both refer to the line search "
          "routine).  For more details on the L-BFGS optimizer, consult either "
          "the "
          "mlpack L-BFGS documentation (in lbfgs.hpp) or the vast set of "
          "published "
...
PARAM_FLAG(
    "normalize",
    "Use a normalized starting point for optimization. This"
...
PARAM_INT_IN("max_line_search_trials",
             "Maximum number of line search trials "
             "for L-BFGS.",
             "T",
             50);
PARAM_DOUBLE_IN("min_step",
                "Minimum step of line search for L-BFGS.",
                "m",
                1e-20);
PARAM_DOUBLE_IN("max_step",
                "Maximum step of line search for L-BFGS.",
                "M",
                1e20);
...
static void mlpackMain()
{
  ...
  RequireParamInSet<string>(
      "optimizer", {"sgd", "lbfgs"}, true, "unknown optimizer type");
  ...
  if (normalize)
  {
    ...
    Log::Info << "Using normalized starting point for optimization." << endl;
  }
  else
  {
    distance.eye();
