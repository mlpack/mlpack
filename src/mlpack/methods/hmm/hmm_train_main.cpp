...
struct Init
{
  ...
  {
    const size_t states = CMD::GetParam<int>("states");
  ...
  static void RandomInitialize(vector<GMM>& e)
  {
    for (size_t i = 0; i < e.size(); ++i)
    {
      ...
      for (int g = 0; g < CMD::GetParam<int>("gaussians"); ++g)
      {
...
struct Train
{
  ...
  {
    const bool batch = CMD::HasParam("batch");
    const double tolerance = CMD::GetParam<double>("tolerance");

    // Do we need to replace the tolerance?
    if (CMD::HasParam("tolerance"))
      hmm.Tolerance() = tolerance;

    const string labelsFile = CMD::GetParam<string>("labels_file");
...
static void mlpackMain()
{
  ...
  const string type = CMD::GetParam<string>("type");
  const bool batch = CMD::HasParam("batch");
  const double tolerance = CMD::GetParam<double>("tolerance");
  ...
  if (CMD::HasParam("input_model") && CMD::HasParam("tolerance"))
  {
    Log::Info << "Tolerance of existing model in '"
        << CMD::GetPrintableParam<HMMModel*>("input_model") << "' will be "
  ...
  if (CMD::HasParam("input_model"))
  {
    hmm = CMD::GetParam<HMMModel*>("input_model");
  ...
  CMD::GetParam<HMMModel*>("output_model") = hmm;
}
