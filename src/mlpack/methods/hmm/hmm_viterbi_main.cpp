...
struct Viterbi
{
  ...
  {
    // Load observations.
    mat dataSeq = std::move(CMD::GetParam<arma::mat>("input"));
    ...
    // Save output.
    CMD::GetParam<arma::Mat<size_t>>("output") = std::move(sequence);
  }
};
...
static void mlpackMain()
{
  ...
  CMD::GetParam<HMMModel*>("input_model")->PerformAction<Viterbi>((void*) NULL);
}
