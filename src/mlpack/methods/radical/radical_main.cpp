...
PROGRAM_INFO(
    "RADICAL",
    "An implementation of RADICAL, a method for independent"
    ...
    "The input matrix to perform ICA on should be specified with the "
        ...
        + " parameter.  The output matrix Y may be "
          "saved with the "
        + PRINT_PARAM_STRING("output_ic")
        + " output parameter, "
          "and the output unmixing matrix W may be saved with the "
        + PRINT_PARAM_STRING("output_unmixing")
        + " output parameter."
          "\n\n"
          "For example, to perform ICA on the matrix "
        + PRINT_DATASET("X")
        + " with "
        ...
        + ", the following command may be used: "
          "\n\n"
        + PRINT_CALL(
              "radical", "input", "X", "replicates", 40, "output_ic", "ic"));
...
PARAM_DOUBLE_IN("noise_std_dev",
                "Standard deviation of Gaussian noise.",
                "n",
                0.175);
PARAM_INT_IN("replicates",
             "Number of Gaussian-perturbed replicates to use "
             "(per point) in Radical2D.",
             "r",
             30);
PARAM_INT_IN("angles",
             "Number of angles to consider in brute-force search "
             "during Radical2D.",
             "a",
             150);
PARAM_INT_IN("sweeps",
             "Number of sweeps; each sweep calls Radical2D once for "
             "each pair of dimensions.",
             "S",
             0);
PARAM_INT_IN("seed", "Random seed.  If 0, 'std::time(NULL)' is used.", "s", 0);
PARAM_FLAG("objective",
           "If set, an estimate of the final objective function "
           "is printed.",
           "O");
...
static void mlpackMain()
{
  ...
  RequireAtLeastOnePassed({"output_ic", "output_unmixing"},
                          false,
                          "no output"
                          " will be saved");

  // Check validity of parameters.
  RequireParamValue<int>("replicates",
                         [](int x) { return x > 0; },
                         true,
  ...
  RequireParamValue<double>(
      ...
      true,
      "standard deviation of Gaussian noise must be greater than or equal"
      " to 0");
  RequireParamValue<int>("angles",
                         [](int x) { return x > 0; },
                         true,
                         "number of angles must be positive");
  RequireParamValue<int>("sweeps",
                         [](int x) { return x >= 0; },
                         true,
