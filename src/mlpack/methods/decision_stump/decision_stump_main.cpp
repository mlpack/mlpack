...
static void mlpackMain()
{
  ...
  if (CMD::HasParam("training"))
  {
    ...
    const size_t bucketSize = CMD::GetParam<int>("bucket_size");
  ...
  {
    model = CMD::GetParam<DSModel*>("input_model");
  }
  ...
  if (CMD::HasParam("test"))
  {
    ...
    if (CMD::HasParam("predictions"))
    {
      ...
      CMD::GetParam<Row<size_t>>("predictions") = std::move(actualLabels);
    }
  }

  // Save the model, if desired.
  CMD::GetParam<DSModel*>("output_model") = model;
}
