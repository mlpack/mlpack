...
PROGRAM_INFO(
    ...
    " number of training points in each bin can be specified with the "
        + PRINT_PARAM_STRING("bucket_size")
        + " parameter."
          "\n\n"
          "The decision stump is parameterized by a splitting dimension and a "
          "vector "
          "of values that denote the splitting values of each bin."
          "\n\n"
          "This program enables several applications: a decision tree may be "
          "trained "
          "or loaded, and then that decision tree may be used to classify a "
          "given set"
          " of test points.  The decision tree may also be saved to a file for "
          "later "
          "usage."
          "\n\n"
          "To train a decision stump, training data should be passed with the "
        + PRINT_PARAM_STRING("training")
        + " parameter, and their corresponding "
          "labels should be passed with the "
        ...
        + PRINT_PARAM_STRING("bucket_size")
        + " "
          "parameter controls the minimum number of training points in each "
          "decision "
          "stump bucket."
          "\n\n"
          "For classifying a test set, a decision stump may be loaded with the "
        + PRINT_PARAM_STRING("input_model")
        + " parameter (useful for the situation "
          "where a stump has already been trained), and a test set may be "
          "specified "
          "with the "
        + PRINT_PARAM_STRING("test")
        + " parameter.  The predicted "
          "labels can be saved with the "
        + PRINT_PARAM_STRING("predictions")
        + " "
          "output parameter."
          "\n\n"
          "Because decision stumps are trained in batch, retraining does not "
          "make "
          "sense and thus it is not possible to pass both "
        + PRINT_PARAM_STRING("training")
        + " and "
        + PRINT_PARAM_STRING("input_model")
        + "; instead, simply build a new "
        ...
        + " output parameter.  That stump may "
...
PARAM_UROW_IN("labels",
              "Labels for the training set. If not specified, the "
              "labels are assumed to be the last row of the training data.",
              "l");
PARAM_MATRIX_IN("test", "A dataset to calculate predictions for.", "T");

// Output.
PARAM_UROW_OUT("predictions",
               "The output matrix that will hold the "
...
PARAM_MODEL_IN(DSModel,
               "input_model",
               "Decision stump model to "
               "load.",
               "m");
PARAM_MODEL_OUT(DSModel,
                "output_model",
