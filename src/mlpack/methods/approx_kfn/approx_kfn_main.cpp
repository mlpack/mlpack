...
PROGRAM_INFO(
    ...
    "Specify a reference set (set to search in) with "
        + PRINT_PARAM_STRING("reference")
        + ", specify a query set with "
        + PRINT_PARAM_STRING("query")
        + ", and specify algorithm parameters with "
        + PRINT_PARAM_STRING("num_tables")
        + " and "
        + PRINT_PARAM_STRING("num_projections")
        + " (or don't and defaults will be "
          "used).  The algorithm to be used (either 'ds'---the default---or "
          "'qdafn') "
        ...
        + " output "
          "parameter may be used to store the built model, and an input model "
          "may be "
        ...
        + PRINT_PARAM_STRING("neighbors")
        + " and "
        + PRINT_PARAM_STRING("distances")
        + " output parameters.  Each row of these "
          "output matrices holds the k distances or neighbor indices for each "
          "query "
          "point."
          "\n\n"
          "For example, to find the 5 approximate furthest neighbors with "
        + PRINT_DATASET("reference_set")
        + " as the reference set and "
        + PRINT_DATASET("query_set")
        + " as the query set using DrusillaSelect, "
          "storing the furthest neighbor indices to "
        + PRINT_DATASET("neighbors")
        + " and the furthest neighbor distances to "
        + PRINT_DATASET("distances")
        + ", one could call"
          "\n\n"
        + PRINT_CALL("approx_kfn",
                     "query",
                     "query_set",
                     "reference",
                     "reference_set",
                     ...
                     "distances")
        + "\n\n"
          "and to perform approximate all-furthest-neighbors search with k=1 "
          "on the "
          "set "
        + PRINT_DATASET("data")
        + " storing only the furthest neighbor "
          "distances to "
        + PRINT_DATASET("distances")
        + ", one could call"
          "\n\n"
        + PRINT_CALL("approx_kfn",
                     ...
                     "distances")
        + "\n\n"
          "A trained model can be re-used.  If a model has been previously "
          "saved to "
        + PRINT_MODEL("model")
        + ", then we may find 3 approximate furthest "
          "neighbors on a query set "
        + PRINT_DATASET("new_query_set")
        + " using "
...
PARAM_INT_IN("num_projections",
             "Number of projections to use in each hash "
             "table.",
             "p",
             5);
...
PARAM_MODEL_IN(ApproxKFNModel,
               "input_model",
               "File containing input model.",
               "m");
PARAM_MODEL_OUT(ApproxKFNModel,
                "output_model",
...
static void mlpackMain()
{
  ...
  RequireAtLeastOnePassed(
      {"reference", "k"}, false, "no task will be performed");
  ...
  RequireParamInSet<string>(
      "algorithm", {"ds", "qdafn"}, true, "unknown algorithm");
  ...
  RequireParamValue<int>("num_tables",
                         [](int x) { return x > 0; },
                         true,
                         "number of tables must be positive");
  RequireParamValue<int>("num_projections",
                         [](int x) { return x > 0; },
                         true,
  ...
  if (CLI::HasParam("k"))
  {
    ...
    if (CLI::HasParam("calculate_error"))
    {
      ...
      const double averageError =
          arma::sum(exactDistances.row(0) / distances.row(0))
          / distances.n_cols;
      const double minError =
          arma::min(exactDistances.row(0) / distances.row(0));
      const double maxError =
          arma::max(exactDistances.row(0) / distances.row(0));
