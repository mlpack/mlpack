...
PROGRAM_INFO(
    ...
    "function used is specified with the "
        + PRINT_PARAM_STRING("kernel")
        + " parameter."
          "\n\n"
          "For example, the following command will calculate, for each point "
          "in the "
          "query set "
        + PRINT_DATASET("query")
        + ", the five points in the "
          "reference set "
        + PRINT_DATASET("reference")
        + " with maximum kernel "
          "evaluation using the linear kernel.  The kernel evaluations may be "
          "saved "
          "with the  "
        + PRINT_DATASET("kernels")
        + " output parameter and the "
          "indices may be saved with the "
        + PRINT_DATASET("indices")
        + " output "
          "parameter."
          "\n\n"
        + PRINT_CALL("fastmks",
                     ...
                     "linear")
        + "\n\n"
          "The output matrices are organized such that row i and column j in "
          "the "
          "indices matrix corresponds to the index of the point in the "
          "reference set "
          "that has j'th largest kernel evaluation with the point in the query "
          "set "
          "with index i.  Row i and column j in the kernels matrix corresponds "
          "to the"
          " kernel evaluation between those two points."
          "\n\n"
          "This program performs FastMKS using a cover tree.  The base used to "
          "build "
          "the cover tree can be specified with the "
        + PRINT_PARAM_STRING("base")
        + " parameter.");

// Model-building parameters.
PARAM_MATRIX_IN("reference", "The reference dataset.", "r");
PARAM_STRING_IN("kernel",
                "Kernel type to use: 'linear', 'polynomial', "
                "'cosine', 'gaussian', 'epanechnikov', 'triangular', 'hyptan'.",
                "K",
                "linear");
PARAM_DOUBLE_IN("base",
...
static void mlpackMain()
{
  ...
  if (CLI::HasParam("k"))
  {
    RequireAtLeastOnePassed(
        {"indices", "kernels"}, false, "no output will be saved");
  }
  ...
  if (CLI::HasParam("k"))
  {
    ...
    if (CLI::HasParam("query"))
    {
      ...
      model.Search(
          queryData, (size_t)CLI::GetParam<int>("k"), indices, kernels, base);
    }
    else
    {
