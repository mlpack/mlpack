 ...
 public:
  ...
  double EvalNonMarkov(Genome& genome, int numStep, double& GuruFitness) 
  {
    ...
    while (step < numStep) 
    {
      ...
      arma::vec input = {state[0] / 4.80, state[2] / 0.52, state[4] / 0.52};
      arma::vec output;
      genome.Classify(input, output);
      double action = output(0);
  ...
  double EvalFitness(Genome& genome) 
  {
    ...
    for (int trial=0; trial<num_trial; ++trial) 
    {
      ...
      for (int step=0; step<num_step; ++step) 
      {
        ...
        arma::vec input = {x, x_dot};
        arma::vec output;
        genome.Classify(input, output);
        int index = arma::as_scalar(arma::find(output.max() == output, 1));
