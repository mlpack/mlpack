...
PROGRAM_INFO(
    ...
    "may also be specified with the "
        ...
        + " parameter.  The l1-norm "
          "regularization parameter is specified with the "
        + PRINT_PARAM_STRING("lambda")
        + " parameter.  For example, to run LCC on "
          "the dataset "
        + PRINT_DATASET("data")
        + " using 200 atoms and an "
          "l1-regularization parameter of 0.1, saving the dictionary "
        + PRINT_PARAM_STRING("dictionary")
        + " and the codes into "
        + PRINT_PARAM_STRING("codes")
        + ", use"
          "\n\n"
        ...
        + " parameter. "
          "Optionally, the input data matrix X can be normalized before coding "
          "with "
          "the "
        ...
        + " output parameter.  Then, to encode "
          "new points from the dataset "
        + PRINT_DATASET("points")
        + " with the "
          "previously saved model "
        + PRINT_MODEL("lcc_model")
        + ", saving the new "
          "codes to "
        + PRINT_DATASET("new_codes")
...
PARAM_MODEL_OUT(LocalCoordinateCoding,
                "output_model",
                "Output for trained LCC "
                "model.",
                "M");
  ...
  if (CLI::HasParam("training"))
  {
    ...
    if (CLI::HasParam("input_model"))
    {
      Log::Info << "Using dictionary from existing model in '"
                << CLI::GetPrintableParam<string>("input_model")
                << "' as initial "
  ...
  if (CLI::HasParam("test"))
  {
    ...
    if (matY.n_rows != lcc.Dictionary().n_rows)
      Log::Fatal << "Model was trained with a dimensionality of "
                 ...
                 << CLI::GetPrintableParam<mat>("test")
                 << " has a dimensionality of " << matY.n_rows << "!" << endl;
