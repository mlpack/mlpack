       ...
       const size_t T)
{
  ...
  Log::Info << "Computing "<<Teffective<<" additional probing codes per table per query"<<std::endl;
  // Go through every query point sequentially.
  for (size_t i = 0; i < referenceSet->n_cols; i++)
  {
    ...
    // 'secondHashTable' to obtain the neighbor candidates.
    arma::uvec refIndices;
    ReturnIndicesFromTable(referenceSet->col(i), refIndices, numTablesToSearch, Teffective);
