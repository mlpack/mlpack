...
PROGRAM_INFO(
    "Split Data",
    "This utility takes a dataset and optionally labels "
    ...
    "dataset to be used as the test set can be specified with the "
        ...
        + " output parameters."
          "\n\n"
          "Optionally, labels can be also be split along with the data by "
          "specifying "
          "the "
        + PRINT_PARAM_STRING("input_labels")
        + " parameter.  Splitting "
          "labels works the same way as splitting the data. The output "
          "training and "
          "test labels may be saved with the "
        + PRINT_PARAM_STRING("training_labels")
        + " and "
        ...
        + PRINT_DATASET("X")
        + " into "
        + PRINT_DATASET("X_train")
        + " and "
        + PRINT_DATASET("X_test")
        + " with 60% of the data in the training set and "
          "40% of the dataset in the test set, we could run "
          "\n\n"
        + PRINT_CALL("preprocess_split",
                     "input",
                     "X",
                     "training",
                     "X_train",
        ...
        + PRINT_DATASET("y")
        + ", and we wanted to split these into "
        + PRINT_DATASET("X_train")
        + ", "
        + PRINT_DATASET("y_train")
        + ", "
        + PRINT_DATASET("X_test")
        + ", and "
        + PRINT_DATASET("y_test")
        + ", with 30% "
...
PARAM_DOUBLE_IN("test_ratio",
                "Ratio of test set; if not set,"
                "the ratio defaults to 0.2",
                "r",
                0.2);
...
static void mlpackMain()
{
  ...
  RequireAtLeastOnePassed({"training"},
                          false,
                          "no training set will be "
                          "saved");
  ...
  {
    RequireAtLeastOnePassed({"training_labels"},
                            false,
                            "no training set "
                            "labels will be saved");
    RequireAtLeastOnePassed({"test_labels"},
                            false,
                            "no test set labels will "
                            "be saved");
  }
  else
  {
