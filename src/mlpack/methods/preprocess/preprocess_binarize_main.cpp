...
PROGRAM_INFO(
    ...
    "to can be specified using the "
        + PRINT_PARAM_STRING("dimension")
        + " parameter; if left unspecified, every dimension will be binarized. "
          " The "
          "threshold for binarization can also be specified with the "
        + PRINT_PARAM_STRING("threshold")
        + " parameter; the default threshold is "
          "0.0."
          "\n\n"
        ...
        + " output parameter."
          ...
          "dataset "
        + PRINT_DATASET("X")
        + " to 1 and variables less than or equal "
          "to 5.0 to 0, and save the result to "
        + PRINT_DATASET("Y")
        + ", we could "
          "run"
          "\n\n"
        + PRINT_CALL("preprocess_binarize",
                     "input",
                     "X",
                     "threshold",
                     5.0,
...
PARAM_INT_IN("dimension",
             "Dimension to apply the binarization. If not set, the"
             " program will binarize every dimension by default.",
             "d",
             0);
PARAM_DOUBLE_IN("threshold",
                "Threshold to be applied for binarization. If not "
                "set, the threshold defaults to 0.0.",
                "t",
                0.0);
...
static void mlpackMain()
{
  ...
  {
    Log::Warn
        << "You did not specify " << PRINT_PARAM_STRING("dimension")
  ...
  if (!CLI::HasParam("threshold"))
  {
    Log::Warn << "You did not specify " << PRINT_PARAM_STRING("threshold")
              << ", so the threshold will be automatically set to '0.0'."
              << endl;
  }
  ...
  RequireParamValue<int>("dimension",
                         [input](int x) { return size_t(x) < input.n_rows; },
                         true,
                         error.str());
