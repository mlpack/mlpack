...
PROGRAM_INFO(
    ...
    "The training file and associated labels are specified with the "
        ...
        + " "
          "output parameter may be used to save the trained model.  A model "
          "may be "
        ...
        + PRINT_PARAM_STRING("input_model")
        + " parameter "
          "may not be specified when the "
        + PRINT_PARAM_STRING("training")
        + " "
          "parameter is specified.  The "
        + PRINT_PARAM_STRING("minimum_leaf_size")
        + " parameter specifies the minimum number of training points that "
          "must fall"
          " into each leaf for it to be split.  If "
        + PRINT_PARAM_STRING("print_training_error")
        + " is specified, the training "
          "error will be printed."
          "\n\n"
          "Test data may be specified with the "
        + PRINT_PARAM_STRING("test")
        + " "
          "parameter, and if performance numbers are desired for that test "
          "set, "
        ...
        + " output parameter."
          "\n\n"
          "For example, to train a decision tree with a minimum leaf size of "
          "20 on "
          "the dataset contained in "
        + PRINT_DATASET("data")
        + " with labels "
        + PRINT_DATASET("labels")
        + ", saving the output model to "
        + PRINT_MODEL("tree")
        + " and printing the training error, one could "
          "call"
          "\n\n"
        + PRINT_CALL("decision_tree",
                     "training",
                     "data",
                     "labels",
                     "labels",
        ...
        + " and print the test error given the "
          "labels "
        + PRINT_DATASET("test_labels")
        + " using that model, while "
...
PARAM_UMATRIX_IN("test_labels",
                 "Test point labels, if accuracy calculation "
                 "is desired.",
                 "L");
...
// Models.
PARAM_MODEL_IN(DecisionTreeModel,
               "input_model",
               "Pre-trained decision tree, "
               "to be used with test points.",
               "m");
PARAM_MODEL_OUT(DecisionTreeModel,
                "output_model",
                "Output for trained decision"
                " tree.",
                "M");
...
static void mlpackMain()
{
  ...
  if (CLI::HasParam("training"))
  {
    ...
    if (CLI::HasParam("weights"))
    {
      ...
      model.tree =
          DecisionTree<>(dataset, labels, numClasses, weights, minLeafSize);
    }
    else
    {
  ...
  if (CLI::HasParam("test"))
  {
    ...
    if (CLI::HasParam("test_labels"))
    {
      ...
      Log::Info << double(correct) / double(testPoints.n_cols) * 100 << "%% "
                << "correct on test set (" << correct << " / "
                << testPoints.n_cols << ")." << endl;
    }
