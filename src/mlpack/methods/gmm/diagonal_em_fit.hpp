/**
 * @file diagonal_em_fit.hpp
 * @author Kim SangYeon
 *
 * Utility class to fit a DiagonalGMM using the EM algorithm.  Used by
 * DiagonalGMM::Estimate<>().
 *
 ...
 */
#ifndef MLPACK_METHODS_GMM_DIAGONAL_EM_FIT_HPP
#define MLPACK_METHODS_GMM_DIAGONAL_EM_FIT_HPP
...
/**
 * This class contains methods which can fit a DiagonalGMM to observations
 * using the EM algorithm.  It requires an initial clustering mechanism, which
 * is by default the KMeans algorithm.  The clustering mechanism must implement
...
template<typename InitialClusteringType = kmeans::KMeans<>,
         typename CovarianceConstraintPolicy = NoConstraint>
class DiagonalEMFit
{
 public:
  /**
   * Construct the DiagonalEMFit object, optionally passing an InitialClusteringType
   ...
   * @param clusterer Object which will perform the initial clustering.
	 * @param constraint Constraint of covariance.
   */
  DiagonalEMFit(
...
#include "diagonal_em_fit_impl.hpp"
