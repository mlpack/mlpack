...
/**
 * Implementation of Bit Flipping task.
 */
class BitFlipping
{
 public:
   ...
   * @param bitWidth Length of the binary vector for state and goal.
   */
   ...
   * @return reward for the transition.
   */
  ...
  {
    stepsPerformed = 0;
    initialState = arma::randi<arma::vec>(bitWidth, arma::distr_param(0, 1));
  ...
  /**
   * This function checks if the episode is terminated.
   *
  ...
  size_t BitWidth() const { return bitWidth; }
  //! Set the size of binary vector.
  size_t& BitWidth() { return bitWidth; }
