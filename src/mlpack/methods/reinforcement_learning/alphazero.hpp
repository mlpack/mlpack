...
public:
  AlphaZeroConfig()
      : TrainingConfig(), selfplaySteps(0),
        explorationUcb(1.41421356237) // square-root of two
  {
    // nothing to do here
  }

  AlphaZeroConfig(size_t numWorkers, size_t updateInterval, size_t stepLimit,
                  size_t explorationSteps, size_t selfplaySteps,
                  double stepSize, double vMin, double vMax,
                  double explorationUcb)
      : TrainingConfig(numWorkers, updateInterval, 100, stepLimit,
                       explorationSteps, stepSize, 0.99, 40, false, false,
                       false, 51, vMin, vMax, 0.005),
...
template <typename EnvironmentType, typename ValueNetworkType,
          typename PolicyNetworkType, typename UpdaterType,
