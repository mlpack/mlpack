...
private:
   ...
   * The default value is 1.41421356237 the approximation of the square root of 2
   * This is valid only for AlphaLike.
   */
  double explorationUcb;
};
 ...
 * }
 * @endcode
 * The present implementation of AlphaZero is inspired by Ciamic’s implementation
 * at https://github.com/ciamic/alphazero/tree/main
...
class AlphaZero
{
 public:
   ...
   * @param valueNetworkUpdater How to apply gradients to the value network when training.
 ...
 struct Node
  ...
  double PUCBscore(const Node* const node)
  {
      ...
      double prior_score = node->proba * std::sqrt(double(node->parent->N)) /double(1 + node->N);
