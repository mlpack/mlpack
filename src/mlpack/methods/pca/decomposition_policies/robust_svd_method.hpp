/**
 * @file robust_svd_method.hpp
 * @author Charan Reddy
 *
 * Implementation of the robust svd method for use in the Principal
...
#ifndef MLPACK_METHODS_PCA_DECOMPOSITION_POLICIES_ROBUST_SVD_METHOD_HPP
#define MLPACK_METHODS_PCA_DECOMPOSITION_POLICIES_ROBUST_SVD_METHOD_HPP

#include <mlpack/prereqs.hpp>
#include <mlpack/methods/robust_svd/robust_svd.hpp>

namespace mlpack {
namespace pca {

/**
 * Implementation of the robust SVD policy.
 */
class RobustSVDPolicy
{
 public:
  /**
   * Use robust SVD method to perform the principal components analysis
   * (PCA).
   *
   ...
   *        (Default: 2).
   */
  RobustSVDPolicy(const size_t iteratedPower = 0,
  ...
  void Apply(const arma::mat& data,
             ...
             arma::vec& eigVal,
             arma::mat& eigvec,const size_t rank)
  {
    // This matrix will store the right singular values; we do not need them.
    arma::mat v;

    // Do singular value decomposition using the robust SVD algorithm.
    svd::RobustSVD rsvd(iteratedPower, maxIterations);
    rsvd.Apply(data, eigvec, eigVal, v);
