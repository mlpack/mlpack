...
PROGRAM_INFO(
    ...
    "The training set is specified with the "
        ...
        + " parameter.  Labels may be either the "
          "last row of the training set, or alternately the "
        + PRINT_PARAM_STRING("labels")
        + " parameter may be specified to pass a "
          "separate matrix of labels."
          "\n\n"
          "If training is not desired, a pre-existing model may be loaded with "
          "the "
        ...
        + " parameter."
          ...
          "\n\n"
          "The "
        + PRINT_PARAM_STRING("incremental_variance")
        + " parameter can be "
          "used to force the training to use an incremental algorithm for "
          "calculating"
          " variance.  This is slower, but can help avoid loss of precision in "
          "some "
          "cases."
          "\n\n"
          "If classifying a test set is desired, the test set may be specified "
          "with "
          "the "
        ...
        + " output parameter.  If saving the trained model is desired, this "
          "may be "
          "done with the "
        ...
        + " "
          "and save the model to "
        + PRINT_MODEL("nbc_model")
        + ", the following "
          "command may be used:"
          "\n\n"
        + PRINT_CALL("nbc",
...
PARAM_MODEL_IN(NBCModel,
               "input_model",
               "Input Naive Bayes "
               "model.",
               "m");
PARAM_MODEL_OUT(NBCModel,
                "output_model",
...
PARAM_FLAG("incremental_variance",
           "The variance of each class will be "
...
PARAM_UROW_OUT("output",
               ...
               "o");
PARAM_MATRIX_OUT("output_probs",
                 "The matrix in which the predicted probability"
...
static void mlpackMain()
{
  ...
  RequireAtLeastOnePassed({"output", "output_model", "output_probs"},
                          false,
  ...
  if (CLI::HasParam("training"))
  {
    ...
    else
    {
      ...
      data::NormalizeLabels(
          trainingData.row(trainingData.n_rows - 1), labels, model.mappings);
    ...
    model.nbc = NaiveBayesClassifier<>(
        trainingData, labels, model.mappings.n_elem, incrementalVariance);
  ...
  if (CLI::HasParam("test"))
  {
    ...
    if (testingData.n_rows != model.nbc.Means().n_rows)
    {
      Log::Fatal << "Test data dimensionality (" << testingData.n_rows << ") "
                 ...
                 << model.nbc.Means().n_rows << ")!" << std::endl;
    }
