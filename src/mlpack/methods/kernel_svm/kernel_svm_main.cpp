 ...
 *
 * Main executable for kernel svm.
 *
...
PROGRAM_INFO("Kenrel SVM is an support vector machine.",
    // Short description.
    ...
    // Long description.
    "An implementation of Kenrel SVMs that uses smo to train the model."
    "\n\n"
...
static void mlpackMain()
{
  ...
  {
    model = CLI::GetParam<KernelSVMModel*>("input_model");
  }
  else
  {
  ...
  if (CLI::HasParam("training"))
  {
    ...
    if (optimizerType == "lbfgs")
    {
      ...
      model->svm.Train(trainingSet, labels, numClasses);
    }
  ...
  CLI::GetParam<KernelSVMModel*>("output_model") = model;
}
