...
static void mlpackMain()
{
  ...
  const int lsInt = CMD::GetParam<int>("leaf_size");
  ...
  else
  {
    ...
    rann = CMD::GetParam<RANNModel*>("input_model");

    Log::Info << "Using rank-approximate kNN model from '"
        << CMD::GetPrintableParam<RANNModel>("input_model") << "' (trained on "
        << rann->Dataset().n_rows << "x" << rann->Dataset().n_cols
        << " dataset)." << endl;

    // Adjust singleMode and naive if necessary.
    rann->SingleMode() = CMD::HasParam("single_mode");
    rann->Naive() = CMD::HasParam("naive");
    rann->LeafSize() = size_t(lsInt);
  }

  // Apply the parameters for search.
  if (CMD::HasParam("tau"))
    rann->Tau() = CMD::GetParam<double>("tau");
  if (CMD::HasParam("alpha"))
    rann->Alpha() = CMD::GetParam<double>("alpha");
  if (CMD::HasParam("single_sample_limit"))
    rann->SingleSampleLimit() = CMD::GetParam<double>("single_sample_limit");
  rann->SampleAtLeaves() = CMD::HasParam("sample_at_leaves");
  rann->FirstLeafExact() = CMD::HasParam("sample_at_leaves");
  ...
  CMD::GetParam<RANNModel*>("output_model") = rann;
}
