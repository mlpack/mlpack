...
PROGRAM_INFO(
    ...
    "The input dataset to be clustered may be specified with the "
        ...
        + " parameter; the radius of each range "
          "search may be specified with the "
        + PRINT_PARAM_STRING("epsilon")
        + " parameters, and the minimum number of points in a cluster may be "
          "specified with the "
        + PRINT_PARAM_STRING("min_size")
        + " parameter."
          "\n\n"
          "The "
        + PRINT_PARAM_STRING("assignments")
        + " and "
        + PRINT_PARAM_STRING("centroids")
        + " output parameters may be "
          "used to save the output of the clustering. "
        + PRINT_PARAM_STRING("assignments")
        + " contains the cluster assignments of "
          "each point, and "
        + PRINT_PARAM_STRING("centroids")
        + " contains the "
        ...
        + ", "
        + PRINT_PARAM_STRING("single_mode")
        + ", and "
        + PRINT_PARAM_STRING("naive")
        + " parameters.  "
        + PRINT_PARAM_STRING("tree_type")
        + " can control the type of tree used for "
          "range search; this can take a variety of values: 'kd', 'r', "
          "'r-star', 'x',"
          " 'hilbert-r', 'r-plus', 'r-plus-plus', 'cover', 'ball'. The "
        + PRINT_PARAM_STRING("single_mode")
        + " parameter will force single-tree "
        ...
        + PRINT_DATASET("input")
        + " with a radius of 0.5 and a minimum cluster size"
          " of 5 is given below:"
          "\n\n"
        + PRINT_CALL(
...
PARAM_UROW_OUT("assignments",
               "Output matrix for assignments of each "
               "point.",
               "a");
...
PARAM_FLAG("single_mode",
           ...
           "S");
PARAM_FLAG("naive",
           "If set, brute-force range search (not tree-based) "
           "will be used.",
           "N");
...
void RunDBSCAN(RangeSearchType rs = RangeSearchType())
{
  ...
  DBSCAN<RangeSearchType> d(
      epsilon, minSize, !CLI::HasParam("single_mode"), rs);
...
static void mlpackMain()
{
  RequireAtLeastOnePassed(
      {"assignments", "centroids"}, false, "no output will be saved");
