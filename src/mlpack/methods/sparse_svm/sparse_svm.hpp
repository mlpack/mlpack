 ...
 *
 * An implementation of support vector machine.
 *
...
/**
 * Support-vector machine is supervised learning models
 ...
 * const size_t inputSize; // Size of input feature vector.
 * const size_t numClasses; // Number of classes.
 * const size_t maxIterations; // Maximum number of iterations.
 * const size_t threadShareSize; // Number of datapoints to be processed.
 * 
 * // Train the model using default options.
 * SparseSVM<> svm1(train_data, labels, inputSize, numClasses);

 * // Use an instantiated optimizer for the training.
 * ParallelSGD<> optimizer(maxIterations, threadShareSize);
 * SparseSVM<ParallelSGD<> > svm2(optimizer);
 *
 ...
 * svm1.Classify(test_data, predictions1);
 * svm2.Classify(test_data, predictions2);
 * @endcode
 */
class SparseSVM
{
 public:
  /**
   * Initialize the SparseSVM without performing training.  Default
   ...
   */
  SparseSVM(const size_t inputSize = 0,
  ...
  /**
   * Train the svm with the given training data.
   *
   * @tparam OptimizerType Desired optimizer type.
