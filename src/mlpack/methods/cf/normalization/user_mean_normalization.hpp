...
class UserMeanNormalization
{
 public:
  ...
  void Normalize(arma::mat& data)
  {
    ...
    userMean = arma::vec(userNum, internal_compact::fill::zeros);
    // Number of ratings for each user.
    arma::Row<size_t> ratingNum(userNum, internal_compact::fill::zeros);
  ...
  {
    // Calculate userMean.
    userMean = arma::vec(cleanedData.n_cols, internal_compact::fill::zeros);
    arma::Col<size_t> ratingNum(cleanedData.n_cols, internal_compact::fill::zeros);
