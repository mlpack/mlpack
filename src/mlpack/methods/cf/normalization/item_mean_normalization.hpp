...
class ItemMeanNormalization
{
 public:
  ...
  void Normalize(arma::mat& data)
  {
    ...
    itemMean = arma::vec(itemNum, internal_compact::fill::zeros);
    // Number of ratings for each item.
    arma::Row<size_t> ratingNum(itemNum, internal_compact::fill::zeros);
  ...
  {
    // Calculate itemMean.
    itemMean = arma::vec(cleanedData.n_rows, internal_compact::fill::zeros);
    arma::Col<size_t> ratingNum(cleanedData.n_rows, internal_compact::fill::zeros);
