...
static void mlpackMain()
{
  ...
  const arma::mat& matReqIn = CMD::GetParam<arma::mat>("mat_req_in");
  const arma::vec& colReqIn = CMD::GetParam<arma::vec>("col_req_in");
  ...
  if (CMD::HasParam("smatrix_in"))
  {
    arma::mat out = move(CMD::GetParam<arma::mat>("smatrix_in"));
    out *= 2.0;

    CMD::GetParam<arma::mat>("smatrix_out") = move(out);
  }

  // An input matrix (pandas.Series) should have all elements multiplied by two.
  if (CMD::HasParam("s_umatrix_in"))
  {
    arma::Mat<size_t> out =
        move(CMD::GetParam<arma::Mat<size_t>>("s_umatrix_in"));
    out *= 2;

    CMD::GetParam<arma::Mat<size_t>>("s_umatrix_out") = move(out);
  }
