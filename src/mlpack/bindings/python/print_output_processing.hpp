        ...
        std::tuple<data::DatasetInfo, arma::mat>>>::type* = 0)
{
  ...
  if (onlyOutput)
  {
     ...
     *
     * result = CMD.GetParam[int]('param_name')
     */
    std::cout << prefix << "result = " << "CMD.GetParam[" << GetCythonType<T>(d)
        << "](\"" << d.name << "\")";
  ...
  else
  {
     ...
     * This gives us code like:
     *
     * result['param_name'] = CMD.GetParam[int]('param_name')
     */
    std::cout << prefix << "result['" << d.name << "'] = CMD.GetParam["
    ...
    const typename boost::enable_if<arma::is_arma_type<T>>::type* = 0)
{
  ...
  if (onlyOutput)
  {
     ...
     * This gives us code like:
     *
     * result = arma_numpy.mat_to_numpy_X(CMD.GetParam[mat]("name"))
     *
     * where X indicates the type to convert to.
     */
    std::cout << prefix << "result = arma_numpy." << GetArmaType<T>()
        << "_to_numpy_" << GetNumpyTypeChar<T>() << "(CMD.GetParam["
        << GetCythonType<T>(d) << "](\"" << d.name << "\"))" << std::endl;
  }
  else
  {
     ...
     *
     * result['param_name'] =
     *     arma_numpy.mat_to_numpy_X(CMD.GetParam[mat]('name')
     *
     * where X indicates the type to convert to.
     */
    std::cout << prefix << "result['" << d.name
        << "'] = arma_numpy." << GetArmaType<T>() << "_to_numpy_"
        << GetNumpyTypeChar<T>() << "(CMD.GetParam[" << GetCythonType<T>(d)
        << "]('" << d.name << "'))" << std::endl;
  }
}
