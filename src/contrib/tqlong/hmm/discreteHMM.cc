...
size_t MaxLength(const ArrayList<DiscreteHMM::OutputSeq>& seqs);
...
void DiscreteHMM::Generate(size_t length, OutputSeq* seq,
			   StateSeq* states){
  Matrix cTR, cE;
...
void DiscreteHMM::calPStates(size_t L, Matrix* pStates, 
			     Matrix* fs, Matrix* bs) {
...
double DiscreteHMM::calPSeq(size_t length, Vector* scale) {
  double logSeq = 0.0;
