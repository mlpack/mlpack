...
class DualtreeKdeCommon {
 ...
 public:
  ...
  static void AddPostponed(TTree *node, size_t destination, 
   ...
   */
  template<typename TTree, typename TAlgorithm>
  static void RefineBoundStatistics(size_t q, TTree *qnode, 
				    TAlgorithm *kde_object) {
  ...
  (double population_size, double sample_size,
   ...
   double population_order_statistics_min_index) {
	...
	sample_size - r_star, population_size, sample_size);
      */
      total_probability +=
	boost::math::binomial_coefficient<double>((int) sample_size, (int) r_star) *
  ...
  (TTree *qnode, TTree *rnode, double probability, Range &dsqd_range,
   Range &kernel_value_range, double &dl, double &de, double &du, 
   double &used_error, double &n_pruned, TAlgorithm *kde_object) {
	...
	for(size_t s = 0; s < num_samples; s++) {
	  ...
	  const arma::vec query_point = 
	    (kde_object->qset_).unsafe_col(random_query_point_index);
	  
	  // Get the pointer to the current reference point.
	  const arma::vec reference_point = 
	    (kde_object->rset_).unsafe_col(random_reference_point_index);
	  
	  // Compute the pairwise distance and kernel value.
	  double squared_distance = kernel::LMetric<2,false>::Evaluate(query_point, reference_point);
