...
void npt::SingleDriver::Compute() {
  ...
  std::vector<NptNode*> this_comp_trees(tuple_size_);
  ...
  for (int num_random = 0; num_random < tuple_size_; num_random++) {
    ...
    for (int i = 0; i < num_resampling_regions_; i++) {
      ...
      if (num_random == 0) {
        ...
        this_comp_trees[1] = data_tree_i;
        this_comp_trees[2] = data_tree_i;
      ...
      for (unsigned int r1_ind = 0; r1_ind < r1_vec_.size(); r1_ind++) {
        
        for (unsigned int theta_ind = 0; theta_ind < theta_vec_.size(); 
             theta_ind++) {
          ...
          results_.ProcessResults(i, num_random, matcher,
                                  r1_ind, theta_ind);
  ...
  for (unsigned int r1_ind = 0; r1_ind < r1_vec_.size(); r1_ind++) {
    ...
    for (unsigned int theta_ind = 0; theta_ind < theta_vec_.size(); 
      ...
      // process and store results from the matcher
      results_.ProcessResults(num_resampling_regions_, num_random, matcher,
                              r1_ind, theta_ind);
