...
int main(int argc, char *argv[]) {
  ...
  size_t num_of_classes = fx_param_int_req(fx_root, "num_of_classes");
  ...
  for(size_t i=0; i<labels.n_cols(); i++) {
    if (labels.get(0, i)<0 || labels.get(0, i)>=num_of_classes) {
      FATAL("Sanity check failed label %i:%i is out of the class range [0 %i]",
            i, size_t(labels.get(0, i)), num_of_classes);
    }
  }
  ...
  size_t new_dimension =fx_param_int_req(fx_root, "/opt_fun/new_dimension");
  size_t num1=labeled_data_points.n_cols();
  ...
  size_t num2=unlabeled_data_points.n_cols();
  if (!unlabeled_points_file.empty()) {
  ...
  if (fx_param_exists(fx_root, "validation_file")) {
    ...
    if (mode=="mvu") {
      ...
      if (mode=="svm") {
        ...
        total_score=0.0;
        for(size_t i=0; i<validation_labels.n_cols(); i++) {
...
double  ComputeClassificationScore(fx_module *module, 
  ...
  size_t knns=fx_param_int(allknn_module, "knns", 5);
  size_t num_of_classes=fx_param_int_req(module, "num_of_classes");
  ...
  for(size_t i=0; i<unlabeled_data_points.n_cols(); i++) {
    size_t score[num_of_classes];
    memset(score, 0, num_of_classes*sizeof(size_t));
    for(size_t j=0; j<knns; j++) {
      size_t n=neighbors[i*knns+j];
      score[size_t(labels.get(0,n))]+=1;
    }
  ...
  if (fx_param_exists(module, "validation_file")) {
    ...
    for(size_t i=0; i<classification_results->n_cols(); i++) {
