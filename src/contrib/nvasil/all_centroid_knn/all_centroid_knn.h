...
class AllCentroidkNN {
  ...
  */
  class QueryStat {
   ...
   public:
    ...
    void set_centroid_id(size_t centroid_id) {
 ...
 // A permutation of the indices for tree building.
  ArrayList<size_t> old_from_new_points_;
 ...
 /////////////////////////////// Helper Functions ///////////////////////////////////////////////////
  ...
  void RetrieveCentroidsRecursion_(TreeType *node, size_t level,
      size_t parent_centroid_id,  
      ArrayList<size_t> *centroid_ids, Matrix *features) { 
  ...
  void FromCentroidsToPointsRecurse_(TreeType *node, Matrix &centroid_features, 
    ...
    if (node->is_leaf()) {
      for(size_t i=node->begin(); i<node->end() ;i++) {
        size_t ind=old_from_new_points_[i];
  ...
  void FromCentroidsToPointsRecurse_(TreeType *node, Matrix &centroid_features, 
      size_t level, Matrix *point_features) {
    if (level==0 || node->is_leaf()) {
      Vector perturb;
  ...
  void ComputeDepth_(TreeType *node, size_t level) {
    if (node->is_leaf()) {
 ...
 public:
  static size_t centroid_counter_; 
  /**
...
*/
  ...
  void RetrieveCentroids(size_t level, ArrayList<size_t> *centroid_ids, 
      Matrix *features) {
    centroid_ids->Init();
    RetrieveCentroidsRecursion_(tree_for_centroids_, level, 0, centroid_ids, features);
  }

  void RetrieveCentroids(double range, ArrayList<size_t> *centroid_ids, 
      Matrix *features) {
    centroid_ids->Init();
    RetrieveCentroidsRecursion_(tree_for_centroids_, range, 0, centroid_ids, features);
  }

  void AllkCentroids(Matrix &centroids, ArrayList<size_t> &centroid_ids,
  ...
  void FromCentroidsToPoints1(Matrix &centroids, Matrix &points, 
      Matrix &centroid_features, ArrayList<size_t> &centroid_ids,
      Matrix *point_features) {
    AllkNN allknn;
    size_t knns=5;
    ...
    for(size_t i=0; i<points.n_cols(); i++)  {
      ...
      for(size_t j=0; j<knns; j++) {
         ...
         size_t ind=resulting_neighbors[i*knns_+j];
         Vector vec1;
  ...
  size_t tree_min_depth() {
    return tree_min_depth_;
  }
 
}; //class AllCentroidkNN 

size_t AllCentroidkNN::centroid_counter_=0; 
