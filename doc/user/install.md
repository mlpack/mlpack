...
| `-DMLPACK_USE_SYSTEM_STB=OFF` | Disable system's STB and use mlpack bundled version, if decided otherwise please be sure to have `stb_image.h`, `stb_image_write.h`, and `stb_image_resize2.h` on the system. | `OFF` |
|--------------|-------------------|---------------|
