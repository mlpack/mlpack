...
| `-DDISABLE_STB=OFF` | Disable STB support within mlpack; use this to reduce program size if you do not need image processing (e.g., you are only using real-time sensor data). | `OFF ` |
...
| `-DDISABLE_CACHE_REMOTE_DATASETS=OFF` | Disable caching of downloaded datasets.  When set to `ON`, files downloaded via `Load()` are saved to the system temporary directory instead of the cache directory. | `OFF` |
| `-DCACHE_DIR=/path/to/cache/` | Override the directory where cached datasets are stored.  Defaults to `$HOME/.mlpack/cache/` on Linux/macOS and `%APPDATA%\mlpack\cache\` on Windows. | |
|--------------|-------------------|---------------|
