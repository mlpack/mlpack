...
| `-DREMOTE_CACHE_DIR=/path/to/cache/` | Override the directory where cached datasets are stored.  Defaults to `$HOME/.mlpack/cache/` on Linux/macOS and `%APPDATA%\mlpack\cache\` on Windows. | |
|--------------|-------------------|---------------|
