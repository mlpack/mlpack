 ...
 * *Splitting strategies*:
   - [`MidpointSplit`](#midpointsplit)

 * *Statistics*:
   - [`EmptyStatistic`](#emptystatistic)
 ...
 * `b = HRectBound<DistanceType>(dimensionality)`
   ...
   - `DistanceType` is required to be an [`LMetric`](distances.md#lmetric), as
     the distance calculation must be decomposable across dimensions.
 ...
 * `b[dim]` will return a [`Range`](math.md#range) object holding the lower and
   upper bounds of `b` in dimension `dim`.
 ...
 * `b.Distance()` returns either a [`EuclideanDistance`](distances.md#lmetric)
   distance metric object, or a `DistanceType` if a custom `DistanceType` has
...
[`KDTree`s](#kdtree).  The class implements the
[`SplitType` policy](#writing-a-custom-splittype).
...
default
[`StatisticType`](../../developer/trees.md#template-parameters-required-by-the-treetype-policy)
...
Statistics are held in each node of a tree structure and can be used to store
precomputed quantities during the tree-building process.  For more details on
how to implement a custom `StatisticType`, see the
[developer documentation on `StatisticType`s](../../developer/trees.md#statistictype).
...
[`BinarySpaceTree`](#binaryspacetree) class, using the
[`HRectBound`](#hrectbound) class as the bounding structure, and using the
[`MidpointSplit`](#midpointsplit) splitting strategy for construction, which
splits a node in the dimension of maximum variance on the midpoint of the
 ...
 * `node.Distance()` will return a [`EuclideanDistance&`](distances.md#lmetric)
   (or a `DistanceType&` if a [custom `DistanceType`](#template-parameters) was
   ...
   - This function is required by the
     ...
     `KDTree` requires an [`LMetric`](distances.md#lmetric) to be used, and
     `LMetric` only has `static` functions and holds no state, this function is
     not likely to be useful.

See also the
[developer documentation](../../developer/trees.md#basic-tree-functionality) for
basic tree functionality in mlpack.
...
Build a `KDTree` on the `cloud` dataset and print basic statistics about the
tree.
...
[`KDTree`](#kdtree).  In general, the `BinarySpaceTree` class is not meant to be
used directly, and instead one of the numerous variants should be used instead:
...
supports the [TreeType API](../../developer/trees.md#the-treetype-api) and can
be used with mlpack's tree-based algorithms, although using custom behavior may
require a template typedef.

 * [Template parameters](#template-parameters-1)
 * [Constructors](#constructors-2)
 * [Basic tree properties](#basic-tree-properties-1)
 * [Bounding distances with the tree](#bounding-distances-with-the-tree-1)
 ...
 * `BoundType`: the class defining the bound for each node.  By default,
   ...
   - Note that this template parameter must be a template class accepting at
     least two arguments: `BoundType<BoundDistanceType, BoundElemType>`.
   - Internally, `BinarySpaceTree` will use `BoundType<DistanceType, typename
     MatType::elem_type>` (where `MatType::elem_type` is the element type of
     given `MatType`, e.g., `double` for `arma::mat`).
   ...
   - [Custom `BoundType` classes](#writing-a-custom-boundtype) can be used.
 ...
 * `SplitType`: the class defining how an individual `BinarySpaceTree` node
   ...
   - Note that this template parameter must be a template class accepting only
     two arguments: `SplitType<SplitBoundType, SplitMatType>`.
   - Internally, `BinarySpaceTree` will use `SplitType<BoundType<DistanceType,
     typename MatType::elem_type>, typename MatType::elem_type>` (where
     `MatType::elem_type` is the element type of the given `MatType`).
   - [Custom `SplitType` classes](#writing-a-custom-splittype) can be used.
...
the typedefs of `BinarySpaceTree` (such as [`KDTree`](#kdtree)) will suffice.

For this reason, all of the examples below explicitly specify all five template
parameters of `BinarySpaceTree`.
[Writing a custom bound type](#writing-a-custom-boundtype) and
[writing a custom splitting strategy](#writing-a-custom-splittype) are discussed
