...
The `RPlusTree` class implements the R+-tree, a variation of the
[R tree](r_tree.md) that attempts to avoid overlap in internal nodes using a
clever splitting strategy inspired by the [kd-tree](kdtree.md).
...
the `RPlusTree` class takes three template parameters:

```
RPlusTree<DistanceType, StatisticType, MatType>
```
...
The `RPlusTree` class itself is a convenience typedef of the generic
[`RectangleTree`](rectangle_tree.md) class, using the
[`RPlusTreeSplit`](rectangle_tree.md#rplustreesplit) class as the split
strategy, the
[`RPlusTreeDescentHeuristic`](rectangle_tree.md#rplustreedescentheuristic) class
...
`RPlusTree`s are constructed by inserting points in a dataset sequentially.
The dataset is not permuted during the construction process.

---

 * `node = RPlusTree(data)`
 * `node = RPlusTree(data, maxLeafSize=20, minLeafSize=8)`
 * `node = RPlusTree(data, maxLeafSize=20, minLeafSize=8, maxNumChildren=5, minNumChildren=2)`
   - Construct an `RPlusTree` on the given `data` with the given construction
     parameters.
 ...
 * `node = RPlusTree<DistanceType, StatisticType, MatType>(data)`
 * `node = RPlusTree<DistanceType, StatisticType, MatType>(data, maxLeafSize=20, minLeafSize=8)`
 * `node = RPlusTree<DistanceType, StatisticType, MatType>(data, maxLeafSize=20, minLeafSize=8, maxNumChildren=5, minNumChildren=2)`
   - Construct an `RPlusTree` on the given `data`, using custom template
 ...
 * `node = RPlusTree(dimensionality)`
   - Construct an empty `RPlusTree` with no children, no points, and
...
***Notes:***
 ...
 - The name `node` is used here for `RPlusTree` objects instead of `tree`,
   because each `RPlusTree` object is a single node in the tree.  The
...
Once an `RPlusTree` object is constructed, various properties of the tree can be
 ...
 * `node.Descendant(i)` returns a `size_t` indicating the index of the `i`'th
   ...
   - Descendant point indices are not necessarily contiguous for
     `RPlusTree`s; that is, `node.Descendant(i) + 1` is not necessarily
     `node.Descendant(i + 1)`.
...
The following quantities are cached for each node in a `RPlusTree`, and so
 ...
 * `node.GetFurthestChild(other)`
   - Return a `size_t` indicating the index of the child that is closest to (or
     furthest from) the `RPlusTree` node `other`, with respect to the
 ...
 * `node.MinDistance(other)`
   - Return a `double` indicating the minimum possible distance between `node`
     and `point`, or the `RPlusTree` node `other`.
 ...
 * `RPlusTree::SingleTreeTraverser`
   - Implements a depth-first single-tree traverser.

 * `RPlusTree::DualTreeTraverser`
   - Implements a dual-depth-first dual-tree traverser.

## Example usage

Build an `RPlusTree` on the `cloud` dataset and print basic statistics about the
tree.
...
// `mlpack::RPlusTree tree(...)` will work fine in C++20 or newer).
mlpack::RPlusTree<> tree(std::move(dataset));
...
Build two `RPlusTree`s on subsets of the corel dataset and compute minimum and
...
mlpack::RPlusTree<> tree2(dataset.cols(dataset.n_cols / 2 + 1,
                          dataset.n_cols - 1));
...
Build an `RPlusTree` on 32-bit floating point data and save it to disk.
...
mlpack::RPlusTree<mlpack::EuclideanDistance,
...
Load a 32-bit floating point `RPlusTree` from disk, then traverse it manually
...
using TreeType = mlpack::RPlusTree<mlpack::EuclideanDistance,
                                   mlpack::EmptyStatistic,
                                   arma::fmat>;
...
Build an `RPlusTree` by iteratively inserting points from the corel dataset,
