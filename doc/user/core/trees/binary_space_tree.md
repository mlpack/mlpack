 ...
 * [`NeighborSearchStat`](#neighborsearchstat): a statistic class storing data
...
The `NeighborSearchStat` class is a utility structure that holds a number of
...
In general, there is no need to directly interact with the `NeighborSearchStat`,
and classes such as [`KNN`](../../methods/knn.md) provide a convenience typedef
(e.g. `KNN::TreeType`) that corresponds to the tree type with
`NeighborSearchStat` as the `StatisticType`.

For more details, see
[the source code](/src/mlpack/src/mlpack/methods/neighbor_search/neighbor_search_stat.hpp).
