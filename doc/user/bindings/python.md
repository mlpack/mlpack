...
```python
>>> from mlpack import decision_tree_train
>>> d = decision_tree_train(check_input_matrices=False,
        copy_all_inputs=False, labels=np.empty([0], dtype=np.uint64),
        maximum_depth=0, minimum_gain_split=1e-07, minimum_leaf_size=20,
        print_training_accuracy=False, training=np.empty([0, 0]), verbose=False,
