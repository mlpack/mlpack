...
```julia
julia> using mlpack: decision_tree_train
julia> output_model = decision_tree_train(training; labels=Int[],
          maximum_depth=0, minimum_gain_split=1e-07, minimum_leaf_size=20,
          print_training_accuracy=false, verbose=false, weights=zeros(0, 0))
```
