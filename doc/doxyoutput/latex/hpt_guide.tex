\doxysection{Introduction}\label{hpt_guide_hptintro}
...
In this tutorial we will see the usage examples of the hyper-\/parameter tuning module, and also more details about the {\ttfamily \doxyref{Hyper\+Parameter\+Tuner}{p.}{classmlpack_1_1hpt_1_1HyperParameterTuner}} class.\doxysection{Basic Usage}\label{hpt_guide_hptbasic}
...
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// Finding a good value for lambda from the discrete set of values 0.0, 0.001,}}
\DoxyCodeLine{\textcolor{comment}{// 0.01, 0.1, and 1.0.}}
...
In this example we have used {\ttfamily ens\+::\+Grid\+Search} (the default optimizer) to find a good value for the {\ttfamily lambda} hyper-\/parameter. For that we have specified what values should be tried.\doxysection{Fixed Arguments}\label{hpt_guide_hptfixed}
When some hyper-\/parameters should not be optimized, you can specify values for them with the {\ttfamily \doxyref{Fixed()}{p.}{namespacemlpack_1_1hpt_ad773f4d1def8deb412ffbf37bdf289ec}} method as in the following example of trying to find good {\ttfamily lambda1} and {\ttfamily lambda2} values for \doxyref{LARS}{p.}{classmlpack_1_1regression_1_1LARS} (least-\/angle regression).
...
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// The hyper-\/parameter tuner should not try to change the transposeData or}}
\DoxyCodeLine{\textcolor{comment}{// useCholesky parameters.}}
...
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// We wish only to search for the best lambda1 and lambda2 values.}}
...
Note that for the call to {\ttfamily hpt2.\+Optimize()}, we have used the same order of arguments as they appear in the corresponding \doxyref{LARS}{p.}{classmlpack_1_1regression_1_1LARS} constructor\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{LARS(\textcolor{keyword}{const} arma::mat\& data,}
\DoxyCodeLine{     \textcolor{keyword}{const} arma::rowvec\& responses,}
\DoxyCodeLine{     \textcolor{keyword}{const} \textcolor{keywordtype}{bool} transposeData = \textcolor{keyword}{true},}
\DoxyCodeLine{     \textcolor{keyword}{const} \textcolor{keywordtype}{bool} useCholesky = \textcolor{keyword}{false},}
\DoxyCodeLine{     \textcolor{keyword}{const} \textcolor{keywordtype}{double} lambda1 = 0.0,}
\DoxyCodeLine{     \textcolor{keyword}{const} \textcolor{keywordtype}{double} lambda2 = 0.0,}
\DoxyCodeLine{     \textcolor{keyword}{const} \textcolor{keywordtype}{double} tolerance = 1e-\/16);}

\end{DoxyCode}
\doxysection{Gradient-\/\+Based Optimization}\label{hpt_guide_hptgradient}
...
In this case, we can use a gradient-\/based optimizer for hyperparameter search. In the following example, we try to optimize the {\ttfamily lambda1} and {\ttfamily lambda2} hyper-\/parameters for \doxyref{LARS}{p.}{classmlpack_1_1regression_1_1LARS} with the {\ttfamily ens\+::\+Gradient\+Descent} optimizer.


\begin{DoxyCode}{0}
\DoxyCodeLine{HyperParameterTuner<LARS, MSE, SimpleCV, GradientDescent> hpt3(validationSize,}
\DoxyCodeLine{    data, responses);}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// GradientDescent can be adjusted in the following way.}}
\DoxyCodeLine{hpt3.Optimizer().StepSize() = 0.1;}
\DoxyCodeLine{hpt3.Optimizer().Tolerance() = 1e-\/15;}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// We can set up values used for calculating gradients.}}
\DoxyCodeLine{hpt3.RelativeDelta() = 0.01;}
\DoxyCodeLine{hpt3.MinDelta() = 1e-\/10;}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keywordtype}{double} initialLambda1 = 0.001;}
\DoxyCodeLine{\textcolor{keywordtype}{double} initialLambda2 = 0.002;}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keywordtype}{double} bestGDLambda1, bestGDLambda2;}
\DoxyCodeLine{std::tie(bestGDLambda1, bestGDLambda2) = hpt3.Optimize(Fixed(transposeData),}
\DoxyCodeLine{    Fixed(useCholesky), initialLambda1, initialLambda2);}

\end{DoxyCode}
\doxysection{The Hyper\+Parameter\+Tuner class}\label{hpt_guide_hpt_class}
The {\ttfamily \doxyref{Hyper\+Parameter\+Tuner}{p.}{classmlpack_1_1hpt_1_1HyperParameterTuner}} class is very similar to the \doxyref{KFold\+CV}{p.}{classmlpack_1_1cv_1_1KFoldCV} and \doxyref{Simple\+CV}{p.}{classmlpack_1_1cv_1_1SimpleCV} classes (see the \doxyref{cross-\/validation tutorial}{p.}{cv} for more information on those two classes), but there are a few important differences.
...
\begin{DoxyItemize}
\item {\ttfamily MLAlgorithm} This is the algorithm to be used.
...
\item {\ttfamily CVType} This is the type of cross-\/validation to be used for evaluating the performance measure; this should be \doxyref{KFold\+CV}{p.}{classmlpack_1_1cv_1_1KFoldCV} or \doxyref{Simple\+CV}{p.}{classmlpack_1_1cv_1_1SimpleCV}.
...
Typically, \doxyref{Simple\+CV}{p.}{classmlpack_1_1cv_1_1SimpleCV} is a good choice for {\ttfamily CVType} because it takes so much less time to compute than full \doxyref{KFold\+CV}{p.}{classmlpack_1_1cv_1_1KFoldCV}; however, the disadvantage is that \doxyref{Simple\+CV}{p.}{classmlpack_1_1cv_1_1SimpleCV} might give a somewhat more noisy estimate of the performance measure on unseen test data.

The constructor for the {\ttfamily \doxyref{Hyper\+Parameter\+Tuner}{p.}{classmlpack_1_1hpt_1_1HyperParameterTuner}} is called with exactly the same arguments as the corresponding {\ttfamily CVType} that has been chosen. For more information on that, please see the \doxyref{cross-\/validation constructor tutorial}{p.}{cv_cvbasic_api}. As an example, if we are using \doxyref{Simple\+CV}{p.}{classmlpack_1_1cv_1_1SimpleCV} and wish to hold out 20\% of the dataset as a validation set, we might construct a {\ttfamily \doxyref{Hyper\+Parameter\+Tuner}{p.}{classmlpack_1_1hpt_1_1HyperParameterTuner}} like this\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// We will use LinearRegression as the MLAlgorithm, and MSE as the performance}}
\DoxyCodeLine{\textcolor{comment}{// measure.  Our dataset is 'dataset' and the responses are 'responses'.}}
\DoxyCodeLine{HyperParameterTuner<LinearRegression, MSE, SimpleCV> hpt(0.2, dataset,}
\DoxyCodeLine{    responses);}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{arma::vec lambdaSet = arma::vec(\textcolor{stringliteral}{"{}0.0 0.1 0.5 1.0"{}});}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{std::vector<bool> interceptSet = \{ \textcolor{keyword}{false}, \textcolor{keyword}{true} \};}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keywordtype}{bool} intercept;}
\DoxyCodeLine{\textcolor{keywordtype}{double} lambda;}
\DoxyCodeLine{std::tie(lambda, intercept) = hpt.Optimize(lambdaSet, interceptSet);}
...
For continuous optimizers like {\ttfamily ens\+::\+Gradient\+Descent}, a range does not need to be specified but instead only a single value. See the \doxyref{Gradient-\/\+Based Optimization}{p.}{hpt_guide_hptgradient} section for more details.\doxysection{Further documentation}\label{hpt_guide_hptfurther}
