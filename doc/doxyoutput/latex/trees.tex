\doxysection{Introduction}\label{trees_treeintro}
...
\item \doxyref{The Tree\+Type API}{p.}{trees_treetype_api}
\item \doxyref{Rigorous API documentation}{p.}{trees_treetype_rigorous}
\begin{DoxyItemize}
...
Although this document is long, there may still be errors and unclear areas. If you are having trouble understanding anything, please get in touch on Github or on the mailing list and someone will help you (and possibly update the documentation afterwards).\doxysection{What is a tree?}\label{trees_whatistree}
In mlpack, we assume that we have some sort of data matrix, which might be sparse or dense (that is, it could be of type {\ttfamily arma\+::mat} or {\ttfamily arma\+::sp\+\_\+mat}, or any variant that implements the Armadillo API). This data matrix corresponds to a collection of points in some space (usually a Euclidean space). A tree is a way of organizing this data matrix in a hierarchical manner---so, points that are nearby should lie in similar nodes.
...
\begin{DoxyCode}{0}
\DoxyCodeLine{@quote}
\DoxyCodeLine{R.R. Curtin, W.B. March, P. Ram, D.V. Anderson, A.G. Gray, and C.L. Isbell Jr.,}
\DoxyCodeLine{\textcolor{stringliteral}{"{}Tree-\/independent dual-\/tree algorithms,"{}} in Proceedings of the 30th}
\DoxyCodeLine{International Conference on Machine Learning (ICML \textcolor{stringliteral}{'13), pp. 1435-\/-\/1443, 2013.}}
\DoxyCodeLine{\textcolor{stringliteral}{@endquote}}
...
So there is a huge amount of possible variety in the types of trees that can fall into the class of {\itshape space trees}. Therefore, it\textquotesingle{}s important to treat them abstractly, and the {\ttfamily Tree\+Type} policy allows us to do just that. All we need to remember is that a node in a tree can be represented as the combination of some points held in the node, some child nodes, and some geometric structure that represents the space that all of the descendant points fall into (this is a restatement of the fourth part of the definition).\doxysection{Template parameters required by the Tree\+Type policy}\label{trees_treetype_template_params}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// The RangeSearch class takes a MetricType and a TreeType template parameter.}}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// This code instantiates RangeSearch with the ManhattanDistance and a}}
\DoxyCodeLine{\textcolor{comment}{// QuadTree.  Note that the QuadTree itself is a template, and takes a}}
\DoxyCodeLine{\textcolor{comment}{// MetricType, StatisticType, and MatType, just like the policy requires.}}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// This example ignores the constructor parameters, for the sake of simplicity.}}
\DoxyCodeLine{RangeSearch<ManhattanDistance, QuadTree> rs(...);}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// This is a much worse alternative, where the user must specify the template}}
\DoxyCodeLine{\textcolor{comment}{// arguments of their tree.}}
\DoxyCodeLine{RangeSearch<ManhattanDistance,}
\DoxyCodeLine{            QuadTree<ManhattanDistance, EmptyStatistic, arma::mat>> rs(...);}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// This is the definition of the BinarySpaceTree class, which has five template}}
\DoxyCodeLine{\textcolor{comment}{// parameters.}}
\DoxyCodeLine{\textcolor{keyword}{template}<\textcolor{keyword}{typename} MetricType,}
\DoxyCodeLine{         \textcolor{keyword}{typename} StatisticType,}
\DoxyCodeLine{         \textcolor{keyword}{typename} MatType,}
\DoxyCodeLine{         \textcolor{keyword}{typename} BoundType,}
\DoxyCodeLine{         \textcolor{keyword}{typename} SplitType>}
\DoxyCodeLine{\textcolor{keyword}{class }BinarySpaceTree;}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// The 'using' keyword gives us a template typedef, so we can define the}}
\DoxyCodeLine{\textcolor{comment}{// MeanSplitKDTree template class, which has three parameters and is a valid}}
\DoxyCodeLine{\textcolor{comment}{// TreeType policy class.}}
\DoxyCodeLine{\textcolor{keyword}{template}<\textcolor{keyword}{typename} MetricType, \textcolor{keyword}{typename} StatisticType, \textcolor{keyword}{typename} MatType>}
\DoxyCodeLine{\textcolor{keyword}{using} MeanSplitKDTree = BinarySpaceTree<MetricType,}
\DoxyCodeLine{                                        StatisticType,}
\DoxyCodeLine{                                        MatType,}
\DoxyCodeLine{                                        HRectBound<MetricType>}
\DoxyCodeLine{                                        MeanSplit<BoundType, MetricType>>;}

\end{DoxyCode}


Now, the {\ttfamily Mean\+Split\+KDTree} class has only three template parameters and can be used as a {\ttfamily Tree\+Type} policy class in various mlpack algorithms. Many types of trees in mlpack have more than three template parameters and rely on template typedefs to provide simplified {\ttfamily Tree\+Type} interfaces.\doxysection{The Tree\+Type API}\label{trees_treetype_api}
As a result of the definition of {\itshape space tree} in the previous section, a simplified API presents itself quite easily. However, more complex functionality is often necessary in mlpack, so this leads to more functions being necessary for a class to satisfy the {\ttfamily Tree\+Type} policy. Combining this with the template parameters required for trees given in the previous section gives us the complete API required for a class implementing the {\ttfamily Tree\+Type} policy. Below is the minimal set of functions required with minor documentation for each function. (More extensive documentation and explanation is given afterwards.)


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// The three template parameters will be supplied by the user, and are detailed}}
\DoxyCodeLine{\textcolor{comment}{// in the previous section.}}
\DoxyCodeLine{\textcolor{keyword}{template}<\textcolor{keyword}{typename} MetricType,}
\DoxyCodeLine{         \textcolor{keyword}{typename} StatisticType,}
\DoxyCodeLine{         \textcolor{keyword}{typename} MatType>}
\DoxyCodeLine{\textcolor{keyword}{class }ExampleTree}
\DoxyCodeLine{\{}
\DoxyCodeLine{ \textcolor{keyword}{public}:}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// This batch constructor does not modify the dataset, and builds the entire}}
\DoxyCodeLine{  \textcolor{comment}{// tree using a default-\/constructed MetricType.}}
\DoxyCodeLine{  ExampleTree(\textcolor{keyword}{const} MatType\& data);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// This batch constructor does not modify the dataset, and builds the entire}}
\DoxyCodeLine{  \textcolor{comment}{// tree using the given MetricType.}}
\DoxyCodeLine{  ExampleTree(\textcolor{keyword}{const} MatType\& data, MetricType\& metric);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Initialize the tree from a given cereal archive.  SFINAE (the}}
\DoxyCodeLine{  \textcolor{comment}{// second argument) is necessary to ensure that the archive is loading, not}}
\DoxyCodeLine{  \textcolor{comment}{// saving.}}
\DoxyCodeLine{  \textcolor{keyword}{template}<\textcolor{keyword}{typename} Archive>}
\DoxyCodeLine{  ExampleTree(}
\DoxyCodeLine{      Archive\& ar,}
\DoxyCodeLine{      \textcolor{keyword}{const} \textcolor{keyword}{typename} boost::enable\_if<typename Archive::is\_loading>::type* = 0);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Release any resources held by the tree.}}
\DoxyCodeLine{  \string~ExampleTree();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// ///////////////////////// //}}
\DoxyCodeLine{  \textcolor{comment}{// // Basic functionality // //}}
\DoxyCodeLine{  \textcolor{comment}{// ///////////////////////// //}}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Get the dataset that the tree is built on.}}
\DoxyCodeLine{  \textcolor{keyword}{const} MatType\& Dataset();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Get the metric that the tree is built with.}}
\DoxyCodeLine{  MetricType\& Metric();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Get/modify the StatisticType for this node.}}
\DoxyCodeLine{  StatisticType\& Stat();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return the parent of the node, or NULL if this is the root.}}
\DoxyCodeLine{  ExampleTree* Parent();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return the number of children held by the node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{size\_t} NumChildren();}
\DoxyCodeLine{  \textcolor{comment}{// Return the i'th child held by the node.}}
\DoxyCodeLine{  ExampleTree\& Child(\textcolor{keyword}{const} \textcolor{keywordtype}{size\_t} i);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return the number of points held in the node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{size\_t} NumPoints();}
\DoxyCodeLine{  \textcolor{comment}{// Return the index of the i'th point held in the node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{size\_t} Point(\textcolor{keyword}{const} \textcolor{keywordtype}{size\_t} i);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return the number of descendant nodes of this node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{size\_t} NumDescendantNodes();}
\DoxyCodeLine{  \textcolor{comment}{// Return the i'th descendant node of this node.}}
\DoxyCodeLine{  ExampleTree\& DescendantNode(\textcolor{keyword}{const} \textcolor{keywordtype}{size\_t} i);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return the number of descendant points of this node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{size\_t} NumDescendants();}
\DoxyCodeLine{  \textcolor{comment}{// Return the index of the i'th descendant point of this node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{size\_t} Descendant(\textcolor{keyword}{const} \textcolor{keywordtype}{size\_t} i);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Store the center of the bounding region of the node in the given vector.}}
\DoxyCodeLine{  \textcolor{keywordtype}{void} Center(arma::vec\& center);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// ///////////////////////////////////////////////// //}}
\DoxyCodeLine{  \textcolor{comment}{// // More complex distance-\/related functionality // //}}
\DoxyCodeLine{  \textcolor{comment}{// ///////////////////////////////////////////////// //}}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return the distance between the center of this node and the center of}}
\DoxyCodeLine{  \textcolor{comment}{// its parent.}}
\DoxyCodeLine{  \textcolor{keywordtype}{double} ParentDistance();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return an upper bound on the furthest possible distance between the}}
\DoxyCodeLine{  \textcolor{comment}{// center of the node and any point held in the node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{double} FurthestPointDistance();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return an upper bound on the furthest possible distance between the}}
\DoxyCodeLine{  \textcolor{comment}{// center of the node and any descendant point of the node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{double} FurthestDescendantDistance();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return a lower bound on the minimum distance between the center and any}}
\DoxyCodeLine{  \textcolor{comment}{// edge of the node's bounding shape.}}
\DoxyCodeLine{  \textcolor{keywordtype}{double} MinimumBoundDistance();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return a lower bound on the minimum distance between the given point and}}
\DoxyCodeLine{  \textcolor{comment}{// the node.}}
\DoxyCodeLine{  \textcolor{keyword}{template}<\textcolor{keyword}{typename} VecType>}
\DoxyCodeLine{  \textcolor{keywordtype}{double} MinDistance(VecType\& point);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return a lower bound on the minimum distance between the given node and}}
\DoxyCodeLine{  \textcolor{comment}{// this node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{double} MinDistance(ExampleTree\& otherNode);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return an upper bound on the maximum distance between the given point and}}
\DoxyCodeLine{  \textcolor{comment}{// the node.}}
\DoxyCodeLine{  \textcolor{keyword}{template}<\textcolor{keyword}{typename} VecType>}
\DoxyCodeLine{  \textcolor{keywordtype}{double} MaxDistance(VecType\& point);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return an upper bound on the maximum distance between the given node and}}
\DoxyCodeLine{  \textcolor{comment}{// this node.}}
\DoxyCodeLine{  \textcolor{keywordtype}{double} MaxDistance(ExampleTree\& otherNode);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return the combined results of MinDistance() and MaxDistance().}}
\DoxyCodeLine{  \textcolor{keyword}{template}<\textcolor{keyword}{typename} VecType>}
\DoxyCodeLine{  math::Range RangeDistance(VecType\& point);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return the combined results of MinDistance() and MaxDistance().}}
\DoxyCodeLine{  math::Range RangeDistance(ExampleTree\& otherNode);}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// //////////////////////////////////// //}}
\DoxyCodeLine{  \textcolor{comment}{// // Serialization (loading/saving) // //}}
\DoxyCodeLine{  \textcolor{comment}{// //////////////////////////////////// //}}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Return a string representation of the tree.}}
\DoxyCodeLine{  std::string ToString() \textcolor{keyword}{const};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Serialize the tree (load from the given archive / save to the given}}
\DoxyCodeLine{  \textcolor{comment}{// archive, depending on its type).}}
\DoxyCodeLine{  \textcolor{keyword}{template}<\textcolor{keyword}{typename} Archive>}
...
\DoxyCodeLine{  \textcolor{comment}{// A default constructor; only meant to be used by cereal.  This}}
\DoxyCodeLine{  \textcolor{comment}{// must be protected so that cereal will work; it does not need}}
\DoxyCodeLine{  \textcolor{comment}{// to return a valid tree.}}
\DoxyCodeLine{  ExampleTree();}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Friend access must be given for the default constructor.}}
...
Now, we can consider each part of the API more rigorously.\doxysection{Rigorous API documentation}\label{trees_treetype_rigorous}
...
\item \doxyref{Complex tree functionality and bounds}{p.}{trees_treetype_rigorous_complex}
\item \doxyref{Serialization}{p.}{trees_treetype_rigorous_serialization}
\end{DoxyItemize}\doxysubsection{Template parameters}\label{trees_treetype_rigorous_template}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// This function is required by the MetricType policy.}}
\DoxyCodeLine{\textcolor{comment}{// Evaluate the metric between two points (which may be of different types).}}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// Default constructor required by the StatisticType policy.}}
\DoxyCodeLine{StatisticType();}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{comment}{// This constructor is required by the StatisticType policy.}}
\DoxyCodeLine{\textcolor{keyword}{template}<\textcolor{keyword}{typename} TreeType>}
\DoxyCodeLine{StatisticType(TreeType\& node);}
...
The last template parameter is the {\ttfamily Mat\+Type} parameter. This is generally {\ttfamily arma\+::mat} or {\ttfamily arma\+::sp\+\_\+mat}, but could be any Armadillo type, including matrices that hold data points of different precisions (such as {\ttfamily float} or even {\ttfamily int}). It generally suffices to write {\ttfamily Mat\+Type} assuming that {\ttfamily arma\+::mat} will be used, since the vast majority of the time this will be what is used.\doxysubsection{Constructors and destructors}\label{trees_treetype_rigorous_constructor}
The {\ttfamily Tree\+Type} API requires at least three constructors. Technically, it does not {\itshape require} a destructor, but almost certainly your tree class will be doing some memory management internally and should have one (though not always).
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{template}<\textcolor{keyword}{typename} Archive>}
\DoxyCodeLine{ExampleTree(MatType\&\& data);}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{ExampleTree exTree(std::move(dataset));}

\end{DoxyCode}


It is, of course, possible to add even more constructors if desired.\doxysubsection{Basic tree functionality}\label{trees_treetype_rigorous_basic}
The basic functionality of a class implementing the {\ttfamily Tree\+Type} API is quite straightforward and intuitive.
...
Details on each bounding function that the {\ttfamily Tree\+Type} API requires are given below.
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{comment}{// We assume that we have a vector 'vec', and a tree node 'node'.}}
\DoxyCodeLine{\textcolor{keywordtype}{double} trueMinDist = DBL\_MAX;}
\DoxyCodeLine{\textcolor{keywordflow}{for} (\textcolor{keywordtype}{size\_t} i = 0; i < node.NumDescendants(); ++i)}
\DoxyCodeLine{\{}
\DoxyCodeLine{  \textcolor{keyword}{const} \textcolor{keywordtype}{double} dist = node.Metric().Evaluate(vec,}
\DoxyCodeLine{      node.Dataset().col(node.Descendant(i)));}
\DoxyCodeLine{  \textcolor{keywordflow}{if} (dist < trueMinDist)}
\DoxyCodeLine{    trueMinDist = dist;}
\DoxyCodeLine{\}}
\DoxyCodeLine{\textcolor{comment}{// At the end of the loop, trueMinDist will hold the true minimum distance}}
\DoxyCodeLine{\textcolor{comment}{// between 'vec' and any descendant point of 'node'.}}
...
The {\ttfamily Range\+Distance()} function allows a way for both bounds to be calculated at once. It is possible to implement this as a call to {\ttfamily Min\+Distance()} followed by a call to {\ttfamily Max\+Distance()}, but this may incur more metric {\ttfamily Evaluate()} calls than necessary. Often calculating both bounds at once can be more efficient and can be done with fewer {\ttfamily Evaluate()} calls than calling both {\ttfamily Min\+Distance()} and {\ttfamily Max\+Distance()}.\doxysubsection{Serialization}\label{trees_treetype_rigorous_serialization}
The last two public functions that the {\ttfamily Tree\+Type} API requires are related to serialization and printing.
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{template}<\textcolor{keyword}{typename} TreeType>}
\DoxyCodeLine{\textcolor{keyword}{class }TreeTraits}
\DoxyCodeLine{\{}
\DoxyCodeLine{ \textcolor{keyword}{public}:}
\DoxyCodeLine{  \textcolor{comment}{// This is true if the subspaces represented by the children of a node can}}
\DoxyCodeLine{  \textcolor{comment}{// overlap.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} HasOverlappingChildren = \textcolor{keyword}{true};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// This is true if Point(0) is the centroid of the node.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} FirstPointIsCentroid = \textcolor{keyword}{false};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// This is true if the points contained in the first child of a node}}
\DoxyCodeLine{  \textcolor{comment}{// (Child(0)) are also contained in that node.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} HasSelfChildren = \textcolor{keyword}{false};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// This is true if the tree rearranges points in the dataset when it is built.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} RearrangesDataset = \textcolor{keyword}{false};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// This is true if the tree always has only two children.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} BinaryTree = \textcolor{keyword}{false};}
\DoxyCodeLine{\};}

\end{DoxyCode}


An example specialization for the \doxyref{mlpack\+::tree\+::\+KDTree}{p.}{namespacemlpack_1_1tree_a73c2146f8d1da65d927c7746bfe7e750} class is given below. Note that \doxyref{mlpack\+::tree\+::\+KDTree}{p.}{namespacemlpack_1_1tree_a73c2146f8d1da65d927c7746bfe7e750} is itself a template class (like every class satisfying the {\ttfamily Tree\+Type} policy), so we are specializing to a template parameter.
...
\DoxyCodeLine{\textcolor{keyword}{class }TreeTraits<KDTree<MetricType, StatisticType, MatType>>}
\DoxyCodeLine{\{}
\DoxyCodeLine{ \textcolor{keyword}{public}:}
\DoxyCodeLine{  \textcolor{comment}{// The regions represented by the two children of a node may not overlap.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} HasOverlappingChildren = \textcolor{keyword}{false};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// There is no guarantee that the first point of a node is the centroid.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} FirstPointIsCentroid = \textcolor{keyword}{false};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Points are not contained at multiple levels (only at the leaves).}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} HasSelfChildren = \textcolor{keyword}{false};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// Points are rearranged during the building of the tree.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} RearrangesDataset = \textcolor{keyword}{true};}
\DoxyCodeLine{}
\DoxyCodeLine{  \textcolor{comment}{// The tree is always binary.}}
\DoxyCodeLine{  \textcolor{keyword}{static} \textcolor{keyword}{const} \textcolor{keywordtype}{bool} BinaryTree = \textcolor{keyword}{true};}
\DoxyCodeLine{\};}

\end{DoxyCode}


Currently, the traits available are each of the five detailed above. For more information, see the \doxyref{mlpack\+::tree\+::\+Tree\+Traits}{p.}{classmlpack_1_1tree_1_1TreeTraits} documentation.\doxysection{A list of trees in mlpack and more information}\label{trees_treetype_more}
mlpack contains several ready-\/to-\/use implementations of trees that satisfy the Tree\+Type policy API\+:


\begin{DoxyItemize}
\item \doxyref{mlpack\+::tree\+::\+KDTree}{p.}{namespacemlpack_1_1tree_a73c2146f8d1da65d927c7746bfe7e750}
\item \doxyref{mlpack\+::tree\+::\+Mean\+Split\+KDTree}{p.}{namespacemlpack_1_1tree_a1028e6acf1fc61997237d3677cae0947}
...
\item \doxyref{mlpack\+::tree\+::\+RTree}{p.}{namespacemlpack_1_1tree_ae4af35641769744ba680cc934e1c1f0e}
\item \doxyref{mlpack\+::tree\+::\+RStar\+Tree}{p.}{namespacemlpack_1_1tree_a879db9c5c88d62f13f4a1667bc5adf5c}
...
\begin{DoxyItemize}
\item \doxyref{mlpack\+::tree\+::\+Binary\+Space\+Tree}{p.}{classmlpack_1_1tree_1_1BinarySpaceTree} -- binary trees, such as the KD-\/tree and ball tree
