...
macro(get_deps LINK DEPS_NAME PACKAGE)
  if (NOT EXISTS "${MLPACK_BINARY_DIR}/deps/${PACKAGE}")
    file(DOWNLOAD ${LINK}
           "${MLPACK_BINARY_DIR}/deps/${PACKAGE}"
    ...
    if (DOWNLOAD_STATUS EQUAL 0)
      execute_process(COMMAND ${CMAKE_COMMAND} -E
          tar xf "${MLPACK_BINARY_DIR}/deps/${PACKAGE}"
          WORKING_DIRECTORY "${MLPACK_BINARY_DIR}/deps/")
    else ()
  ...
  file (GLOB DIRECTORIES RELATIVE "${MLPACK_BINARY_DIR}/deps/"
      "${MLPACK_BINARY_DIR}/deps/${DEPS_NAME}*.*")
  ...
  if (DIRECTORIES_LEN GREATER 0)
    ...
    set(GENERIC_INCLUDE_DIR "${MLPACK_BINARY_DIR}/deps/${DEPENDENCY_DIR}/include")
    install(DIRECTORY "${GENERIC_INCLUDE_DIR}/" DESTINATION "${CMAKE_INSTALL_INCLUDEDIR}")
  else ()
    message(FATAL_ERROR
            "Problem unpacking ${DEPS_NAME}! Expected only one directory ${DEPS_NAME};. Try to remove the directory ${MLPACK_BINARY_DIR}/deps and reconfigure.")
  endif ()
endmacro()
