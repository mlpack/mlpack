function [newx]=sdpca_min_sum_max_eig_trad(x, knn, r)
%This function computes the  minimum sum of maximum eigenvalues
...
Att=[sparse(knn*n+1,1) sparse(knn*n+1,n^2) At sparse(knn*n+1,n^2) ;...
     -vec(speye(n)) -AA  AA AA];
 
Att=sparse(Att);
b(end)=0;
b = [b ; -sparse(n^2, 1)];
c=[r vec(speye(n))' sparse(1,n^2) sparse(1,n^2)];
c=sparse(c);
K.l=1;
K.s=[n n n];
pars.maxiter=60;
