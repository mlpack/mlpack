...
class SVM {
 ...
 public:
  ...
  double BatchPredict(int learner_typeid, Dataset& testset);
  void BatchPredict(int learner_typeid, Dataset& testset, String);
...
void SVM<TKernel>::Init(int learner_typeid, const Dataset& dataset, datanode *module){
  ...
  //printf("C=%f,C_p=%f,C_n=%f...\n",param_.C_,param_.Cp_,param_.Cn_);
...
void SVM<TKernel>::InitTrain(int learner_typeid, const Dataset& dataset, datanode *module) {
  ...
  if (learner_typeid == 0) { // Multiclass SVM Clssification
    ...
    //    printf("Solving a classification problem...\n");
...
void SVM<TKernel>::SVM_R_Train_(int learner_typeid, const Dataset& dataset, datanode *module) {
  ...
  if (opt_method_== "smo") {
    ...
    //    printf("The kernel to be used is %s..\n",kernel);
...
/* This function won't output to a file
 */
template<typename TKernel>
double SVM<TKernel>::BatchPredict(int learner_typeid, Dataset& testset) {
  ...
  return double(err_ct)/double(testset.n_points());
}
