...
template <typename MyTKernel> class NaiveKdeLP{
 ...
 double get_plugin_bandwidth(){
   
   return plugin_bandwidth_;
 }
 ...
 void Compute(){
   ...
   for(index_t q=0;q<qset_.n_cols();q++){ //for each query point
     ...
     density_val/=(my_kernel_.EvalNormConstant()*number_of_contributions);
 ...
 void InitBandwidth(Vector &bwidth){
   ...
   //bandwidth_ has already been initialized, hence just copy the
   //values
 ...
 void InitBandwidth(double *bandwidth){
   ...
   my_kernel_.InitInitialized(bandwidth);

   //reset densities for fresh calc
   densities_.SetZero();
