...
/**
 * This function resets a lot of the query stats in 
 * the tree by recursively traversing the whole tree
 */
void ApproxNN::reset_tree_(TreeType *tree) {

  DEBUG_ASSERT(tree != NULL);

  tree->stat().set_max_distance_so_far(DBL_MAX);
...
void ApproxNN::ComputeBaseCase_(TreeType* query_node,
  ...
  if (!fx_param_bool(module_, "dual_tree", true))
  ...
  for (index_t query_index = query_node->begin(); 
    ...
    if (fx_param_bool(module_, "dual_tree", true)) {
      nn_dc_[query_index] += reference_node->end() - reference_node->begin();
    } else {
...
void ApproxNN::ComputeApproxBaseCase_(TreeType* query_node,
  ...
  for (index_t query_index = query_node->begin(); 
    ...
    if (fx_param_bool(module_, "dual_tree", true)) {
      nn_dc_[query_index] += sample_size;
    } else {
      nn_dc_[query_] += sample_size;
    }
...
void ApproxNN::InitApprox(const Matrix& queries_in,
  ...
  // Making the trees 
  // using the single tree algorithm for the queries.
  if (fx_param_bool(module_, "dual_tree", true)) {
    query_tree_ 
      = tree::MakeKdTreeMidpoint<TreeType>(queries_,
					   leaf_size_, 
    ...
    NOTIFY("Doing single tree so making one point trees.\n");
    for (index_t i = 0; i < queries_.n_cols(); i++) {
      Matrix query;
      queries_.MakeColumnSlice(i, 1, &query);
      TreeType *single_point_tree
	= tree::MakeKdTreeMidpoint<TreeType>(query,
      ...
      DEBUG_ASSERT(single_point_tree->end() - single_point_tree->begin()
		   == 1);
      query_trees_.push_back(single_point_tree);
      old_from_new_queries_.Renew();
    }
  ...
  // resetting the number of distance computations
...
// Cleaning up the stats in the query tree and getting it
// ready to work again for another computation task.
void ApproxNN::QuickCleanUp() {
  ...
  // resetting the number of prunes made
  number_of_prunes_ = 0;


  // resetting the query stats for the query tree!!
  if (fx_param_bool(module_, "dual_tree", true)) {
    ...
    // fix for each tree individually
    for (std::vector<TreeType*>::iterator query_tree = query_trees_.begin();
	 query_tree < query_trees_.end(); ++query_tree)
      reset_tree_(*query_tree);
  }
...
void ApproxNN::ComputeApprox(ArrayList<index_t>* resulting_neighbors,
  ...
  if (fx_param_bool(module_, "dual_tree", true)) {
    ...
    // NOTIFY("Doing single tree individual queries.\n");
    ...
    for (std::vector<TreeType*>::iterator query_tree = query_trees_.begin();
	 query_tree < query_trees_.end(); ++query_tree, ++query_) {
      ...
      DEBUG_ASSERT((*query_tree)->end() - (*query_tree)->begin() == 1);
      ComputeApproxRecursion_(*query_tree, reference_tree_, 
			      MinNodeDistSq_(*query_tree,
					     reference_tree_));
      DEBUG_ASSERT_MSG((*query_tree)->stat().total_points()
		       == references_.n_cols(),"N':%"LI"d, N:%"LI"d",
		       (*query_tree)->stat().total_points(),
		       references_.n_cols());
      DEBUG_ASSERT_MSG((*query_tree)->stat().samples()
		       >= min_samples_per_q_,"n':%"LI"d, n:%"LI"d",
		       (*query_tree)->stat().samples(),
		       min_samples_per_q_);
    }

    // NOTIFY("Done with single tree queries.");
  }

  // NOTIFY("Number of prunes: %"LI"d", number_of_prunes_);
  ...
  if (fx_param_bool(module_, "dual_tree", true)) {
    ...
    for (index_t i = 0; i < neighbor_indices_.size(); i++) {
      (*resulting_neighbors)[i]
	= old_from_new_references_[neighbor_indices_[i]];
      (*distances)[i] = neighbor_distances_[i];
    }
  }

  index_t sum = 0;
  for (index_t i = 0; i < nn_dc_.size(); i++)
    sum += nn_dc_[i];

  //NOTIFY("Avg. NN DC: %lg", (double) sum / (double) nn_dc_.size());
  printf("%lg,", (double) sum / (double) nn_dc_.size());
} // ComputeApprox
