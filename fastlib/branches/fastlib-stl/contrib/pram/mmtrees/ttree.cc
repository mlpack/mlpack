...
bool TTree::FindSplit_(Matrix& x, Vector& y, index_t total_n,
  ...
  index_t n_t = x.n_cols();
  double min_error = error_;
  bool some_split_found = false;
  index_t point_mass_in_dim = 0;
  index_t min_leaf_size = fx_param_int(module_, "min_leaf_size", 1),
...
//       // printf("range: %Lg\n", range); fflush(NULL);
	...
	y_sorted_x[i] = dim_val_vec[i].second;
      ...
      DEBUG_ASSERT((index_t) dim_val_vec.size() > max_leaf_size);
      ...
      index_t left_child_size = min_leaf_size; // , right_child_size;
...
//   *split_val = (*lsplit_val + *rsplit_val) / 2 ;
  ...
  ArrayList<index_t> temp_swap_list;
