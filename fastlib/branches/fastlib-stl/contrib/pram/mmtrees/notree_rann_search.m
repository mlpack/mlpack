function [qRes, ann_accuracy, time] = notree_rann_search(Q, R,...
						  tau, alpha, rank_mat)
...
display(sprintf('Tau: %d, Alpha: %f, n:%d, N:%d', tau, alpha,...
		samples_reqd, nRs));
...
%while error ~= 0
  ...
  for i = 1:nQs
    ...
    tic;
    q = sample_search(q, R, samples_reqd);
    q.nn_time = toc;
    time = time + toc;
  ...
  %  comp_stats = [qResTemp(:).nn_time;...
    ...
    % display(sprintf('%d: E:%d/%d, AEps:%f, MEps:%f', ...
    %	    numLeaf, error, nQs, avg_eps, max_eps));
    display(sprintf('Avg. NN time:%f, Avg. NN DC:%f',...
		    avg_stats(1), avg_stats(2)));
    display(sprintf('Avg. RE:%f, Max. RE:%f, Error Prob.:%f',...
		    avg_rank_error/nQs, max_error, error_prob/ ...
		    nQs));
    
    %display(sprintf('Avg. points seen:%f, Avg. Samples made:%f',...
	%	    avg_sample_stats(1), avg_sample_stats(2)));
    %display(sprintf('Avg. from leaf:%f, Avg. from int node:%f',...
	%	    avg_sample_stats(3), avg_sample_stats(4)));
    %display(sprintf('Avg. leaves:%f, Avg. summed:%f, Avg. almost:%f',...
...
function [q] = sample_search(q, R, nSamples)

[nDims, nPoints] = size(R);

sampleIndices = randi([1 nPoints], nSamples, 1);
leafData = R(:, sampleIndices);
