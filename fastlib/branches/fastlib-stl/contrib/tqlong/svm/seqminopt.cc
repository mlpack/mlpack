...
void maxIup(const Vector& y, const Vector& grad, const IndexSet& Iup, 
	    size_t& ii, double& val1);
void minIdown(const Vector& y, const Vector& grad, const IndexSet& Idown, 
	    size_t& jj, double& val2);
size_t CheckKKT(const Vector& y, const Vector& box, const Vector& grad, 
		 const Vector& alpha, double offset, double tolKKT);
void UpdateAlphas(size_t i, size_t j, Kernel& kernel,
...
int seqminopt(const Matrix& X, const Vector& y, const Vector& box,
  ...
  for (size_t iter = 0; /*iter < 20000*/; iter++) {
    ...
    if (iter % 500 == 0) {
      ...
      size_t count = CheckKKT(y, box, grad, alpha, offset, options.tolKKT);
...
void UpdateAlphas(size_t i, size_t j, Kernel& kernel,
  ...
  for (size_t k = 0; k < grad.length(); k++) {
...
void minIdown(const Vector& y, const Vector& grad, const IndexSet& Idown, 
	    size_t& jj, double& val2) {
  double min = INFINITY;
  for (size_t i = 0; i < Idown.get_n(); i++) {
...
bool vecgeq(const Vector& x, double tol) {
  ...
  void SplitDataToXy(const Matrix& data, Matrix& X, Vector& y) {
    size_t n_points = data.n_cols();
    size_t d = data.n_rows()-1;
    y.Init(n_points);
    X.Init(d, n_points);
    for (size_t i = 0; i < n_points; i++) {
      for (size_t j = 0; j < d; j++) X.ref(j, i) = data.get(j, i);
      y[i] = data.get(d, i);
    }
  }
  ...
  void ScaleData(Matrix& X, Vector& shift, Vector& scale) {
    ...
    for (size_t i = 0; i < d; i++) {
      ...
      for (size_t j = 0; j < n; j++) sumX += X.get(i, j);
      shift[i] = -sumX/n;
      for (size_t j = 0; j < n; j++) X.ref(i, j) += shift[i];
      for (size_t j = 0; j < n; j++) sumX2 += X.get(i, j)*X.get(i, j);
      scale[i] = (sumX2 > 1e-16)?1.0/(sqrt(sumX2/(n-1))) : 1.0;
      for (size_t j = 0; j < n; j++) X.ref(i, j) *= scale[i];
    }
  }

  void SetBoxConstraint(const Vector& y, double C, Vector& box) {
    size_t n_points = y.length(), n1 = 0;
    box.Init(n_points);
    for (size_t i = 0; i < n_points; i++)
      if (y[i] == 1) n1++;
    for (size_t i = 0; i < n_points; i++) 
  ...
  double svm_output_on_sample(size_t i, Kernel& kernel, 
    ...
    double s = offset;
    for (size_t j = 0; j < SVindex.get_n(); j++) {
      size_t jj = SVindex[j];
      //printf(" jj=%d\n", jj);
