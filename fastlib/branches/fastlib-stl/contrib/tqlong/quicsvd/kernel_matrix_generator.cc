...
void gen_gaussian_kernel(const Matrix& ref, Matrix* kernel_matrix,
  ...
  for(size_t r = 0; r < ref.n_cols(); r+=rate) {
    const double *r_col = ref.GetColumnPtr(r);
    
    for(size_t q = 0; q < ref.n_cols(); q+=rate) {
...
#define RUNNING 1
  ...
  kernel_matrix.Init(n_cols, n_cols);
  for(size_t r = 0; r < references.n_cols(); r+=rate) {
    const double *r_col = references.GetColumnPtr(r);

    for(size_t q = 0; q < references.n_cols(); q+=rate) {
  ...
  */
  ...
  for(size_t r = 0; r < kernel_matrix.n_rows(); r++) {
    for(size_t c = 0; c < kernel_matrix.n_cols(); c++) {
