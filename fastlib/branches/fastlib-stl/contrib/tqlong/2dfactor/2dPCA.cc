...
namespace la {
  ...
  void Get2dBasisMajor(double rowPart, const Vector& rowEigenValues, 
    ...
    for (size_t col = 0; col < rowEigenValues.length(); col++)
    ...
    for (size_t row = 0; row < colEigenValues.length(); row++)
  ...
  void Project2dBasis(const Matrix& A, const Matrix& rowBasis, 
    ...
    size_t n_rows = colBasis.n_cols();
    size_t n_cols = rowBasis.n_cols();
	...
	for (size_t A_row = 0; A_row < A.n_rows(); A_row++)
	  for (size_t A_col = 0; A_col < A.n_cols(); A_col++)
  ...
  void computeRowCovariance(const ArrayList<Matrix>& imageList, 
			    const Matrix& mean, 
			    Matrix& rowCovariance_out) {
    size_t n_matrix = imageList.size();
    DEBUG_ASSERT(n_matrix > 0);
    size_t n_rows = imageList[0].n_rows();
    size_t n_cols = imageList[0].n_cols();

    rowCovariance_out.Init(n_cols, n_cols);
    rowCovariance_out.SetZero();

    for (size_t i_matrix = 0; i_matrix < n_matrix; i_matrix++) {
      DEBUG_ASSERT(imageList[i_matrix].n_rows() == n_rows);
      DEBUG_ASSERT(imageList[i_matrix].n_cols() == n_cols);
      for (size_t i_row = 0; i_row < n_rows; i_row++) 
	for (size_t i_col1 = 0; i_col1 < n_cols; i_col1++)
	  for (size_t i_col2 = 0; i_col2 < n_cols; i_col2++)     
  ...
  void computeColCovariance(const ArrayList<Matrix>& imageList, 
    ...
    for (size_t i_matrix = 0; i_matrix < n_matrix; i_matrix++) {
      ...
      DEBUG_ASSERT(imageList[i_matrix].n_cols() == n_cols);
      for (size_t i_col = 0; i_col < n_cols; i_col++)
	for (size_t i_row1 = 0; i_row1 < n_rows; i_row1++) 
	  for (size_t i_row2 = 0; i_row2 < n_rows; i_row2++) 
