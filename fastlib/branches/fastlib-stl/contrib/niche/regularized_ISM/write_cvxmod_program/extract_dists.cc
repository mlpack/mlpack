...
int main(int argc, char* argv[]) {
  ...
  if (data::Load(data_file.c_str(), &data)==false) {
  ...
  if (data::Load(labels_file.c_str(), &labels_mat)==false) {
  ...
  for(size_t i = 1; i < labels_mat.n_cols(); i++) {
  ...
  NOTIFY("Tree(s) built");
  size_t knns = fx_param_int_req(module, "knns");
  NOTIFY("Computing %zud nearest neighbors", knns);
  ...
  size_t n_constraints = n_points * knns;
