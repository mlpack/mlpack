...
/*
  ...
  for(size_t i=0; i<nearest_distances_.size(); i++) {
    size_t p1=nearest_neighbor_pairs_[i].first;
    size_t p2=nearest_neighbor_pairs_[i].second;
...
void GeometricNmf::ComputeGradient(Matrix &coordinates, 
  ...
  for(size_t i=0; i<values_.size(); i++) {
    size_t w_i=rows_[i];
    size_t h_i=columns_[i]+offset_h_;
    double v=values_[i];
    double new_v=0;
    for(size_t j=0; j<new_dim_; j++) {
  ...
  if (lower_bound_!=NULL) {
    for(size_t i=0; i<coordinates.n_rows(); i++) {
      for(size_t j=0; j<coordinates.n_cols(); j++) {
...
/*
  ...
  for(size_t i=0; i<nearest_neighbor_pairs_.size(); i++) {
    size_t w_1=nearest_neighbor_pairs_[i].first;
    size_t w_2=nearest_neighbor_pairs_[i].second;
