...
void SetParameters(NonConvexMVU &engine) {
  size_t knns = fx_param_int(NULL, "knns", 4);
  engine.set_knns(knns);
  size_t kfns = fx_param_int(NULL, "kfns", 0);
  ...
  size_t max_iterations=fx_param_int(NULL, "max_iterations", 10000);
  engine.set_max_iterations(max_iterations);
  size_t new_dimension=fx_param_int(NULL, "new_dimension", 3);
  ...
  size_t mem_bfgs = fx_param_int(NULL, "mem_bfgs", 50);
...
void LoadData(std::string data_file, 
  ...
  if (sample_factor==1.0) {
    ...
    size_t num_of_points = temp.n_cols();
    size_t reduced_num_of_points = size_t(num_of_points *sample_factor);
    size_t *permutations = new size_t[num_of_points];
    for(size_t i=0; i<num_of_points; i++) {
      permutations[i]=i;
    }
    std::random_shuffle(permutations, permutations+num_of_points);
    data->Init(temp.n_rows(), reduced_num_of_points);
    for(size_t i=0; i<reduced_num_of_points; i++) {
