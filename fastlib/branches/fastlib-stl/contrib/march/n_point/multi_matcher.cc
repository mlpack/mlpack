...
bool npt::MultiMatcher::TestPointPair(double dist_sq, index_t new_ind, index_t old_ind,
  ...
  for (index_t perm_ind = 0; perm_ind < num_permutations_; perm_ind++) {
    ...
    if (lo == matcher_dists_[matcher_ind].end()) {
      ...
      double high_dist = matcher_dists_[matcher_ind][low_ind] - dist_sq;
      double low_dist = dist_sq - matcher_dists_[matcher_ind][low_ind - 1];
    ...
    bool this_matches = (dist_sq <= (closest_matcher + half_band_)) &&
                        (dist_sq >= (closest_matcher - half_band_));
