...
bool AugLagrangian<LagrangianFunction>::Optimize(int num_iterations,
  ...
  for (it = 0; it != (num_iterations - 1); it++) {
    ...
    if(!lbfgs.Optimize(0, coordinates)) {
      NOTIFY("L-BFGS reported an error during optimization.");
    }
...
//    NOTIFY("Penalty is %lf (threshold %lf)", penalty, penalty_threshold);
...
//      NOTIFY("Updated lambda to ");
//      std::cout << lambda;
//      NOTIFY("Updated penalty threshold to %lf", penalty_threshold);
...
//      NOTIFY("Updated sigma to %lf", sigma);
    }
  }

//  NOTIFY("Terminating after %d iterations", it);
  return false;
}
