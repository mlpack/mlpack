 ...
 public:
  ...
  void Init(const GenMatrix<T>& queries, 
    ...
    if (flag == NULL) {
      ...
      query_tree_ = ctree::MakeCoverTree<TreeType, T>(queries_, flag);
      reference_tree_ = ctree::MakeCoverTree<TreeType, T>(references_, flag);
    }

    //fx_timer_stop(module_, "tree_building");

    //NOTIFY("Normal Query Tree:");
    // ctree::PrintTree<TreeType>(query_tree_);
    //NOTIFY("Normal Reference Tree:");
  ...
  void MakeCentroidRootTree(GenMatrix<T> data, 
    ...
    *root_node_index = neighbor_index[0];
    ...
    if (flag != NULL) {
      (*cdata_tree) = ctree::MakeCoverTree<TreeType, T>(data, flag);
    }
    else {
  ...
  void MakeCCoverTrees(index_t *flag = NULL) {
    ...
    if (flag != NULL) {
      MakeCentroidRootTree(references_, reference_tree_, 
			   &ref_root_node_index_, &creference_tree_, 
			   flag);
      //NOTIFY("Special Centroid Tree Reference Tree:");
      //ctree::PrintTree<TreeType>(creference_tree_);
    }
    else {
      MakeCentroidRootTree(references_, reference_tree_, 
			   &ref_root_node_index_, &creference_tree_);
      //NOTIFY("Centroid Tree Reference Tree:");
      //ctree::PrintTree<TreeType>(creference_tree_);
    }
    if (flag != NULL) {
      MakeCentroidRootTree(queries_, query_tree_, 
			   &query_root_node_index_, &cquery_tree_,
			   flag);
      //NOTIFY("Special Centroid Tree Query Tree:");
      //ctree::PrintTree<TreeType>(cquery_tree_);
    }
    else {
      MakeCentroidRootTree(queries_, query_tree_, 
			   &query_root_node_index_, &cquery_tree_); 
      //NOTIFY("Centroid Tree Query Tree:");
      //ctree::PrintTree<TreeType>(cquery_tree_);
    }
