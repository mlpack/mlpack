...
void MLPObjective::Init2(int count_init2) {
	...
	first_stage_dot_logit_postponed_.Init();
...
void MLPObjective::Init3(int sample_size,
	...
	first_stage_dot_logit_postponed_.Destruct();
	first_stage_dot_logit_postponed_.Init(num_selected_people);
	...
	*/
    ...
    exp_betas_times_x1_[i]=0;
		...
		first_stage_dot_logit_[i].Init(first_stage_x_[i].n_cols());
		first_stage_dot_logit_[i].SetZero();
		first_stage_ddot_logit_[i].Init(first_stage_x_[i].n_cols(),first_stage_x_[i].n_cols()+1);
		first_stage_ddot_logit_[i].SetZero();
...
void MLPObjective::ComputeObjective(double current_sample,
...
void MLPObjective::ComputeGradient(double current_sample,
																Vector &current_parameter, 
																Vector *gradient) { 
...
void MLPObjective::ComputeHessian(double current_sample,
															 Vector &current_parameter, 
															 Matrix *hessian) { 
...
void MLPObjective::ComputeChoiceProbability(Vector &current_parameter, 
  ...
  //betas.Alias(x.ptr(), x.n_rows());
	...
	betas.Alias(current_parameter.ptr(), num_of_betas_);
  Vector betas1;
  betas1.Alias(current_parameter.ptr(), num_of_betas_-1);
	choice_probability->Init(first_stage_x_.size());
  //cout<<"beta_length="<<betas.length()<<endl;
  ...
  for(index_t n=0; n<first_stage_x_.size(); n++) {
    if (first_stage_y_[n]<0) { 
			...
			(*choice_probability)[n]=betas[betas.length()-1]-log(exp_betas_times_x1_[n]+exp(betas[betas.length()-1]));
      ...
      first_stage_x_[n].MakeColumnVector((first_stage_y_[n]-1), &temp);
			...
			(*choice_probability)[n]=la::Dot(betas1, temp)-log(exp_betas_times_x1_[n]+exp(betas[betas.length()-1]));
			//cout<<"beta_length="<<betas.length()<<endl;
     }
...
/* deleted on Jan.16th
double MLPObjective::ComputeTerm1_(Vector &betas) {
  ...
  for(index_t n=0; n<first_stage_x_.size(); n++) {
    if (first_stage_y_[n]<0) { 
      ...
      //continue;
			term1+=(-1*log(exp_betas_times_x1_[n]));
    } else {
      Vector temp;
      first_stage_x_[n].MakeColumnVector((first_stage_y_[n]-1), &temp);
			//cout<<"term1"<<endl;
			...
			//else{
				...
				term1+=(la::Dot(betas1, temp) - log(exp_betas_times_x1_[n])-betas[betas.length()-1]);
			//}
    //cout<<"beta_length="<<betas.length()<<endl;
...
double MLPObjective::ComputeTerm1_(Vector &betas) {
  ...
  for(index_t n=0; n<first_stage_x_.size(); n++) {
    if (first_stage_y_[n]<0) { 
      ...
      //continue;
			//term1+=(-1*log(exp_betas_times_x1_[n]));
			term1+=(betas[betas.length()-1] - log( exp_betas_times_x1_[n]+exp(betas[betas.length()-1]) ));
    } else {
      Vector temp;
      first_stage_x_[n].MakeColumnVector((first_stage_y_[n]-1), &temp);
			//cout<<"term1"<<endl;
			...
			//else{
				//term1+=la::Dot(betas, temp) - log(exp_betas_times_x1_[n]);
				//term1+=(la::Dot(betas1, temp) - log(exp_betas_times_x1_[n])-betas[betas.length()-1]);
				term1+=(la::Dot(betas1, temp) - log( exp_betas_times_x1_[n] + exp(betas[betas.length()-1]) ));
			//}
    //cout<<"beta_length="<<betas.length()<<endl;
...
void MLPObjective::ComputeExpBetasTimesX1_(Vector &betas) {
...
void MLPObjective::ComputeDotLogit_(Vector &betas) {
	...
	*/
	...
	for(index_t n=0; n<first_stage_x_.size(); n++){
		...
		//first_stage_dot_logit_[n][first_stage_x_[n].n_cols()]=(1/exp_betas_times_x1_[n]);
	}	//n
...
void MLPObjective::ComputeDerivativeBetaTerm1_(Vector *beta_term1) {
  ...
  //cout<<"num_of_betas="<<num_of_betas_<<endl;
	...
	Vector x_np;
	x_np.Init(num_of_betas_);
	x_np.SetZero();
	x_np[num_of_betas_-1]=1;

	Vector xl_p;
	xl_p.Init(num_of_betas_);
  xl_p.SetZero();
  ...
  for(index_t n=0; n<first_stage_x_.size(); n++) {
    ...
    if (first_stage_y_[n]<0) { 
      ...
      //
			...
			//la::SubOverwrite(num_of_betas_, temp4.ptr(), x_np.ptr(), temp2.ptr());
			la::SubOverwrite(temp4, x_np, &temp2);	
			...
			//continue;
      Vector temp5;
			first_stage_x_[n].MakeColumnVector(first_stage_y_[n]-1, &temp5);
			...
			la::SubOverwrite(temp4, temp6, &temp2);
...
void MLPObjective::ComputeDerivativeBetaTerm1_(Vector *beta_term1) {
	...
	Vector temp2;
	temp2.Init(num_of_betas_-1);

	Vector temp3;
	temp3.Init(num_of_betas_-1);
	temp3.SetZero();

	double temp4=0;

	Vector temp5;
	temp5.Init(num_of_betas_);
	temp5.SetAll(0);
...
//////////////////////////////////////////////////
void MLPObjective::CheckGradient(double current_sample,
															Vector &current_parameter, 
...
void MLPObjective::CheckHessian(double current_sample,
														 Vector &current_parameter, 
...
void MLPObjective::CheckHessian3(double current_sample,
														 Vector &current_parameter, 
														 Matrix *approx_hessian){
...
void MLPObjective::CheckHessian2(double current_sample, 
									  Vector &current_parameter, 
										Matrix *approx_hessian) {
...
void MLPObjective::ComputePostponedProbability_(Vector &betas){
  ...
  for(index_t n=0; n<first_stage_x_.size(); n++) {
    if (first_stage_y_[n]<0) { 
			//(*choice_probability)[n]=log(postponed_probability_[n]);
			//(*choice_probability)[n]=0;
			postponed_probability_[n]=exp(betas[betas.length()-1])/(exp_betas_times_x1_[n]+exp(betas[betas.length()-1]));
...
void MLPObjective::ComputePredictionError(double current_sample, 
  ...
  predicted_decision.SetAll(-1.0);
	...
	for(index_t n=0; n<number_of_test; n++){
		...
		if(postponed_probability_[n]>max_choice_probability){
			predicted_decision[n]=-1;
			predicted_choice_probability[n]=postponed_probability_[n];
		}
  ...
  mtx_predicted_choice_probability.Init(number_of_test,1);
	...
	data::Save("MLP_predicted_choice_probability.csv",  mtx_predicted_choice_probability);

	cout<<"Save predicted decision..."<<endl;
	data::Save("MLP_predicted_decision.csv", mtx_predicted_decision);
