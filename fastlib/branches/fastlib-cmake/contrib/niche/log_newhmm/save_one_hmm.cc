...
-model_classes=class1 or class0
...
void GetOneHMM(bool model_class1, bool model_class0,
  ...
  hmm.Init(n_states, n_dims, GAUSSIAN, 1e-10, true);
...
void SaveKFoldHMMs(int n_folds) {
  ...
  for(int fold_num = 0; fold_num < n_folds; fold_num++) {
    ...
    ArrayList<GenMatrix<double> > selected_training_sequences;
    ...
    hmm.BaumWelch(selected_training_sequences,
		  ...
		  1000);
    printf("fold %d\n", fold_num);
    hmm.PrintDebug("hmm after calling Baum-Welch");
