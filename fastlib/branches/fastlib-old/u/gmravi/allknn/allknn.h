...
class AllNNSingleTree{
 ...
 public:
  ...
  /** This function simply loads the reference dataset 
   *  and the query dataset into matrices 
  ...
  //This is the function which will perform the actual 
  //single tree algorithm and spit the results 
  ...
  void ComputeAllNNSingleTree(){
    ...
    for(index_t q=0;q<q_matrix_.n_cols();q++){
      ...
      //This function traverses the tree and finds
      // out the nearest neighbour for each point 
   ...
   *potential_distance: The distance of the nearest neighbour 
   *estimate we have as of now. 
  ...
  void FindNearestNeighbour(Tree *rnode,double *point, Matrix r_matrix, 
	...
	//Remember we need to find neighbours within 
	//the potential_distance estimate we have
	
	for(index_t i=rnode->begin();i<rnode->end();i++)
	  {
	    double temp_dist=la::DistanceSqEuclidean(r_matrix_.n_rows(),
						     r_matrix_.GetColumnPtr(i),point);
	    ...
	    if(check_if_equal(r_matrix_.GetColumnPtr(i),point,
			      r_matrix.n_rows())==false && temp_dist < 
	       potential_distance) {
	...
	double min_distance_to_left_child=
	  rnode->left()->bound().MinDistanceSq(point);

	double min_distance_to_right_child=
	  rnode->right()->bound().MinDistanceSq(point);
	...
	if(potential_distance > nearest_bb_distance){
	  ...
	  if(potential_distance>farther->bound().MinDistanceSq(point)){
	    ...
	    FindNearestNeighbour(farther,point,r_matrix,
				 potential_distance,potential_index);	    
	  }
	}
