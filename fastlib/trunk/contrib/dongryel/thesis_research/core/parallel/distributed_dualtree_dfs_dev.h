...
void DistributedDualtreeDfs<DistributedProblemType>::AllToAllIReduce_(
  ...
  InitialSetup_(
    ...
    &hashed_essential_reference_subtrees_to_send,
    &distributed_tasks);
  *initial_setup_time = distributed_tasks.reference_tree_walk_time();
...
#pragma omp parallel
  {
    ...
    do {
      ...
      if(thread_id == 0) {
        ...
        // more messages.
        if((world_->size() == 1 &&
            distributed_tasks.num_remaining_tasks() < 10 * omp_get_num_threads()) ||
            (world_->size() > 1 &&
          ...
          distributed_tasks.WalkReferenceTree(
            metric, problem_->global(), *world_, 20 * omp_get_num_threads(),
...
void DistributedDualtreeDfs<DistributedProblemType>::Compute(
  ...
  if(weak_scaling_measuring_mode_) {
    ...
    // Re-adjust so that the subtree is transfered in around 5 rounds.
    max_subtree_size_ =
      std::max(
        std::min(
          static_cast<int>(max_num_reference_points_to_pack_per_process_) / 5,
          20000),
        leaf_size_ * 2);
