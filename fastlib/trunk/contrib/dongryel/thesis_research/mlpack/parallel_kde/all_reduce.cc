...
int Allreduce(void *sbuf, void *rbuf, int count, MPI_Op op, MPI_Comm comm) {
  ...
  int i;
  for(i = 1; i < num_processes; i *= 2) {

    // If the current process ID's bit mask does not match i, then send.
    if((rank & i) == 0) {
      MPI_Send(&sub_reduction, 1, MPI_DOUBLE, rank + i, rank, comm);
      break;
    }
    ...
    // If it does, then receive from its partner.
      ...
      MPI_Status status;
      MPI_Irecv(
        &received, 1, MPI_DOUBLE, rank - i, MPI_ANY_TAG,
  ...
  // The last process broadcasts the result using a balanced binary
  // tree rooted at it.

  // Put a barrier so that all processes get here.
  MPI_Barrier(comm);
...
int main(int ac, char *av[]) {
  ...
  start = MPI_Wtime();
  Allreduce(sbuf, rbuf, vsize, MPI_SUM, MPI_COMM_WORLD);
  stop = MPI_Wtime();
