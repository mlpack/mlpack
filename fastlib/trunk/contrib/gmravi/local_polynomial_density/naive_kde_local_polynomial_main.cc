...
int main(int argc, char *argv[]){
 ...
 if(strcmp(true_density_file_name,"default")){
  ...
  if(strcmp(bandwidth_file,"default")){
    ...
    bandwidths.Init(query.n_rows(),3);
  ...
  if(strcmp(true_density_file_name,"default")){
    ...
    //Since true density has been provided
  ...
  printf("Query is...\n");
  printf("rows:%d, cols:%d\n",query.n_cols(),query.n_rows());

  printf("Bandwidths is..\n");*/
  bandwidths.PrintDebug();
  naive_kde_lp.Init(query,references,
		    bandwidths.GetColumnPtr(0),
		    bandwidths.n_rows());
  ...
  if(bandwidths.get(0,0)<0){
    ...
    bandwidths.SetAll(naive_kde_lp.get_plugin_bandwidth()); 
  ...
  if(!strcmp(fx_param_str(nklpm,"naive_kde","false"),"true")){
    ...
    naive_kde.Init(query,references,bandwidths.get(0,1)); 
    naive_kde.Compute();
  ...
  if(!strcmp(fx_param_str(nklpm,"local_likelihood","false"),"true")){
    ...
    printf("starting local likelihood caclulationtions...\n");
