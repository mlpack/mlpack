...
double ComputeDTWAlignmentScore(int b,
  ...
  if(locked_features) {
    ...
    return ComputeDTWAlignmentScoreLockedFeatures(b, x, y, &best_path);
  }
  else {
    return ComputeDTWAlignmentScoreUnlockedFeatures(b, x, y);
  }
}
...
double ComputeDTWAlignmentScoreUnlockedFeatures(int b,
  ...
  for(int k = 0; k < n_features; k++) {
    ...
    ArrayList< GenVector<int> > best_path;
    score += (ComputeDTWAlignmentScore(b, x_cur_ts, y_cur_ts, &best_path));
  }
  return score;
}
...
double ComputeDTWAlignmentScoreLockedFeatures(int b,
  ...
  for(int i = 1; i <= n_x; i++) {
	...
	la::DistanceSqEuclidean(n_dims,
				x.GetColumnPtr(i - 1), y.GetColumnPtr(j - 1));
