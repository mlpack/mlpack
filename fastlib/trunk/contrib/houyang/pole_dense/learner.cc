...
void Learner::OnlineLearn() {
  ...
  // Get input data
  if (read_port_) {
    TR_ = new Data(NULL, port_, false, false);
    TR_->ReadFromPort();
  }
  else {
    if (fn_learn_ != "") {
      TR_ = new Data(fn_learn_, 0, random_data_, center_data_);
      TR_->ReadFromFile();
    }
    else {
...
void Learner::BatchLearn() {
  ...
  if (fn_predict_ != "") {
    if (fn_predict_ != fn_learn_) {
      TE_ = new Data(fn_predict_, 0, false, center_data_);
      TE_->ReadFromFile();
    }
    else {
    ...
    cout << "No testing file provided! Training only!" << endl;
  }
}
...
///////////////////////////////
//bool Learner::GetImmedExample(Data *D, Col<T_VAL>** x_p, T_LBL* l_p, T_IDX tid) {
bool Learner::GetImmedExample(Data *D, T_IDX &data_idx, T_IDX tid) {
  T_IDX ring_idx = 0;
  ...
  if (epoch_ct_ < n_epoch_) {
    ...
    D->GetExample(ring_idx, data_idx);
...
double Learner::LinearPredictBias(const Col<T_VAL> &w, const Col<T_VAL> &x, double bias) const {
  return dot(w, x) + bias;
}

T_LBL Learner::LinearPredictBiasLabelBinary(const Col<T_VAL> &w, const Col<T_VAL> &x, double bias) const {
  if (dot(w, x) + bias > 0.0)
    return (T_LBL)1;
  else
    return (T_LBL)-1;
