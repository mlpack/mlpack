...
void Pole::ParseArgs(int argc, char *argv[]) {
  ...
  desc.add_options()
    ...
    ("center", po::value<bool>(&L_->center_data_)->default_value(false), 
     "Center the input examples. Default: do not center.")
    ("random", po::value<bool>(&L_->random_data_)->default_value(true), 
     "Randomly permute the input examples. Default: do randomly permute.")
