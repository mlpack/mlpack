...
class Learner {
 ...
 public:
  ...
  bool GetImmedExample(Data *D, T_IDX &data_idx, T_IDX tid);

  virtual void Learn() {};
  virtual void Test() {};

  double LinearPredictBias(const Col<T_VAL> &w, const Col<T_VAL> &x, double bias) const;
  T_LBL LinearPredictBiasLabelBinary(const Col<T_VAL> &w, const Col<T_VAL> &x, double bias)const;
};
