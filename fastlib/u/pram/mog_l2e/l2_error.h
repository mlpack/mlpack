 ...
 * @file l2_error.h
 *
 * Description of the L2 error function 
 * we are trying to minimize, and the 
 * functions used to calculate the 
 * L2 error.
 * 
 * This namespace also includes two optimizers
 * which minimize the L2 error
 *
 */
...
namespace l2computations{
   ...
   */
  long double L2Error(Matrix&, index_t, double*);
   ...
   * mixture
   */
  long double L2ErrorWithGradients(Matrix&, index_t, double*, Vector*);
   ...
   */ 
  long double RegularizationTerm_(MoGL2E&);
   ...
   * the regularization part of the error
   */ 
  long double RegularizationTermWithGradients_(MoGL2E&, Vector*);
   ...
   */ 
  long double GoodnessOfFitTerm_(Matrix&, MoGL2E&);
   ...
   * the goodness-of-fit part of the error
   */ 
  long double GoodnessOfFitTermWithGradients_(Matrix&, MoGL2E&, Vector*);
   
  /**
   * The function that modifies the simplex in the 
   * parameter space to move to a better position.
   * 
   * Used by the 'PolytopeOptimizer' function to change
   * the shape of the simplex
   */ 
  long double ModifySimplex_(double**, long double[], double[], index_t,
			     long double (*funk)(Matrix&, index_t, double*),
			     index_t, float, Matrix&, index_t) ;
  
  /**
   * This function minimizes the 'L2Error' function
   ...
   * bool converged;
   * 
   * converged = l2computations::PolytopeOptimizer(simplex_vertices,
   *                                               function_values,
   *                                               dim, tolerance_value,
   *                                               l2.L2Error,&number_of_function_evaluations,
   *                                               data,number_of_gaussians);
   * @endcode
   */
  bool PolytopeOptimizer(double**, long double*, index_t, long double,
   ...
   * and its gradients
   * 
   * This function is used by the 'QuasiNewtonOptimizer' optimizer.
   */
  void LineSearch_(index_t, Vector, long double, Vector*, Vector*, Vector*,
   ...
   * index_t number_of_gaussians;
   *
   * l2computations::QuasiNewtonOptimizer(point, dim, gradient_tolerance_value,
   *                                      &number_of_iterations, minimum_function_value_obtained,
   *                                      l2.L2Error, data, number_of_gaussians);
   * @endcode
   */
  void QuasiNewtonOptimizer(double*, index_t, double, index_t*, long double*, 
			    long double (*func)(Matrix&, index_t, double*, Vector*), 
			    Matrix&, index_t);
  
  long double L2Error(Matrix& d, index_t num_gauss, double* theta) {
    MoGL2E mog;
    ...
    reg = RegularizationTerm_(mog);
    fit = GoodnessOfFitTerm_(d, mog);
    l2e = reg - 2 * fit / number_of_points ;
    return l2e;
  }
	

  long double L2ErrorWithGradients(Matrix& d, index_t num_gauss, 
				   double* theta, Vector *g_l2error) {
    MoGL2E mog;
    long double reg, fit, l2e; 
    index_t dim, number_of_points;
    ...
    reg = RegularizationTermWithGradients_(mog, &g_reg);
    fit = GoodnessOfFitTermWithGradients_(d,mog, &g_fit);
    l2e = reg - 2*fit / number_of_points ;
    double alpha = -2.0 / number_of_points;
    la::ScaleInit(alpha, g_fit, &tmp_fit);
    la::SubOverwrite(tmp_fit, g_reg, g_l2error);
    return l2e;
  }


  long double RegularizationTerm_(MoGL2E& mog) {
  ...
  long double RegularizationTermWithGradients_(MoGL2E& mog, Vector *g_reg){
    Matrix phi_mu, sum_covar;
    Vector x, y;
    long double reg, tmpVal;
    index_t num_gauss, dim;
  ...
  long double GoodnessOfFitTerm_(Matrix& data, MoGL2E& mog) {
    long double fit;
    Matrix phi_x;
  ...
  long double GoodnessOfFitTermWithGradients_(Matrix& data, MoGL2E& mog, Vector *g_fit) {
    long double fit;
    Matrix phi_x;
    Vector weights, x, y, identity_vector;
  ...
  bool PolytopeOptimizer(double **p, long double y[], index_t ndim, long double ftol,
			 long double (*funk)(Matrix&, index_t, double*),index_t *nfunk, 
			 Matrix& data, index_t k_comp) {

    index_t i, j, ihi, ilo, inhi,mpts = ndim + 1, NMAX = 50000;
    ...
    *nfunk = 0;
	...
	psum[j] = sum / ndim;
      ...
      ytry = ModifySimplex_(p, y, psum, ndim, funk, ihi, -1.0, data, k_comp);
      if( ytry <= y[ilo] ) {	
	// result better than best point 
	// so additional extrapolation by a factor of 2
	ytry = ModifySimplex_(p, y, psum, ndim, funk, ihi, 2.0, data, k_comp);
      }
      else if( ytry >= y[ihi] ) { 
	...
	ytry = ModifySimplex_(p, y, psum, ndim, funk, ihi, -0.5, data, k_comp);
	if( ytry > y[ihi] ) { 
  ...
  long double ModifySimplex_(double **p, long double y[], double psum[], index_t ndim,
  ...
  void QuasiNewtonOptimizer(double *p, index_t n, double gradient_tolerance,
			    index_t *number_of_iterations, long double *f_min,
			    long double(*func)(Matrix&, index_t, double*, Vector*),
			    Matrix& data, index_t k_comp) {

    index_t check, i, its, MAXIMUM_ITERATIONS = 200;
    ...
    for(its = 0; its < MAXIMUM_ITERATIONS; its++) {
      *number_of_iterations = its;
      ...
      LineSearch_(n, pold, f_previous, &grad, &xi,
  ...
  void LineSearch_(int n, Vector pold, long double fold, Vector *grad,
