...
Figure \ref{fastlib_archi} shows the overall organization of FASTlib. FASTlib is designed with the goal of being:


\begin{itemize}
\item scalable - the code should be able to handle large-scale machine learning problems and programmers should have the ability to 
\item flexible - the design is modular so that only the core library is necessary and components can be plugged in as needed.
\item easy to use - provides simple and consistent interface, a rich set of API, and rich set of compilation and debugging tools to enable ease of use. 
\item $\cdots$
\end{itemize}  
