...
maximize $Trace(K)$
\begin{itemize}
  \item subject to $Trace(F_iK)=c_i, i=1,\dots,kN$
  \item $K\geq 0$
\end{itemize}
...
\begin{itemize}
  ...
  with ones,
  \item $F_i$ is a sparse matrix with nonzero values (set to 1)
  the $\{n, n\}, \{m,m\}, \{n, m\}, \{m, n\}$ elements. In other words
  $F_i$ represents the link between the $n$ point and its neighbor $m$
  \item $c_i =G_{ii}+G_{jj}-G_{ij}-G_{ji}$
\end{itemize}
\end{minipage}
}
