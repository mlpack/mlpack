...
\begin{figure}[b]
  ...
  \fbox{$$
    \begin{array}[t]{l}
      ...
      \x \text{if }\distlo(\outstat(X), \outstat(Y)) > w\text{, return }0
      \x \text{if }\distup(\outstat(X), \outstat(Y)) \leq w\text{, return } |X| \cdot |Y|
...
%\authorsnote{Should probably show in a table how tree building, etc takes for the algorithms}
