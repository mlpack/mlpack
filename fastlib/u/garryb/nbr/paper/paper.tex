% To-do list
%
% - Draw a picture of locally essential trees
...
Applying machine learning methods to large datasets is increasingly critical for many applications.
Unfortunately, powerful nonparametric methods that analyze all pairs of distances are commonly $O(N^2)$, given $O(N)$ test (or query) points and training (or reference) points.
For a significant subclass of these problems, which we call \defterm{generalized $N$-body problems (GNP's)}, a higher-order divide-and-conquer technique pioneered in \cite{gray_nbody} can avoid nearly all computation by skipping combinations of regions with little mutual interaction.
% TODO: balltrees
...
This approach is used by some of the fastest known serial algorithms for many statistical and physical problems on metric data, such as bounded-error kernel density estimation \cite{gray_nbody, gray_kde, lee_gauss1, lee_gauss2},  $k$-nearest-neighbor classification \cite{ting-liu}, kernel discriminant analysis \cite{nbc-compstat}, nearest neighbor search \cite{gray_nbody}, $n$-point correlation \cite{gray_nbody, moore01fast}, and more.
For each of these problems, no overall faster algorithms are known.
...
Many current and future data mining techniques remain to be treated in depth with this strategy, not to mention a large array of related fundamental problems in computational geometry, physics, and applied mathematics.
...
We catalog the parallel implementation of this model, THOR, and show its performance when applied to several realistic massive data mining problems, with the intial version demonstrating up to an 81-fold speedup on 90 processors.
...
Ball trees \cite{anchors_balltrees_moore} are binary and uses bounding radii, work with arbitrary metric spaces including non-Euclidean, and are believed to have good high-dimensional properties.
...
In the \defterm{monochromatic} case the query and reference sets are the same, but may be distinct in the \defterm{bichromatic} case.
...
\begin{figure}[t]
  ...
  \fbox{$$
   \begin{array}[t]{l}
     \text{for each query }q \in \kdroot{Q}\text{,}
     \x \text{initialize }a \gets \infty
     \x \text{define singlenn}(q,R)\text{,}
     \xx \text{if }a < \distlo(q, \outstat(R))\text{, return}
     \xx \text{else if }R = \{r\}\text{,}
     \xxx \text{if }q \neq r\text{, }a \gets \dist{q}{r}
     \xx \text{else, prioritize by node distance,}
     \xxx \text{singlenn}(q, \kdleft{R}); \text{ singlenn}(q, \kdright{R})
     \x \text{singlenn}(q, \kdroot{R})
   \end{array}
  $$}
  \caption{\label{fig:singlenn}
    Single-tree nearest-neighbor search, from \cite{friedman_bentley_finkel}.}
\end{figure}
...
\begin{figure}[t]
  ...
  \fbox{$$
   \begin{array}[t]{l}
     \text{for each node }Q\text{ in tree } \kdroot{Q}\text{, initialize }a(Q) \gets \infty
     \X \text{define allnn}(Q,R)\text{,}
     \x \text{if }a(Q) < \distlo(\outstat(Q), \outstat(R))\text{, return}
     \x \text{else if }Q = \{q\} \text{ and } R = \{r\}\text{,}
     \xx \text{if }q \neq r\text{, }a(\{q\}) \gets \min(a(\{q\}), \dist{q}{r})
...
In applications from manifold learning to classification, one must compute the nearest point from reference set $R$ for each of a batch of queries $Q$.
We express this computation,
\[\map_{q \in Q} \argmin_{r \in R} d(q,r).\]
\noindent
Friedman, Bentley, and Finkel in \cite{friedman_bentley_finkel} pioneered the use of $kd$-trees in nearest neighbor search, creating the single-tree algorithm in \fig{singlenn}.
The code in \fig{allnn} additionally recurses on the query as hinted by \cite{gray_nbody}.
The variable $a(Q)$ maintains the furthest candidate neighbor distance for each node of queries $Q$.
%The dual-tree algorithm has $O(N)$ runtime characteristics whereas the single-tree has $O(N \log N)$.
...
By targeting only this class of problems and taking advantage of lack of inter-datum dependencies, MapReduce can use optimally efficient data decomposition methods, out-of-core sorting algorithms, and mechanisms for fault-tolerance.
...
It is imperative to state that although many machine learning tasks are simple in MapReduce, generalized $N$-body problems are an extremely poor fit for the model\footnote{
  ...
  Single-tree algorithms suffer from the same problem, but are marginally simpler to implement on MapReduce.
  }.
...
We show an extremely general formalization of the problems solved.
...
For example, two-point correlation can compute the range count for each query as a $\map$ of $\sum$, postprocessing with $h$ to add the list of range counts.
...
%Nonetheless,
Although we have already shown a general model of the {\it problems}, we next characterize the {\it algorithms} used.
...
%\mysub{Single-Operator Reduce Problems}
%
...
\begin{figure}
  ...
  \begin{array}[t]{l}
    ...
    \X \text{return }\outqr
    \X
    \X \text{define Recurse}(Q, R, \varmuparent)\text{, \com{explore a node pair}}
...
%, but in the common case that there are significantly more queries than references, the extra communication overhead is quite small.
...
We use a Gaussian kernel at its optimal bandwith, determined using least-squares cross validation via the Gaussian convolution kernel.
...
%    \\ \hline
%    KDE         & Colors  & 4          &               &  ??          & ??     & 1,000,000    & ??
%    \\ \hline
%    Range Count & LCDM  & 3          & $w$ @ 50\%ile &  ??         & ??     & 3,000,000    & ??
%    \\ \hline
%    Range Count & LCDM  & 3          & $w$ @ 10\%ile &  ??         & ??     & 16,000,000   & ??
%    \\ \hline
%  \end{tabular}
%  \caption{
...
In the previous subsection, we divide the query tree into pieces and execute them on a single processor.
By executing these threads on two processors, we measure simple multi-threaded performance.

On a dual-core AMD Athlon 2.0 GHz processor...
97\%....
median of three...

write a short section like above...
...
A modified computational model would allow efficient execution of 3-tree, 4-tree, and further multi-tree algorithms, to solve problems such as 3-point correlation that currently have scant efficient parallel implementations.
