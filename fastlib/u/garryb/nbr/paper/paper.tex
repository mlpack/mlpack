...
Recent work\authorsnote{not yet officially published}\cite{ryan_nips} has characterized these {\it generalized $N$-body problems} (GNP's) and shown how a multi-tree algorithmic approach directly follows, leading quite often to asymptotically faster algorithms, sometimes nearly linear.
...
{\bf Notation.}
Here, we define some basic notation that we use throughout the paper.
Every node $X$ in a spatial tree $\kdroot{X}$ is a set of points.
Every internal node is partitioned $\kdleft{X} \union \kdright{X} = X$ with parent $\kdparent{X}$, with each leaf a single point $\{x\}$\footnote{In practice, leaves contain multiple points to amortize recursion overhead.}.
...
{\bf Single-tree versus dual-tree.}
...
For each query point, a single-tree algorithm must start at the root of the tree and traverse until pruning occurs, and is at best $O(N log N)$.
...
Empirical studies show that dual-tree algorithms significantly overtake similar single-tree algorithms for datasets with millions or more points\authorsnote{Verify}.
...
{\bf Two-point correlation and range count.}
The fundamental two-point-correlation statistic looks at the number of pairs of points within fixed radius $h$.
For data set $X$, radius $h$, and indicator function $I$, two-point correlation is
\[\sum_{x \in X} \sum_{y \in X} I(d(x, y) \leq h).\]
\noindent Figure~\ref{fig:allnntpc} shows a formulation that recursively considers a subset pair $Y$ and $X$, returning immediately if all points in $Y$ and $X$ are completely inside or outside the radius.
...
Computational solutions break the simulation into discrete time steps, during which force is calculated as a query-reference problem,
...
\authorsnote{Should this go in the intro?}
...
Extrinsically\authorsnote{sp?} pruning problems that contentrate on spatially nearby computations may prefer a depth-first expansion along with an ordering heuristic .
...
We define \defterm{query summary results} $\inmu(Q, R)$ as a compact representation of only those summary results relevant for the computation $(Q,R)$.
...
A dual-tree algorithm bounds $\inmu$ using summary statistics,
\begin{equation*}
\inmu(Q, R) \gets \deltamu(\outstat(Q), \outstat(R)).
\label{eqn:approxmu}
\end{equation*}

\noindent Next, if we know $\inmu$ for disjoint reference sets, we may compose them in a way analagous to the original $\Opqr$,
\begin{equation*}
...
Ultimately $\inqrv(Q, R)$ will be propagated\authorsnote{sp?} to individual queries, but until that happens, we bound the prune's worst-case contribution:
\begin{equation*}
...
The quality of $\inmu$ generally increases as more node pairs are explored, but the order of exploration is extremely important\authorsnote{This line reads funny}.
...
%\authorsnote{Does $\deltaqrv$ want to take in $\lettermu$ as a parameter?}.
...
Parallelization of any algorithm requires addressing the way work is divided, the communication that is necessary, and how work imbalances are kept to a minimum.
...
Finally, it is worth noting that past parallelization of both Barnes-Hut and the FMM universally divide queries among processors \cite{all, of, the, nbody, parallelization, papers}.
...
\authorsnote{Mention what a locally essential tree is}
\authorsnote{Look at dichromatic}
...
Consistent with previous literature\cite{warren92astrophysical, salmon90thesis, singh-first}, we refer to this subset of reference nodes and points as a \defterm{locally essential tree}.
...
Salmon's earlier distributed implementations of Barnes-Hut \cite{salmon90thesis} decompose via orthogonal recursive bisection (ORB), or $kd$-tree that is balanced by estimated runtime.
Each processor is assigned to a leaf in the orthogonal recursive bisection.
The spatial contiguity of the leaf parallelepipeds\authorsnote{Verify this word parallelepiped} generally leads to good overall spatial locality.
...
Singh in \cite{singh93thesis} argues that the high data requirements of $N$-body methods prevent efficient distributed-memory implementations, instead recommending costly shared-memory architectures to achieve parallelization.
However, several authors \cite{salmon90thesis, liu94experiences, salmon97parallel} have made successful implementations of the classic force-calculation methods.
...
Initially, Salmon\cite{salmon90thesis}, and Liu and Bhatt\cite{liu94experiences} promote the idea of sender-initiated communication.
...
One array contains points, with blocks roughly 100 points or 8 kilobytes; the nodes array block has 8 nodes or 2 kilobytes\footnote{These sizes depend on the communications network, on the order of latency times transfer rate.}.
...
Previous work \cite{salmon90thesis, singh93thesis} on scheduling $N$-body force calculation problems assume that relative run-time can be predicted from prior iterations.
...
\begin{table*}
  \begin{tabular}{|l|l|l|r||r|r||r|r|}
    ...
    \\ \hline
    Range Count & Galaxy  & 3          & $h$ @ 50\%ile &  100,000         & 1.0 s       & 16,000,000   & 30.0s
    \\ \hline
    Range Coutn & Galaxy  & 3          & $h$ @ 10\%ile &  100,000         & 1.0 s       & 16,000,000   & 30.0s
    \\ \hline
  \end{tabular}
  \caption{
...
In all cases, the depth-first algorithms run at quite comparable speeds, with the best slowdown WALDO\% for WALDO and the worst WALDO\% for WALDO algorithm.
