...
Many problems in spatial statistics and data mining nominally require an all-pairs analysis of metric data.
Recent work\cite{ryan_nips} has formally characterized these {\it generalized $N$-body problems} (GNP's) and shown how a multi-tree algorithmic approach directly follows, leading quite often to asymptotically faster algorithms, sometimes nearly linear.
The multi-tree approach treats these problems as a hierarchical decomposition of the work over a Cartesian product of points, resulting in the fastest known practical serial algorithms for many statistical and physical problems, such as bounded-error kernel density estimation, $n$-point correlation, nearest neighbor finding, Gaussian process regression, and more.
...
Additionally, some data sets too massive to be handled practically on a single processor become practical if the algorithm is massively parallelized.
Fortunately, multi-tree algorithms are in fact quite parallel due to their inherent decomposition.
Techniques that have long been used to parallelize the physical $N$-body force calculation methods can be, to an extent, applied to all GNP's.

Unfortunately, parallel programming often takes a severe toll on development time that is often not worth the speedup achieved.
The formal characterization of GNP's leads to an intriguing solution: Write a parallel solver for the general problem, allowing users to replace problem-specific portions.
We show that this approach leads to efficient parallel and serial performance, often reducing the development to the extent that the parallel algorithm may take less time to develop than a from-scratch serial algorithm.
    ...
    \begin{minipage}{2.5in}
      \begin{displaymath}
        \begin{array}[t]{l}
          ...
          \x \text{if }\distup(X, Y) > h\text{, return }0
          \x \text{if }\distlo(X, Y) \leq h\text{, return } |X| \cdot |Y|
...
Here, we introduce the concept of spatial trees and define some basic notation that we use throughout the paper.
Every node $X$ in a spatial tree $\kdroot{X}$ is a set of points, which lie in a metric space, usually a vector space.
Every internal node is partitioned $\kdleft{X} \union \kdright{X} = X$ with parent $\kdparent{X}$, with each leaf a single point\footnote{In practice, leaves contain multiple points to amortize recursion overhead.} $\{x\}$.
...
We denote $\dist{x}{y}$ to be a distance metric, such as $L_2$ or Euclidean distance $||x-y||$.
...
A \defterm{single-tree} algorithm replaces the inner loop with a tree traversal, by skipping parts of the tree that are irrelevant to the query.
...
The dual-tree algorithm prunes pairs of nodes from consideration when possible.
As a note, dual-tree algorithms can solve problems such as two-point correlation that have no intrinsic queries versus references differentiation, such as if only a single scalar result is computed, but such nomenclature simplifies our discourse with a small loss in generality.
...
A dual-tree instead algorithm considers the root-level nodes only with other root-level nodes, therefore amortizing the cost of considering the root-level nodes, and can be $O(N)$ in practice.
Empirical studies show that dual-tree algorithms significantly overtake similar single-tree algorithms for datasets with millions or more points\authorsnote{I need to find the proximity project to justify this claim.}.
Such performance for large datasets make dual-tree algorithms a prime candidate for parallelization as a way to handle larger data sets.

\mysub{Two-point correlation and range count}
The two-point-correlation statistic looks at the number of pairs of points within fixed radius $h$.
For data set $X$, radius $h$, and indicator function $I$, two-point correlation is
\[\sum_{x \in X} \sum_{y \in X} I(d(x, y) \leq h).\]
\noindent Figure~\ref{fig:allnntpc} shows a formulation that recursively considers a subset pair $X$ and $Y$, returning immediately if all points are completely inside or outside the radius.
...
Range count has a similar, but slightly more complex, dual-tree algorithm, that we enumerate in a further section.

\mysub{All-nearest-neighbors}
In applications from manifold learning to classification, one computes the nearest points from reference set $R$ for a batch of queries $Q$.
...
When considering creating a parallel program, development time may be far more expensive than CPU time.
Making parallelization accessible to a wider audience has been a common goal of parallelization communities.
\authorsnote{This paragraph has some nice discourse that may want to be moved to the introduction.}

One common approach is to create parallel programming languages or language extensions that allow a user to write serial or minimally parallel code and have it run on a cluster.
These languages or extensions may provide terriffic performance for problems with regular access patterns with minimal programming overhead, but leave the developer at the mercy of the language's abilities.
Generalized $N$-body problems rarely exhibit the regularity of which these languages can take advantage.
...
The parallel programming language NESL has implementations for Barnes-Hut and the FMM, but little is published on its parallel scalability \cite{nesl_nbody}.
We find that fine implementation control of parallelizing dual-tree algorithms by hand to be advantageous when considering different communication or scheduling strategies, or when considering future features like fault tolerance.

Another popular approach requires identifying a specific, yet diverse, class of problems that share common properties.
One can create an arbitrarily fine-tuned implementation, but allow the user to plug in the algorithmic specifics.
...
We do build on major points from both authors, prime figures in distributed-memory $N$-body problems, especially their other ground-breaking works.
However, dual-tree algorithms differ fundamentally from single-tree algorithms, and the Hashed Oct-Tree approach to parallelization is fundamentally based on properties of oct-trees.

Instead, we use a recent generalization of dual-tree algorithms and take advantage of it to create parallel code.
...
\[\begin{array}{ccccl}
  ...
  \\
  \multicolumn{3}{r}{f(x,y)} &\equiv& I(d(x,y) < h)
  \\
  \multicolumn{3}{r}{g(x, a)} &\equiv& a
  \\
  \multicolumn{3}{r}{h(b)} &\equiv& b
\end{array}{c}\]
...
\noindent
The above is satisfied trivially by commutativity and associativity if $\bigodot = \bigotimes$; we call this a \defterm{single-operator second-order reduce problem}.
If $\bigodot$ is the higher-order operator $\map$, i.e. a query-reference problem, then a minor key-value pair transform is formally required to satisfy the property, although such formality is not strictly necessary in our discourse.

It is clear that {\it any} second-order reduce problem can be transformed into a query-reference GNP by replacing the outer operator with $\map$, and then treating both $g$ and $\odot$ as a postprocessing step.
...
A dual-tree execution may use a heuristic such as $|X| \geq |Y|$ to prefer considering node pairs with approximately equal cardinality.

This hierarchical computation motivates the popular use of trees in GNP's.
The \defterm{generalized $N$-body algorithm} initially considers the expression $\gnp(\kdroot{X}, \kdroot{Y})$, continually replacing any subexpression $\gnp(X, Y)$ with its aforementioned expansion.
...
Each node in the data tree has a statistic $\outstat(X)$ that may contain arbitrary problem-specific information, such as bounding box, mean, or variance.
Dual-tree algorithms can use these statistics to prove facts about node pairs at a higher level, and \defterm{prune} part of the tree.
A prune based only on statistics is an \defterm{intrinsic prune}.

In some cases, the results of other computations can imply that no further exploration for a node pair is required.
For example, in all-nearest-neighbors, a distant node pair can be excluded if the query node already has closer candidate neighbors.
This type of prune is an \defterm{extrinsic prune}.
...
With intrinsic pruning only, $\prefsplit{X}{Y}$ solely determines the asymptotic runtime.
Extrinsically pruning problems that contentrate on spatially nearby computations may prefer a depth-first expansion along with an ordering heuristic .
Others achieve good performance by a breadth-first expansion, which closely mimics an iterative refinement strategy.
Our paper treats parallel execution as an additional expansion pattern by noting that, with few performance implications, each subproblem can be considered independent.
...
Both algorithms, though, may divide the reference tree,
\begin{equation*}
...
At completion, we desire the \defterm{ideal} value $\inmu(Q, \kdroot{R})$ for each query node, computed
\begin{equation*}
...
\noindent The value of $\inmu(Q, \kdroot{R})$ is then used to make pruning decisions via the extrinsic prune check $\canprunemu(\outstat(Q), \outstat(R), \inmu(Q, \kdroot{R}))$.

During computation, the value of $\outqr(q, \kdroot{R})$ is obviously not known, and thus the ideal value is not attainable.
...
\noindent where $\deltamu$ is a pessimistic approximation.
Next, if we know $\inmu$ for disjoint reference sets, we may compose them in a way analagous to the original $\Opqr$,
\begin{equation*}
\inmu(Q, R_1 \union R_2) \gets \inmu(Q, R_1) \opmu \inmu(Q, R_1).
\label{eqn:combinemu}
\end{equation*}

\noindent It is important to note that the above composition may be quite pessimistic.
...
Next, $\inqrv(Q, R)$ may be defined because this computation was pruned.
...
\mysubsub{Quality} A final note is that extrinsic pruning requires ``high-quality'' values of $\inmu$, that is, values that approach the aforementioned ideal value.
...
\[
  \begin{array}[t]{l}
    ...
    %   \end{array}
    \x \!\!\!\begin{array}{llll}
         \psty\lettermu &\psty\!\!\!\!\gets\!\!\!\!& \psty\varmuchild(Q)                     & \!\!\!\text{\com{bottom-up $\lettermu$}}
         \\             &\psty\!\!\!\!\opmu\!\!\!\!& \psty\fmuv(\outstat(Q), \varqrv(Q))     & \!\!\!\text{\com{mass-result $\lettermu$}}
         \\             &\psty\!\!\!\!\opmu\!\!\!\!& \psty\deltamu(\outstat(Q), \outstat(R)) & \!\!\!\text{\com{node pair's $\lettermu$}}
         \\             &\psty\!\!\!\!\opmu\!\!\!\!& \psty\varmuparent                       & \!\!\!\text{\com{unvisited $\lettermu$}}
       \end{array}
    \x \text{if } \exists q \exists r ~ Q = \{q\}\text{ and }R = \{r\}\text{, \com{base case}}
    ...
    \xx \text{\com{prune the computation to the mass result}}
...
The main premise of this paper is to extend the concept of an abstract expansion pattern to encompass parallel execution.
...
Consistent with previous literature\cite{warren92astrophysical, salmon_thesis, singh_thesis}, we refer to this subset of reference nodes and points as a \defterm{locally essential tree}.
...
\authorsnote{Mention dichromatic problems}
...
Salmon's earlier distributed implementations of Barnes-Hut \cite{salmon_thesis} decompose via orthogonal recursive bisection (ORB), or $kd$-tree that is balanced by estimated runtime.
...
The points are then divided into contiguous segments with roughly equal estimated runtime.
...
THOR combines the strengths from both ideas by building $kd$-trees balanced in a way that each processor can be assigned a tree node.
In a later section, we discuss dynamic adaptations that THOR performs when information about the relative amount of work is unavailable for specific problems.
...
Additionally, we do not assume the existence of a closed-form runtime estimator.
...
THOR divides the query tree into $k$ work items of roughly equal cardinality.
...
The first algorithm, which we call random-assignment, simply assigns a random task to a processor, ignoring the domain decomposition.
The overhead of this scheduler is small -- $O(\hat{k})$ for the decomposition, and $O(\hat{k})$ for the work assignment.
For multithreaded implementations only, random assignment typically allows close to optimal performance.

Our second algorithm takes into account locality when memory is distributed among several machines.
Tasks are initially assigned to machines based on the domain decomposition.
When a machine requests work, a task is assigned from its domain decomposition if possible, but if its tasks have been exhausted, an heuristic is used to steal tasks from other processors.
One simple heuristic that works well in practice is to find an unassigned work item closest to the centroid of tasks already completed by the machine in question.
We efficiently compute the closest work item by using a single-tree nearest-neighbor traversal.\authorsnote{There are some more interesting algorithms out there with solid theoretical backing, but we have not researched them thoroughly.  I should either fix this immediately or leave this to future work.}
...
\authorsnote{Ensure we explain that THOR doesn't parallelize tree-building at this stage.}
...
Our rule of thumb for these is to aim for at least a minute of runtime, still far shorter than the bureaucratic red tape typically involved with obtaining a time slice to such a cluster.
...
Since a run of affinity propagation typically solves 100 or more iterations each solving two generalized $N$-body problems, we calibrate our data set size based on total time.
...
\begin{table*}
  \begin{tabular}{|l|l|l|r||r|r||r|r|}
    ...
    \\ \hline
    Affinity    & Galaxy  & 3          & $p$ @ 50\%ile &  100,000         & 1.0 s       & 16,000,000   & 30.0s
    \\ \hline
  ...
  Affinity propagation times are mean per-iteration.
...
For each test algorithm, we obtained fine-tuned serial C\authorsnote{Do I need to cite Auton here?} code from the original authors of the respective test algorithm papers \cite{various, algorithms, WALDO} in the depth-first expansion pattern\footnote{Affinity propagation's only dual-tree implementation is within THOR, because of its assistance in rapid algorithm development.}.
