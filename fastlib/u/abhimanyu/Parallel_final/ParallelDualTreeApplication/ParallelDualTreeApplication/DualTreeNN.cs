...
namespace Algorithms
{
    public class DualTreeNN
    {
        ...
        public static String OPFILE_SEPERATOR = "\t";
        ...
        /// </summary>
        private void ReadDataPoints(String treeDataSource, int k)
        {
            if (isFile)
            {
                ParseAndStoreData(treeDataSource, k);
            }
            else
            {
        ...
        private void ParseAndStoreData(String fileName, int k)
        {
            int prevNodeId = -1;
            int nodeId;
            ...
            using (StreamReader sr = File.OpenText(fileName))
            {
               ...
               String[] ipSplit = input.Split(new char[] { KDTreeFileWriter.nodeValSeperator });
               nodeId = int.Parse(ipSplit[0]);
               prevNodeId = nodeId;
               idList.Add(int.Parse(ipSplit[1]));
               for (int i = 2; i < k + 2; ++i)
               {
                   doubleList.Add(double.Parse(ipSplit[i]));
               }

               while ((input = sr.ReadLine()) != null) 
               {
                   ipSplit = input.Split(new char[] { KDTreeFileWriter.nodeValSeperator });
                   nodeId = int.Parse(ipSplit[0]);
                    ...
                    if (nodeId != prevNodeId)
                    {
                        ...
                        dataIds.Add(prevNodeId, idList.ToArray());
        ...
        private void ParseAndStoreData(SqlDataReader rdr, int k)
        {
            int prevNodeId = -1;
            int nodeId;
            List<double> doubleList = new List<double>(50 * k);
        ...
        /// </summary>
        private void InitDataStructures(KDTree tree, String treeDataSource, int kNN)
        {
            ...
            ReadDataPoints(treeDataSource, tree.GetDimensionality());
            ...
            for (int i = 0; i < subTreeNumNodes; ++i)
            {
                ...
                {
                    Console.WriteLine("Rank : " + MPIRank + " subtree rt node id" + subTreeRoot.GetNodeId()  + " Node Id: " + nodes[subTreeRootNodeId - 1 + i].GetNodeId());
        ...
        TableDefinition = "QueryPointId int, ReferencePointId int, Distance double",
        FillRowMethodName = "OutputContract")]
        public IEnumerable GetNearestNeighbors(String treeFileName, String dataFileName, int kNN)
        {
            isFile = true; ;
            SetupEnvironment();
            KDTree tree = new KDTree();
            tree.InitializeTree(treeFileName);
            double startTime, endTime;
            startTime = MPI.Environment.Wtime;
            InitMPIVars();
            InitDataStructures(tree, dataFileName, kNN);
            IEnumerable results = DualTreeNearestNeighbors(tree, kNN);
            endTime = MPI.Environment.Wtime;
            double t1 = endTime - startTime;
            Console.WriteLine("Rank " + MPI.Communicator.world.Rank
            ...
            KDTree tree, int kNN)
        {
            ...
            TimeSpan duration;
            Trace.Flush();
            Console.WriteLine("subtree id" + subTreeRoot.GetNodeId()
                + " num points " + subTreeRoot.GetNumPoints()
                + " tree " + tree.GetDataSize());
        ...
        public void SaveResultsToFile(String fileName, bool append)
        {
            if (append)
            {
                if (!File.Exists(fileName))
                {
                    Console.WriteLine("{0} does not exist. Aborting append.", fileName);
                    return;
                }
            }
            else
            {
                if (File.Exists(fileName))
                {
                    Console.WriteLine("{0} already exists. Aborting write.", fileName);
                    return;
                }
            }
            ...
            using (StreamWriter sw = new StreamWriter(fileName, append))
            {
                if (!append)
                {
                    sw.WriteLine("Query Id" + OPFILE_SEPERATOR
                    + "Reference Id" + OPFILE_SEPERATOR + "Distance");
                }

                for (int i = 0; i < results.Count; i++)
                {
                    OutputData op = results[i];
                    sw.WriteLine(op.queryPointId + OPFILE_SEPERATOR
                        + op.referencePointId + OPFILE_SEPERATOR
                        + op.distance);
                }
                sw.Close();
            }
