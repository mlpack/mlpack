...
SingleTreeResults *AllNNSingleTree::find_nearest_neighbour(Tree *root,double *point, Matrix *r_matrix,SingleTreeResults *nearest_nn)
{
  ...
  if(root->is_leaf())
  ...
  else
    {
      ...
      // printf("minimum distance to right child %f\n",min_distance_to_right_child);
      Tree *farther;
      SingleTreeResults *potential_nn;
      if(min_distance_to_left_child < min_distance_to_right_child)
	{
	  ...
	  farther=root->right();
	  potential_nn=find_nearest_neighbour(root->left(),point,r_matrix,nearest_nn);
	}
      else
	{
	  ...
	  farther=root->left();
	  potential_nn=find_nearest_neighbour(root->right(),point,r_matrix,nearest_nn);	
	}
	...
	{
	  SingleTreeResults *potential_nn=find_nearest_neighbour(farther,point,r_matrix,nearest_nn);
	  if(potential_nn->get_distance()<nearest_nn->get_distance()) 
	    {
	      nearest_nn->set_result(potential_nn->get_distance(),potential_nn->get_index());
	      return nearest_nn;
...
void AllNNSingleTree::ComputeAllNNSingleTree(Matrix* q_matrix, Matrix  *r_matrix)
{
  ...
  Tree *root=tree::MakeKdTreeMidpoint<Tree>(*r_matrix,1,NULL,NULL); //here the leaf size is set to 1
  ...
  for(int i=0;i<q_matrix->n_cols();i++)
  {
    ...
    str_[i]=*find_nearest_neighbour(root,q_matrix->GetColumnPtr(i),r_matrix,nearest_nn);
...
int push_into_array(SingleTreeResults *str,int position, double dist,int length, int index,int k)
{
  ...
  SingleTreeResults *temp=new SingleTreeResults[length];

  if(position==0)
    {
      str[0].set_result(dist,index);
      for(int t=length;t>0;t--)
	str[t].set_result(str[t-1].get_distance(),str[t-1].get_index());
      return 1;
    }
  ...
  for(int i=0;i<position;i++)
    temp[i].set_result(str[i].get_distance(),str[i].get_index());

  //printf("Position is %d\n",position);
  //push the element at the position
  temp[position].set_result(dist,index);

  //printf("Reuslt set\n");
  //printf("Temp[position] is dist:%f index:%d\n",temp[position].get_distance(),temp[position].get_index());
  int t;
  for(t=position+1;t<length;t++)
    {
      //  printf("will do rest of work\n");
  ...
  if(length<k)
      ...
      for(int j=0;j<length+1;j++)
	{
	 ...
	 printf("Distance:%f index:%d", str[p].get_distance(),str[p].get_index());*/
      return 1;
    }
  ...
  if(root->is_leaf())
      ...
      for(int i=root->begin();i<root->end();i++)
	{
	  ...
	  if(!same_points(point,r_matrix->GetColumnPtr(i),r_matrix->n_rows()))
		...
		{
		  //  printf("In str Distance:%f\n",str[t].get_distance());
		  // printf("In str Index:%d\n",str[t].get_index());
		}
	    }
	...
	{
	  //check the other half only if the kth nn distance is greater than the distance of thje point form the farther bounding box
	  if(str[k].get_distance()> farther->bound().MinDistanceSq(point))
	    {
 ...
 void AllKNNSingleTree::ComputeAllKNNSingleTree(Matrix *q_matrix,Matrix *r_matrix,int k)
{
  ...
  results_matrix.astr=(SingleTreeResults**)malloc(r_matrix->n_cols()*sizeof(SingleTreeResults*));
  for(int i=0;i<r_matrix->n_cols();i++)
    results_matrix.astr[i]=(SingleTreeResults*)malloc(k*sizeof(SingleTreeResults));
