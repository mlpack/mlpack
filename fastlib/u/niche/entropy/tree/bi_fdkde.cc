...
/**
 * Optimal L2E bandwidth selection for Kernel Entropy Estimation
 * Prefer infinite support kernels (e.g. Gaussian)
 */
...
public:
  ...
  static double DoKdeEntropyL2E(datanode *module,
    ...
    ArrayList<double> test_entropy_estimates;
    test_entropy_estimates.Init();
    ...
    */
	...
	for (index_t i = 0; i < param->q_count; i++,
	  ...
	  if(unlikely((*q_points_iter).index() == param->r_count)) {
	    min_linspace = (*q_points_iter).vec()[0];
	  }
	...
	index_t num_linspace = param->q_count - param->r_count;

	Vector ref_densities;
	Vector linspace_densities;
       
	densities.MakeSubvector(0, param->r_count,
				&ref_densities);
	densities.MakeSubvector(param->r_count, num_linspace,
				&linspace_densities);
	...
	double sum_f_log_f = 0;
	double sum_f_log_f_squared = 0;
	for(index_t i = 0; i < num_linspace; i++) {
	  double p = linspace_densities[i];
	  if(p > 0) {
	    double p_log_p = p * log(p);
	    sum_f_log_f += p_log_p;
	    sum_f_log_f_squared += pow(p_log_p, 2);
	  }
	}

	double entropy_estimate =
	  linspace_range * sum_f_log_f / ((double)(num_linspace));

	double first_term = 
	  linspace_range * sum_f_log_f_squared / ((double)(num_linspace));
	
	double score = first_term - 2 * second_term;

	printf("first_term = %f\n", first_term);
	printf("second_term = %f\n", second_term);
	printf("score = %f\n", score);

    
	test_bandwidths.AddBackItem(param->bandwidth);
	test_scores.AddBackItem(score);
	test_entropy_estimates.AddBackItem(entropy_estimate);
    ...
    printf("bandwidth\tscore\tentropy estimate\n");
    
    for(index_t i = 0; i < test_bandwidths.size(); i++) {
      printf("%f\t%f\t%f\n",
	     test_bandwidths[i],
	     test_scores[i],
	     test_entropy_estimates[i]);

    }

    printf("optimal entropy estimate: %f\n", *(test_entropy_estimates.last()));
