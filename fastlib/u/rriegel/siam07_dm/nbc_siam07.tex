...
%Silverman provides some theoretical insights on how a data set's
%variance and size should inform bandwidth choice \cite{silverman86},
...
its midpoint.
%\footnote{When parallelizing, as discussed later, it is beneficial to
%use median splits for the first few levels of the tree.}
...
\begin{figure}[tb]
  ...
  \begin{displaymath}\begin{array}{l}
    ...
    \x \xinit {\rm next\_work} = \{\} \text{ // filled below}
    \x \xfor \xeach Z_{1,2} \in {\rm work}
    \xx \xif \hi{d}(X,Z_{1,2}) \leq \min\{h_1,h_2\},
    \xxx \text{// Inclusion prune; add to parabola}
...
%    \xxx T_1 \peq T(Z_1);\:T_2 \peq T(Z_2)
    ...
    \xxx \text{// No prune; record contribution}
...
%    \xxx \hi{\Phi_2} \peq W(Z_2) K_{h_2}(\lo{d}(X,Z_2))
    \xxx \text{add $\kdleft{Z}$ and $\kdright{Z}$ to } {\rm next\_work}
    \xx \xelse \text{// Exclusion prune; do nothing}
    \x \text{// Account for inclusion prunes}
...
%    \x \hi{\Phi_2} \peq W_2 K_{h_2}(\lo{\delta}(X,W_2,S_2,T_2))
    ...
    \x \kda(\kdleft{X},{\rm next\_work},W_{1,2},S_{1,2},T_{1,2})
    \x \kda(\kdright{X},{\rm next\_work},W_{1,2},S_{1,2},T_{1,2})
...
\begin{figure}[t]
  ...
  \begin{displaymath}\begin{array}{l}
    ...
    \xxxx \text{// Inclusion prune; add to parabolas}
...
%    \xxxx \vec{T}_1{\scriptstyle \!(i)} \peq T(Z_1)
    ...
    \xxxx \text{// No prune; record contributions}
    ...
    \xxx \xelse \xbreak
    \xx \text{// No work for exclusion prunes}
