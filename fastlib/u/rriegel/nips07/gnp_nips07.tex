...
\end{array} \]
where $\cpos{i}{j}(x)$ is $x$ if $i = j$ and $\max(0,x)$ otherwise;
likewise, $\cneg{i}{j}(x)$ for $\min(0,x)$.  To stem oscillations, it
is customary to mix the updated values of $R$ and $A$ and their
previous values according to some damping factor $\lambda$.  For $N$
...
We observe that the undamped affinity propagation may be posed as two
second-order GNPs, $\vecalpha$ and $\vecrho$, derived as follows.
