...
We observe that affinity propagation may be posed as two second-order
GNPs, $\vecalpha$ and $\vecrho$, derived as follows.  Rearrange computation
of $A$ and $R$ with
\[ \begin{array}{ll}
...
\end{array} \]
Note that $\falphaj{i}{j}$ depends on $j$ only for exclusion from
$\min$.  This is achieved computationally by finding the first and
second minima and returning the later if $j$ is the index of the
first.  We further substitute
\[ \begin{array}{rcl}
...
\end{array} \]
Both $\vecalpha$ and $\vecrho$ are GNPs by Lemma~\ref{lem:map}.  When
points are in a metric space, thereby permitting bounding radii, and
similarity is a monotonically decreasing function of distance, thereby
allowing us to bound it given bounding radii, $\vecalpha$ and
$\vecrho$ support extrinsic and intrinsic prunes, respectively.

\begin{lemma}[Pruning in $\vecalpha$]
  We may perform extrinsic pruning when bounds on a component's
  results render them greater than the greatest second minimum found
  so far.
\end{lemma}
\begin{proof}
  Let $\sigma$ find bounding radii for points in $\mathcal{X}$ as well
  as bounds on the current values of $\vecalpha$ and $\vecrho$
  corresponding to the points.  Let $\simhrectmax$ and $\simhrectmin$
  find upper and lower bound similarities between bounding radii and
  let $\falphamax$, $\falphamin$, $\frhomax$, and $\frhomin$ extract
  the bounds on their respective vectors.  Summary
  results\footnote{Shown ranges are squared because summary results
  must be found for both minima.  Similarly, $\falphamax$ and
  $\falphamin$ form bound for both minima.} are
  \[
...
these algorithms shown in Figure~\ref{fig:alpharho}.  Combined, these computations 

We now have a problem nearly\footnote{To aid convergence, affinity
propagation adds noise to the similarity matrix and dampens both
$\respo{}{}$ and $\avail{}{}$ between time-steps.  Since we store no
matrices, we both dampen and add noise to $\vecrho$.} indentical to
traditional affinity propagation.  However, we can show that this is a
